...
// alef:hash:0d099055543cb32fdbf6d008110f8bf4ee6175a052e7876813e7a04735155507
...
func Test_SmokeImagePng(t *testing.T) {
	...
	if result.Metadata.Format != nil {
		metadataFormat = fmt.Sprintf("%v", *result.Metadata.Format)
	}
	if strings.TrimSpace(string(result.MimeType)) != `image/png` {
		t.Errorf("equals mismatch: got %v", result.MimeType)
	}
	if strings.TrimSpace(string(metadataFormat)) != `PNG` {
		t.Errorf("equals mismatch: got %v", metadataFormat)
	}
}
