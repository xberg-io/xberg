...
// alef:hash:edd07e582928791549f4d200f761482783d4b0d8b06a5fea930f772ee8d7eb2b
...
func Test_ConfigElementTypes(t *testing.T) {
	...
	{
		found := false
		if strings.Contains(string(result.MimeType), `application/vnd.openxmlformats-officedocument.wordprocessingml.document`) {
			found = true
		}
		if !found {
			t.Errorf("expected to contain at least one of the specified values")
...
func Test_ConfigPages(t *testing.T) {
	...
	{
		found := false
		if strings.Contains(string(result.Content), `PAGE`) {
			found = true
		}
		if !found {
			t.Errorf("expected to contain at least one of the specified values")
...
func Test_ConfigSecurityLimits(t *testing.T) {
	...
	{
		found := false
		if strings.Contains(string(result.MimeType), `application/zip`) {
			found = true
		}
		if strings.Contains(string(result.MimeType), `application/x-zip-compressed`) {
			found = true
		}
		if !found {
			t.Errorf("expected to contain at least one of the specified values")
