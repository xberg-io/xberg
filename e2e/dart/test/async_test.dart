...
void main() {
  ...
  test('extract_bytes empty MIME async', () async {
    await expectLater(() async {
      final _config = await createExtractionConfigFromJson(json: '{}');
      return XbergBridge.extractBytes(File('text/plain.txt').readAsBytesSync(), '', config: _config);
    }(), throwsA(anything));
  });
  ...
  test('extract_bytes unsupported MIME async', () async {
    await expectLater(() async {
      final _config = await createExtractionConfigFromJson(json: '{}');
      return XbergBridge.extractBytes(File('text/plain.txt').readAsBytesSync(), 'application/x-nonexistent', config: _config);
    }(), throwsA(anything));
  });
