...
void main() {
  ...
  test('OCR: PNG image extraction with OCR enabled. In WASM this exercises the Uint8Array bridge parameter and Promise await in the generated OcrBackend bridge.', () async {
    ...
    final result = await XbergBridge.extractBytes(File('images/test_hello_world.png').readAsBytesSync(), 'image/png', config: _config);
  ...
  test('Smoke test: DOCX with formatted text', () async {
    final _config = await createExtractionConfigFromJson(json: '{}');
    final result = await XbergBridge.extractBytes(File('docx/fake.docx').readAsBytesSync(), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', config: _config);
  ...
  test('Smoke test: HTML table extraction', () async {
    final _config = await createExtractionConfigFromJson(json: '{}');
    final result = await XbergBridge.extractBytes(File('html/simple_table.html').readAsBytesSync(), 'text/html', config: _config);
  ...
  test('Smoke test: PNG image (without OCR, metadata only)', () async {
    ...
    final result = await XbergBridge.extractBytes(File('images/sample.png').readAsBytesSync(), 'image/png', config: _config);
    expect(result.mimeType.toString().trim(), equals('image/png'.toString().trim()));
  });

  test('Smoke test: JSON file extraction', () async {
    final _config = await createExtractionConfigFromJson(json: '{}');
    final result = await XbergBridge.extractBytes(File('json/simple.json').readAsBytesSync(), 'application/json', config: _config);
  ...
  test('Smoke test: PDF with simple text extraction', () async {
    final _config = await createExtractionConfigFromJson(json: '{}');
    final result = await XbergBridge.extractBytes(File('pdf/fake_memo.pdf').readAsBytesSync(), 'application/pdf', config: _config);
  ...
  test('Smoke test: Plain text file', () async {
    final _config = await createExtractionConfigFromJson(json: '{}');
    final result = await XbergBridge.extractBytes(File('text/report.txt').readAsBytesSync(), 'text/plain', config: _config);
  ...
  test('Smoke test: XLSX with basic spreadsheet data including tables', () async {
    final _config = await createExtractionConfigFromJson(json: '{}');
    final result = await XbergBridge.extractBytes(File('xlsx/stanley_cups.xlsx').readAsBytesSync(), 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', config: _config);
