...
void main() {
  ...
  test('Tests document structure with DOCX heading-driven nesting', () async {
    ...
    final result = await XbergBridge.extractBytesSync(File('docx/fake.docx').readAsBytesSync(), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', config: _config);
  ...
  test('Tests element-based result format with element type assertions on DOCX', () async {
    ...
    final result = await XbergBridge.extractBytesSync(File('docx/unit_test_headers.docx').readAsBytesSync(), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', config: _config);
  ...
  test('Tests that extraction_timeout_secs config field is accepted and does not affect fast extractions', () async {
    ...
    final result = await XbergBridge.extractBytesSync(File('pdf/fake_memo.pdf').readAsBytesSync(), 'application/pdf', config: _config);
    expect(result.mimeType.toString().trim(), equals('application/pdf'.toString().trim()));
    expect(result.content.length, greaterThanOrEqualTo(10));
  });
  ...
  test('Tests archive extraction with custom security limits', () async {
    ...
    final result = await XbergBridge.extractBytesSync(File('archives/documents.zip').readAsBytesSync(), 'application/zip', config: _config);
  ...
  test('Tests tree-sitter configuration round-trip', () async {
    ...
    final result = await XbergBridge.extractFileSync('code/hello.py', 'text/x-source-code', config: _config);
