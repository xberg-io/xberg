...
void main() {
  ...
  test('extract_batch: happy path with mixed inputs', () async {
    final inputs = await Future.wait((jsonDecode(r'[{"kind":"bytes","bytes":[72,101,108,108,111,44,32,119,111,114,108,100,33],"mime_type":"text/plain"},{"kind":"bytes","bytes":[60,104,116,109,108,62,60,98,111,100,121,62,84,101,115,116,60,47,98,111,100,121,62,60,47,104,116,109,108,62],"mime_type":"text/html"}]') as List<dynamic>).cast<Map<String, dynamic>>().map((m) => createExtractInputFromJson(json: jsonEncode(m))));
  ...
  test('extract_batch with invalid bytes MIME type', () async {
    final inputs = await Future.wait((jsonDecode(r'[{"kind":"bytes","bytes":[72,101,108,108,111],"mime_type":"application/x-nonexistent"}]') as List<dynamic>).cast<Map<String, dynamic>>().map((m) => createExtractInputFromJson(json: jsonEncode(m))));
    final result = await XbergBridge.extractBatch(inputs);
  });

  test('extract_batch: handles unsupported MIME gracefully', () async {
    final inputs = await Future.wait((jsonDecode(r'[{"kind":"bytes","bytes":[80,68,70,32,112,108,97,99,101,104,111,108,100,101,114],"mime_type":"application/x-unknown"}]') as List<dynamic>).cast<Map<String, dynamic>>().map((m) => createExtractInputFromJson(json: jsonEncode(m))));
    final result = await XbergBridge.extractBatch(inputs);
  });

  test('extract_batch with unsupported bytes MIME type', () async {
    final inputs = await Future.wait((jsonDecode(r'[{"kind":"bytes","bytes":[100,97,116,97],"mime_type":"application/x-unknown"}]') as List<dynamic>).cast<Map<String, dynamic>>().map((m) => createExtractInputFromJson(json: jsonEncode(m))));
    final result = await XbergBridge.extractBatch(inputs);
  });
