...
# alef:hash:aeae34502cdd622411e46a94d7d9fe42705cac3d2464b4797b3364aa40ba3f16
...
async def test_url_batch_mixed_inputs() -> None:
    ...
    assert "Batch URL document content" in result.results[0].content
...
async def test_url_gzip_encoded_document() -> None:
    ...
    result = await extract(input, config)
    assert "Remote document hello" in result.results[0].content
...
async def test_url_html_page_extract() -> None:
    ...
    result = await extract(input, config)
    assert "Xberg URL Page" in result.results[0].content
