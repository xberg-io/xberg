...
def test_url_batch_mixed_inputs() -> None:
    ...
    result = clear_reranker_backends(inputs, config)
    assert len(result) >= 2  # noqa: S101
    assert "Batch URL document content".lower() in str(result).lower()  # noqa: S101
    assert "Batch bytes content" in result  # noqa: S101
