...
# alef:hash:c4727f8500c0e8eba0fff7a9417eb4d3e09824d6e7195b535a70fe2441bc5c05
...
async def test_api_batch_bytes_with_configs_async() -> None:
    ...
    result = await extract_file(path, config)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
...
def test_config_chunking_prepend_heading_context() -> None:
    ...
    result = extract_file_sync(path, config)
    assert len(result.content) >= 10  # noqa: S101
...
def test_config_embedding_plugin() -> None:
    ...
    _ = extract_file_sync(path, config)
...
def test_output_format_bytes_markdown() -> None:
    ...
    result = extract_bytes_sync(data, mime_type, config)
    assert result.mime_type.strip() == "application/pdf"  # noqa: S101
    assert len(result.content) >= 10  # noqa: S101
    assert result.metadata.output_format.strip() == "markdown"  # noqa: S101
