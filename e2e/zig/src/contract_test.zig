...
// alef:hash:cee4b76025652a16ee94de51b25091597181f01afe4592a7f98374ec0e2b17a1
...
test "api_batch_bytes_async" {
    ...
    const result = try kreuzberg.extract_file("pdf/fake_memo.pdf", null, "{}");
    try testing.expectEqual("application/pdf", result.mime_type);
    try testing.expect(result.content.len >= 10);
    try testing.expect(
        std.mem.indexOf(u8, result.content, "May 5, 2023") != null or
        std.mem.indexOf(u8, result.content, "Mallori") != null
    );
}
...
test "api_batch_bytes_with_configs_async" {
    ...
    const result = try kreuzberg.extract_file("pdf/fake_memo.pdf", null, "{\"output_format\":\"markdown\"}");
    try testing.expectEqual("application/pdf", result.mime_type);
    try testing.expect(result.content.len >= 10);
...
test "config_chunking_prepend_heading_context" {
    ...
    try testing.expect(result.object.get("chunks_have_content").?.bool);
    try testing.expect(result.object.get("chunks_have_heading_context").?.bool);
    try testing.expect(result.object.get("first_chunk_starts_with_heading").?.bool);
}
...
test "config_element_types" {
    ...
    try testing.expect(result.object.get("elements").?.array.items.len >= 1);
}
...
test "config_keywords" {
    ...
    try testing.expectEqualStrings("application/pdf", std.mem.trim(u8, result.object.get("mime_type").?.string, " \n\r\t"));
    try testing.expect(result.object.get("content").?.string.len >= 10);
    try testing.expect(result.object.get("keywords").? != .null);
    try testing.expect(result.object.get("keywords").?.array.items.len >= 1);
}
...
test "config_quality_enabled" {
    ...
    try testing.expect(result.object.get("quality_score").?.float >= 0.0);
    try testing.expect(result.object.get("quality_score").?.float <= 1.0);
}
