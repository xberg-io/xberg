...
test "format_docx_standalone" {
    ...
    const _result_json = try xberg.extract_bytes_sync(content_bytes, "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "{}");
...
test "format_hwpx_standalone" {
    ...
    defer std.heap.c_allocator.free(content_bytes);
    const _result_json = try xberg.extract_bytes_sync(content_bytes, "application/haansofthwpx", "{}");
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    defer _parsed.deinit();
    const result = &_parsed.value;
...
test "format_pdf_text" {
    ...
    defer std.heap.c_allocator.free(content_bytes);
    const _result_json = try xberg.extract_bytes_sync(content_bytes, "application/pdf", "{}");
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    defer _parsed.deinit();
    const result = &_parsed.value;
...
test "format_pptx" {
    ...
    suppress_abort();
    const _result_json = try xberg.extract_file_sync("pptx/simple.pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation", "{}");
    defer std.heap.c_allocator.free(_result_json);
}
