...
// alef:hash:fe77b7a2adbe16c35786d909e45861cb88b87adca4d2b5bc276245636787cba2
...
test "smoke_image_png" {
    ...
    try testing.expectEqualStrings("image/png", std.mem.trim(u8, result.object.get("mime_type").?.string, " \n\r\t"));
    {
        const _fmt_obj = result.object.get("metadata").?.object.get("format").?.object;
        const _fmt_type = _fmt_obj.get("format_type").?.string;
        const _fmt_display: []const u8 = if (std.mem.eql(u8, _fmt_type, "image")) _fmt_obj.get("format").?.string else _fmt_type;
        try testing.expectEqualStrings("PNG", std.mem.trim(u8, _fmt_display, " \n\r\t"));
    }
}
