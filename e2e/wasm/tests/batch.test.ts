...
describe("batch", () => {
  it("extract_batch_bytes_happy: extract_batch: happy path with mixed inputs", async () => {
    const result = await extractBatch(
      [
        { kind: "bytes", bytes: [72, 101, 108, 108, 111, 44, 32, 119, 111, 114, 108, 100, 33], mimeType: "text/plain" },
        {
          kind: "bytes",
          bytes: [
  ...
  it("extract_batch_bytes_invalid_mime: extract_batch with invalid bytes MIME type", async () => {
    await extractBatch(
      [{ kind: "bytes", bytes: [72, 101, 108, 108, 111], mimeType: "application/x-nonexistent" }],
      undefined,
    );
  }, 30000);
  ...
  it("extract_batch_bytes_unsupported_mime: extract_batch with unsupported bytes MIME type", async () => {
    await extractBatch([{ kind: "bytes", bytes: [100, 97, 116, 97], mimeType: "application/x-unknown" }], undefined);
  }, 30000);
