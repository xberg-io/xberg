...
// alef:hash:fe9b29a4503e57e2379fa266c0820001d5a93217d6a4194d966f1e5fd84b0c91
...
async fn test_smoke_xlsx_basic() {
    ...
    assert!(
        result.metadata.format.as_ref().unwrap().excel.sheet_count >= 2,
        "expected >= 2"
    );
    assert!(
        format!("{:?}", result.metadata.format.as_ref().unwrap().excel.sheet_names).contains(r#"Stanley Cups"#),
        "expected to contain: {}",
        r#"Stanley Cups"#
    );
}
