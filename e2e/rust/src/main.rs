...
async fn handle_request(State(routes): State<RouteTable>, req: Request<Body>) -> Response {
    ...
    // Fallback: serve a file from the test-documents corpus. Language suites that
    // ingest documents over HTTP (e.g. Node) request `/<rel>/<file>` against the
    // shared server; these paths are not fixture routes, so resolve them against the
    // test-documents directory and serve the bytes with an extension-derived
    // content-type. Suites that pass local file paths (Python, Go, Rust) never hit this.
...
async fn main() {
    ...
    let docs_dir = std::env::var_os("XBERG_TEST_DOCUMENTS_DIR")
        .map(PathBuf::from)
