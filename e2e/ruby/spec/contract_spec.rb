...
RSpec.describe 'contract' do
  it 'api_batch_bytes_async: Tests async batch bytes extraction API (batch_extract_bytes)' do
    result = Xberg.extract_file_async('pdf/fake_memo.pdf')
  ...
  it 'api_batch_bytes_with_configs_async: Tests async batch bytes extraction with per-file configs (batch_extract_bytes with file_configs parameter)' do
    result = Xberg.extract_file_async('pdf/fake_memo.pdf', nil, Xberg::ExtractionConfig.new(output_format: 'markdown'))
    expect(result.mime_type.to_s.strip).to eq('application/pdf'.strip)
    expect(result.content.length).to be >= 10
  ...
  it 'config_document_structure_with_headings: Tests document structure with DOCX heading-driven nesting' do
    result = Xberg.extract_file_sync('docx/fake.docx', nil, Xberg::ExtractionConfig.new(include_document_structure: true))
  ...
  it 'config_element_types: Tests element-based result format with element type assertions on DOCX' do
    result = Xberg.extract_file_sync('docx/unit_test_headers.docx', nil, Xberg::ExtractionConfig.new(result_format: 'element_based'))
  ...
  it 'config_extraction_timeout: Tests that extraction_timeout_secs config field is accepted and does not affect fast extractions' do
    result = Xberg.extract_file_sync('pdf/fake_memo.pdf', nil, Xberg::ExtractionConfig.new(extraction_timeout_secs: 300))
    expect(result.mime_type.to_s.strip).to eq('application/pdf'.strip)
    expect(result.content.length).to be >= 10
  ...
  it 'config_keywords: Tests keyword extraction via YAKE algorithm' do
    result = Xberg.extract_file_sync('pdf/fake_memo.pdf', nil, Xberg::ExtractionConfig.new(keywords: { 'algorithm' => 'yake', 'max_keywords' => 10 }))
    expect(result.mime_type.to_s.strip).to eq('application/pdf'.strip)
    expect(result.content.length).to be >= 10
  ...
  it 'config_pages: Tests page extraction and page marker configuration' do
    result = Xberg.extract_file_sync('pdf/fake_memo.pdf', nil, Xberg::ExtractionConfig.new(pages: { 'extract_pages' => true, 'insert_page_markers' => true }))
    expect(result.mime_type.to_s.strip).to eq('application/pdf'.strip)
    expect(result.content.length).to be >= 10
  ...
  it 'config_quality_enabled: Tests quality scoring produces a score value in [0.0, 1.0]' do
    result = Xberg.extract_file_sync('pdf/fake_memo.pdf', nil, Xberg::ExtractionConfig.new(enable_quality_processing: true))
    expect(result.mime_type.to_s.strip).to eq('application/pdf'.strip)
    expect(result.content.length).to be >= 10
  ...
  it 'config_security_limits: Tests archive extraction with custom security limits' do
    result = Xberg.extract_file_sync('archives/documents.zip', nil, Xberg::ExtractionConfig.new(security_limits: { 'max_archive_size' => 104857600, 'max_compression_ratio' => 50, 'max_files_in_archive' => 100 }))
  ...
  it 'config_tree_sitter: Tests tree-sitter configuration round-trip' do
    result = Xberg.extract_file_sync('code/hello.py', nil, Xberg::ExtractionConfig.new(tree_sitter: { 'groups' => ['web'], 'languages' => ['python', 'rust'], 'process' => { 'comments' => false, 'diagnostics' => false, 'docstrings' => false, 'exports' => true, 'imports' => true, 'structure' => true, 'symbols' => false } }))
  ...
  it 'output_format_bytes_markdown: Tests markdown output format via bytes extraction API' do
    ...
    result = Xberg.extract_bytes_sync(content, 'application/pdf', Xberg::ExtractionConfig.new(output_format: 'markdown'))
    expect(result.mime_type.to_s.strip).to eq('application/pdf'.strip)
    expect(result.content.length).to be >= 10
    expect(result.metadata.output_format.to_s.strip).to eq('markdown'.strip)
  ...
  it 'output_format_markdown: Tests Markdown output format' do
    result = Xberg.extract_file_sync('pdf/fake_memo.pdf', nil, Xberg::ExtractionConfig.new(output_format: 'markdown'))
    expect(result.mime_type.to_s.strip).to eq('application/pdf'.strip)
    expect(result.content.length).to be >= 10
    expect(result.metadata.output_format.to_s.strip).to eq('markdown'.strip)
