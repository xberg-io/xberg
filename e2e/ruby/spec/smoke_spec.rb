...
RSpec.describe 'smoke' do
  ...
  it 'ocr_image_png: OCR: PNG image extraction with OCR enabled. In WASM this exercises the Uint8Array bridge parameter and Promise await in the generated OcrBackend bridge.' do
    ...
    result = Xberg.extract_bytes_async(content, 'image/png', Xberg::ExtractionConfig.new())
  ...
  it 'smoke_docx_basic: Smoke test: DOCX with formatted text' do
    result = Xberg.extract_file_async('docx/fake.docx', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', Xberg::ExtractionConfig.new())
  ...
  it 'smoke_html_basic: Smoke test: HTML table extraction' do
    result = Xberg.extract_file_async('html/simple_table.html', 'text/html', Xberg::ExtractionConfig.new())
  ...
  it 'smoke_image_png: Smoke test: PNG image (without OCR, metadata only)' do
    result = Xberg.extract_file_async('images/sample.png', nil, Xberg::ExtractionConfig.new(disable_ocr: true))
    expect(result.mime_type.to_s.strip).to eq('image/png'.strip)

  end

  it 'smoke_json_basic: Smoke test: JSON file extraction' do
    result = Xberg.extract_file_async('json/simple.json', 'application/json', Xberg::ExtractionConfig.new())
  ...
  it 'smoke_pdf_basic: Smoke test: PDF with simple text extraction' do
    result = Xberg.extract_file_async('pdf/fake_memo.pdf', 'application/pdf', Xberg::ExtractionConfig.new())
  ...
  it 'smoke_txt_basic: Smoke test: Plain text file' do
    result = Xberg.extract_file_async('text/report.txt', 'text/plain', Xberg::ExtractionConfig.new())
  ...
  it 'smoke_xlsx_basic: Smoke test: XLSX with basic spreadsheet data including tables' do
    result = Xberg.extract_file_async('xlsx/stanley_cups.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', Xberg::ExtractionConfig.new())
