...
# alef:hash:6ed1626de4f77096cc5b13725ad79cc85d3c03a420c718c07d3392a5f042493a
...
test_that("smoke_docx_basic: Smoke test: DOCX with formatted text", {
  result <- extract_file(path = "docx/fake.docx", mime_type = "application/vnd.openxmlformats-officedocument.wordprocessingml.document", config = list())
...
test_that("smoke_html_basic: Smoke test: HTML table extraction", {
  result <- extract_file(path = "html/simple_table.html", mime_type = "text/html", config = list())
...
test_that("smoke_json_basic: Smoke test: JSON file extraction", {
  result <- extract_file(path = "json/simple.json", mime_type = "application/json", config = list())
...
test_that("smoke_pdf_basic: Smoke test: PDF with simple text extraction", {
  result <- extract_file(path = "pdf/fake_memo.pdf", mime_type = "application/pdf", config = list())
...
test_that("smoke_txt_basic: Smoke test: Plain text file", {
  result <- extract_file(path = "text/report.txt", mime_type = "text/plain", config = list())
...
test_that("smoke_xlsx_basic: Smoke test: XLSX with basic spreadsheet data including tables", {
  result <- extract_file(path = "xlsx/stanley_cups.xlsx", mime_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", config = list())
