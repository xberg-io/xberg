...
defmodule E2e.BatchTest do
  ...
  describe "extract_batch_bytes_happy" do
    test "extract_batch_bytes_happy" do
      {:ok, result} = Xberg.extract_batch(inputs: [%{"kind" => "bytes", "bytes" => [72, 101, 108, 108, 111, 44, 32, 119, 111, 114, 108, 100, 33], "mime_type" => "text/plain"}, %{"kind" => "bytes", "bytes" => [60, 104, 116, 109, 108, 62, 60, 98, 111, 100, 121, 62, 84, 101, 115, 116, 60, 47, 98, 111, 100, 121, 62, 60, 47, 104, 116, 109, 108, 62], "mime_type" => "text/html"}])
  ...
  describe "extract_batch_bytes_invalid_mime" do
    test "extract_batch_bytes_invalid_mime" do
      {:ok, result} = Xberg.extract_batch(inputs: [%{"kind" => "bytes", "bytes" => [72, 101, 108, 108, 111], "mime_type" => "application/x-nonexistent"}])
    end
  end

  describe "extract_batch_bytes_mixed_format" do
    test "extract_batch_bytes_mixed_format" do
      {:ok, result} = Xberg.extract_batch(inputs: [%{"kind" => "bytes", "bytes" => [80, 68, 70, 32, 112, 108, 97, 99, 101, 104, 111, 108, 100, 101, 114], "mime_type" => "application/x-unknown"}])
    end
  end

  describe "extract_batch_bytes_unsupported_mime" do
    test "extract_batch_bytes_unsupported_mime" do
      {:ok, result} = Xberg.extract_batch(inputs: [%{"kind" => "bytes", "bytes" => [100, 97, 116, 97], "mime_type" => "application/x-unknown"}])
    end
  end
