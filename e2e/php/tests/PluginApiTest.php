<?php

// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:9b6c82fdc18c3106bdec2c926b44f0262b7dcc51189a52c181c04410d16a2fd9
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code

declare(strict_types=1);

namespace Xberg\E2e;

use PHPUnit\Framework\TestCase;
use Xberg\ExtractionConfig;
use Xberg\Xberg;

/** E2e tests for category: plugin_api. */
final class PluginApiTest extends TestCase
{
    /** register_embedding_backend: trait bridge */
    public function test_register_embedding_backend_trait_bridge(): void
    {
        $stub = new class implements \Xberg\EmbeddingBackend {
            public function name(): string
            {
                return 'test-embedding-backend';
            }

            public function dimensions(): int
            {
                return 1;
            }

            public function embed($texts): mixed
            {
                return [];
            }

            public function version(): string
            {
                return '';
            }

            public function initialize(): mixed
            {
                return null;
            }

            public function shutdown(): mixed
            {
                return null;
            }

            public function description(): string
            {
                return '';
            }

            public function author(): string
            {
                return '';
            }
        };
        $this->expectNotToPerformAssertions();
        $result = Xberg::registerEmbeddingBackend($stub);

        Xberg::unregisterEmbeddingBackend('test-embedding-backend');
    }

    /** register_ocr_backend: trait bridge */
    public function test_register_ocr_backend_trait_bridge(): void
    {
        $stub = new class implements \Xberg\OcrBackend {
            public function name(): string
            {
                return 'test-backend';
            }

            public function process_image($image_bytes, $config): \Xberg\ExtractedDocument
            {
                return '{}';
            }

            public function process_image_file($path, $config): \Xberg\ExtractedDocument
            {
                return '{}';
            }

            public function supports_language($lang): bool
            {
                return false;
            }

            public function backend_type(): mixed
            {
                return '{}';
            }

            public function supported_languages(): mixed
            {
                return [];
            }

            public function supports_table_detection(): bool
            {
                return false;
            }

            public function supports_document_processing(): bool
            {
                return false;
            }

            public function emits_structured_markdown(): bool
            {
                return false;
            }

            public function process_document($_path, $_config): \Xberg\ExtractedDocument
            {
                return '{}';
            }

            public function version(): string
            {
                return '';
            }

            public function initialize(): mixed
            {
                return null;
            }

            public function shutdown(): mixed
            {
                return null;
            }

            public function description(): string
            {
                return '';
            }

            public function author(): string
            {
                return '';
            }
        };
        $this->expectNotToPerformAssertions();
        $result = Xberg::registerOcrBackend($stub);

        Xberg::unregisterOcrBackend('test-backend');
    }

    /** register_post_processor: trait bridge */
    public function test_register_post_processor_trait_bridge(): void
    {
        $stub = new class implements \Xberg\PostProcessor {
            public function name(): string
            {
                return 'test-processor';
            }

            public function process($result, $config): mixed
            {
                return null;
            }

            public function processing_stage(): mixed
            {
                return '{}';
            }

            public function should_process($_result, $_config): bool
            {
                return false;
            }

            public function estimated_duration_ms($_result): int
            {
                return 1;
            }

            public function priority(): int
            {
                return 1;
            }

            public function version(): string
            {
                return '';
            }

            public function initialize(): mixed
            {
                return null;
            }

            public function shutdown(): mixed
            {
                return null;
            }

            public function description(): string
            {
                return '';
            }

            public function author(): string
            {
                return '';
            }
        };
        $this->expectNotToPerformAssertions();
        $result = Xberg::registerPostProcessor($stub);

        Xberg::unregisterPostProcessor('test-processor');
    }

    /** register_reranker_backend: trait bridge */
    public function test_register_reranker_backend_trait_bridge(): void
    {
        $stub = new class implements \Xberg\RerankerBackend {
            public function name(): string
            {
                return 'test-reranker-backend';
            }

            public function rerank($query, $documents): mixed
            {
                return [];
            }

            public function version(): string
            {
                return '';
            }

            public function initialize(): mixed
            {
                return null;
            }

            public function shutdown(): mixed
            {
                return null;
            }

            public function description(): string
            {
                return '';
            }

            public function author(): string
            {
                return '';
            }
        };
        $this->expectNotToPerformAssertions();
        $result = Xberg::registerRerankerBackend($stub);

        Xberg::unregisterRerankerBackend('test-reranker-backend');
    }

    /** register_validator: trait bridge */
    public function test_register_validator_trait_bridge(): void
    {
        $stub = new class implements \Xberg\Validator {
            public function name(): string
            {
                return 'test-validator';
            }

            public function validate($result, $config): mixed
            {
                return null;
            }

            public function should_validate($_result, $_config): bool
            {
                return false;
            }

            public function priority(): int
            {
                return 1;
            }

            public function version(): string
            {
                return '';
            }

            public function initialize(): mixed
            {
                return null;
            }

            public function shutdown(): mixed
            {
                return null;
            }

            public function description(): string
            {
                return '';
            }

            public function author(): string
            {
                return '';
            }
        };
        $this->expectNotToPerformAssertions();
        $result = Xberg::registerValidator($stub);

        Xberg::unregisterValidator('test-validator');
    }

    /** unregister_embedding_backend */
    public function test_unregister_embedding_backend_after_register(): void
    {
        $this->expectNotToPerformAssertions();
        $result = Xberg::unregisterEmbeddingBackend('test-embedding-backend');
    }

    /** unregister_post_processor */
    public function test_unregister_post_processor_after_register(): void
    {
        $this->expectNotToPerformAssertions();
        $result = Xberg::unregisterPostProcessor('test-processor');
    }

    /** unregister_reranker_backend */
    public function test_unregister_reranker_backend(): void
    {
        $this->expectNotToPerformAssertions();
        $result = Xberg::unregisterRerankerBackend('test-reranker-backend');
    }

    /** unregister_validator */
    public function test_unregister_validator_after_register(): void
    {
        $this->expectNotToPerformAssertions();
        $result = Xberg::unregisterValidator('test-validator');
    }
}
