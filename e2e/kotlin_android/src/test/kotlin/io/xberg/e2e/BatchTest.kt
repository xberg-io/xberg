...
class BatchTest {
    ...
    fun testExtractBatchBytesHappy() = runBlocking {
        ...
        val result = Xberg.extractBatch(listOf(MAPPER.readValue("{\"kind\":\"bytes\",\"bytes\":[72,101,108,108,111,44,32,119,111,114,108,100,33],\"mime_type\":\"text/plain\"}", ExtractInput::class.java), MAPPER.readValue("{\"kind\":\"bytes\",\"bytes\":[60,104,116,109,108,62,60,98,111,100,121,62,84,101,115,116,60,47,98,111,100,121,62,60,47,104,116,109,108,62],\"mime_type\":\"text/html\"}", ExtractInput::class.java)), ExtractionConfig())
    ...
    fun testExtractBatchBytesInvalidMime() = runBlocking {
        ...
        val result = Xberg.extractBatch(listOf(MAPPER.readValue("{\"kind\":\"bytes\",\"bytes\":[72,101,108,108,111],\"mime_type\":\"application/x-nonexistent\"}", ExtractInput::class.java)), ExtractionConfig())
    }

    @Test
    fun testExtractBatchBytesMixedFormat() = runBlocking {
        // extract_batch: handles unsupported MIME gracefully
        val result = Xberg.extractBatch(listOf(MAPPER.readValue("{\"kind\":\"bytes\",\"bytes\":[80,68,70,32,112,108,97,99,101,104,111,108,100,101,114],\"mime_type\":\"application/x-unknown\"}", ExtractInput::class.java)), ExtractionConfig())
    }

    @Test
    fun testExtractBatchBytesUnsupportedMime() = runBlocking {
        // extract_batch with unsupported bytes MIME type
        val result = Xberg.extractBatch(listOf(MAPPER.readValue("{\"kind\":\"bytes\",\"bytes\":[100,97,116,97],\"mime_type\":\"application/x-unknown\"}", ExtractInput::class.java)), ExtractionConfig())
    }
