...
describe("office fixtures", () => {
  ...
  it("office_hwp_styled", () => {
    ...
    if (!existsSync(documentPath)) {
      ...
      console.warn("Notes: HWP styled doc yields no extractable plain text with current parser. Extraction returns empty content on ARM Linux.");
      return;
    }
  ...
  it("office_keynote_basic", () => {
    const documentPath = resolveDocument("iwork/test.key");
    if (!existsSync(documentPath)) {
      console.warn("Skipping office_keynote_basic: missing document at", documentPath);
      return;
    }
    const config = buildConfig(undefined);
    let result: ExtractionResult | null = null;
  ...
  it("office_numbers_basic", () => {
    const documentPath = resolveDocument("iwork/test.numbers");
    if (!existsSync(documentPath)) {
      console.warn("Skipping office_numbers_basic: missing document at", documentPath);
      return;
    }
    const config = buildConfig(undefined);
    let result: ExtractionResult | null = null;
  ...
  it("office_pages_basic", () => {
    const documentPath = resolveDocument("iwork/test.pages");
    if (!existsSync(documentPath)) {
      console.warn("Skipping office_pages_basic: missing document at", documentPath);
      return;
    }
    const config = buildConfig(undefined);
    let result: ExtractionResult | null = null;
