// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:9b6c82fdc18c3106bdec2c926b44f0262b7dcc51189a52c181c04410d16a2fd9
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code

package io.xberg.e2e;

import org.junit.jupiter.api.Test;
import static org.junit.jupiter.api.Assertions.*;
import io.xberg.Xberg;
import io.xberg.ExtractInput;
import io.xberg.ExtractionConfig;
import java.util.Optional;
import io.xberg.JsonUtil;
import org.junit.jupiter.api.BeforeAll;
/** E2e tests for category: batch. */
public class BatchTest {
    @BeforeAll
    static void initEnv() {        if (System.getProperty("CRAWLBERG_ALLOW_PRIVATE_NETWORK") == null) {
        System.setProperty("CRAWLBERG_ALLOW_PRIVATE_NETWORK", "true");
    }    }

    @Test
    void testExtractBatchBytesHappy() throws Exception {
        // extract_batch: happy path with mixed inputs
        var result = Xberg.extractBatch(java.util.Arrays.asList(JsonUtil.fromJson("{\"kind\":\"bytes\",\"bytes\":[72,101,108,108,111,44,32,119,111,114,108,100,33],\"mime_type\":\"text/plain\"}", ExtractInput.class), JsonUtil.fromJson("{\"kind\":\"bytes\",\"bytes\":[60,104,116,109,108,62,60,98,111,100,121,62,84,101,115,116,60,47,98,111,100,121,62,60,47,104,116,109,108,62],\"mime_type\":\"text/html\"}", ExtractInput.class)), ExtractionConfig.builder().build());
        assertTrue(result.results().size() >= 1, "expected at least 1 elements");

    }


    @Test
    void testExtractBatchBytesInvalidMime() throws Exception {
        // extract_batch with invalid bytes MIME type
        var result = Xberg.extractBatch(java.util.Arrays.asList(JsonUtil.fromJson("{\"kind\":\"bytes\",\"bytes\":[72,101,108,108,111],\"mime_type\":\"application/x-nonexistent\"}", ExtractInput.class)), ExtractionConfig.builder().build());


    }


    @Test
    void testExtractBatchBytesMixedFormat() throws Exception {
        // extract_batch: handles unsupported MIME gracefully
        var result = Xberg.extractBatch(java.util.Arrays.asList(JsonUtil.fromJson("{\"kind\":\"bytes\",\"bytes\":[80,68,70,32,112,108,97,99,101,104,111,108,100,101,114],\"mime_type\":\"application/x-unknown\"}", ExtractInput.class)), ExtractionConfig.builder().build());


    }


    @Test
    void testExtractBatchBytesUnsupportedMime() throws Exception {
        // extract_batch with unsupported bytes MIME type
        var result = Xberg.extractBatch(java.util.Arrays.asList(JsonUtil.fromJson("{\"kind\":\"bytes\",\"bytes\":[100,97,116,97],\"mime_type\":\"application/x-unknown\"}", ExtractInput.class)), ExtractionConfig.builder().build());


    }


    @Test
    void testExtractBatchEmptyInputs() throws Exception {
        // extract_batch: empty batch
        var result = Xberg.extractBatch(java.util.List.of(), ExtractionConfig.builder().build());
        assertEquals(0, result.results().size(), "expected exactly 0 elements");

    }


    @Test
    void testExtractBatchUriAllMissing() throws Exception {
        // extract_batch with missing URI inputs
        var result = Xberg.extractBatch(java.util.Arrays.asList(JsonUtil.fromJson("{\"kind\":\"uri\",\"uri\":\"/nonexistent/a.pdf\"}", ExtractInput.class), JsonUtil.fromJson("{\"kind\":\"uri\",\"uri\":\"/nonexistent/b.txt\"}", ExtractInput.class)), ExtractionConfig.builder().build());


    }


    @Test
    void testExtractBatchUriBasic() throws Exception {
        // extract_batch over URI inputs
        var result = Xberg.extractBatch(java.util.Arrays.asList(JsonUtil.fromJson("{\"kind\":\"uri\",\"uri\":\"pdf/fake_memo.pdf\"}", ExtractInput.class), JsonUtil.fromJson("{\"kind\":\"uri\",\"uri\":\"text/fake_text.txt\"}", ExtractInput.class)), ExtractionConfig.builder().build());


    }


    @Test
    void testExtractBatchUriNotFound() throws Exception {
        // extract_batch with missing URI input
        var result = Xberg.extractBatch(java.util.Arrays.asList(JsonUtil.fromJson("{\"kind\":\"uri\",\"uri\":\"/nonexistent/a.pdf\"}", ExtractInput.class)), ExtractionConfig.builder().build());


    }


    @Test
    void testExtractBatchUriPartialFailure() throws Exception {
        // extract_batch with mixed valid and missing URI inputs
        var result = Xberg.extractBatch(java.util.Arrays.asList(JsonUtil.fromJson("{\"kind\":\"uri\",\"uri\":\"text/plain.txt\"}", ExtractInput.class), JsonUtil.fromJson("{\"kind\":\"uri\",\"uri\":\"/nonexistent/missing.pdf\"}", ExtractInput.class)), ExtractionConfig.builder().build());


    }

}
