...
// alef:hash:e397f1b1f1cf158ea2c8ba8be9fc885fc79b29eb28c3acd3f9967548a4241ac0
...
assertTrue(result.content().length() >= 10, "expected length >= 10");        assertTrue(chunks.size() >= 2, "expected >= 2 chunks");
        assertTrue(result.chunks().orElse(java.util.List.of()).stream().allMatch(c -> c.content() != null && !c.content().isBlank()), "expected true");        assertTrue(result.chunks().orElse(java.util.List.of()).stream().allMatch(c -> c.metadata().headingContext().isPresent()), "expected true");        assertTrue(result.chunks().orElse(java.util.List.of()).stream().findFirst().map(c -> c.metadata().headingContext().isPresent()).orElse(false), "expected true");
    }
