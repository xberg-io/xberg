...
// alef:hash:6e68d92d66bbf1b04bdf15227a74d8d135291dbd2dbe0bf1191aa510d2cda329
...
public class SmokeTests
{
    ...
    {
        // OCR: PNG image extraction with OCR enabled. In WASM this exercises the Uint8Array bridge parameter and Promise await in the generated OcrBackend bridge.        var result = await KreuzbergLib.ExtractBytes(System.Text.Encoding.UTF8.GetBytes("images/test_hello_world.png"), "image/png", new ExtractionConfig());
...
Assert.True(result.Content.ToString().Contains("Hello") || result.Content.ToString().Contains("World") || result.Content.ToString().Contains("hello") || result.Content.ToString().Contains("world"), "expected to contain at least one of the specified values");
    ...
    {
        // Smoke test: DOCX with formatted text        var result = await KreuzbergLib.ExtractFile("docx/fake.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", new ExtractionConfig());
...
Assert.True(result.Content.ToString().Contains("Lorem") || result.Content.ToString().Contains("ipsum") || result.Content.ToString().Contains("document") || result.Content.ToString().Contains("text"), "expected to contain at least one of the specified values");
    ...
    {
        // Smoke test: HTML table extraction        var result = await KreuzbergLib.ExtractFile("html/simple_table.html", "text/html", new ExtractionConfig());
...
Assert.True(result.Content.ToString().Contains("Sample Data Table") || result.Content.ToString().Contains("Laptop") || result.Content.ToString().Contains("Electronics") || result.Content.ToString().Contains("Product"), "expected to contain at least one of the specified values");
    ...
    {
        // Smoke test: JSON file extraction        var result = await KreuzbergLib.ExtractFile("json/simple.json", "application/json", new ExtractionConfig());
...
Assert.True(result.Content.Length >= 5, "expected length >= 5");
    ...
    {
        // Smoke test: PDF with simple text extraction        var result = await KreuzbergLib.ExtractFile("pdf/fake_memo.pdf", "application/pdf", new ExtractionConfig());
...
Assert.True(result.Content.ToString().Contains("May 5, 2023") || result.Content.ToString().Contains("To Whom it May Concern"), "expected to contain at least one of the specified values");
    ...
    {
        // Smoke test: Plain text file        var result = await KreuzbergLib.ExtractFile("text/report.txt", "text/plain", new ExtractionConfig());
...
Assert.True(result.Content.Length >= 5, "expected length >= 5");
    ...
    {
        // Smoke test: XLSX with basic spreadsheet data including tables        var result = await KreuzbergLib.ExtractFile("xlsx/stanley_cups.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", new ExtractionConfig());
