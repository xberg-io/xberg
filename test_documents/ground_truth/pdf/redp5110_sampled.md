...
## 1 Securing and protecting IBM DB2 data
Recent news headlines are filled with reports of data breaches and cyber-attacks impacting
global businesses of all sizes. The Identity Theft Resource Center¹ reports that almost 5000
...
- First, and most important, is the definition of a company's security policy. Without a
...
- The second fundamental in securing data assets is the use of resource security. If
...
Table 2-1 FUNCTION USAGE view

| Column name | Data type | Description |
| --- | --- | --- |
| FUNCTION_ID | VARCHAR(30) | ID of the function. |
| USER NAME | VARCHAR(10) | Name of the user profile that has a usage setting for this function. |
| USAGE | VARCHAR(7) | Usage setting:  - ALLOWED: The user profile is allowed to use the function.  - DENIED: The user profile is not allowed to use the function. |
| USER_TYPE | VARCHAR(5) | Type of user profile:  - USER: The user profile is a user.  - GROUP: The user profile is a group. |
...
| QUSRJOBI() API format 900 or System i Navigator's SQL Details for Job | X | X | X | X | X |
| Visual Explain within Run SQL scripts | X | X |  |  | X |
...
## 3 The SQL CREATE PERMISSION statement that is shown in Figure 3-1 is used to define and
...
| --- | --- |
| USER or SESSION USER | The effective user of the thread excluding adopted authority. |
| CURRENT USER | The effective user of the thread including adopted authority. When no adopted authority is present, this has the same value as USER. |
| SYSTEM USER | The authorization ID that initiated the connection. |
...
| CLIENT IPADDR | VARCHAR(128) | IP address of the current client as returned by the system |
| CLIENT PORT | INTEGER | Port used by the current client to communicate with the server |
...
| ROUTINE SPECIFIC NAME | VARCHAR(128) | Name of the currently running routine |
...
AND SESSION USER = EMPLOYEES USER ID
THEN EMPLOYEES DATE_OF_BIRTH
...
RETURN
CASE
WHEN VERIFY GROUP FOR USER ( SESSION USER, 'HR' ) = 1
THEN EMPLOYEES TAX ID

WHEN VERIFY_GROUP_FOR_USER ( SESSION_USER , 'MGR' ) = 1
AND SESSION_USER = EMPLOYEES. USER_ID
THEN EMPLOYEES TAX_ID

WHEN VERIFY GROUP FOR USER ( SESSION USER, 'MGR' ) = 1
AND SESSION USER <> EMPLOYEES USER ID
...
```
CREATE MASK BANK_SCHEMA.MASK_DRIVERS_LICENSE_ON_CUSTOMERS ON BANK SCHEMA.CUSTOMERS AS C
...
```
CREATE MASK BANK_SCHEMA.MASK_SECURITY_C
_SCHEMA.MASK_SECURITY_QUESTION_ANSWER_ON_CUSTOMERS ON BANK_SCHEMA.CUSTOMERS AS C
