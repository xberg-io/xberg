    ...
    public static BatchBytesItem FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<BatchBytesItem>(json, JsonOptions)
                ?? throw new XbergException($"Failed to parse BatchBytesItem from JSON: deserializer returned null");
        }
        ...
        {
            throw new XbergException($"Failed to parse BatchBytesItem from JSON: {e.Message}", e);
        }
    }
