...
public sealed record FileExtractionConfig
{
    ...
    public static FileExtractionConfig FromJson(string json)
    {
        try
        {
            // Filter out unknown properties (e.g., extraction_timeout_secs, security_limits which are top-level ExtractionConfig fields)
            var knownProperties = new HashSet<string>
            {
                "enable_quality_processing", "ocr", "force_ocr", "force_ocr_pages", "disable_ocr",
                "chunking", "content_filter", "images", "pdf_options", "token_reduction",
                "language_detection", "pages", "keywords", "postprocessor", "html_output",
                "result_format", "output_format", "include_document_structure", "layout",
                "transcription", "timeout_secs", "tree_sitter", "structured_extraction", "url",
                "ner", "redaction", "summarization", "translation", "page_classification",
                "captioning", "qr_codes"
            };
            var filteredJson = JsonLeniency.FilterUnknownProperties(json, knownProperties);
            return JsonSerializer.Deserialize<FileExtractionConfig>(filteredJson, JsonOptions)
