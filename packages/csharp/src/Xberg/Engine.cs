...
{    private static readonly JsonSerializerOptions JsonOptions = new()
    ...
    {
        var nativeResult = NativeMethods.EngineCacheBackend(
            Handle
        );
        if (nativeResult == IntPtr.Zero)
        {
            var ec = NativeMethods.LastErrorCode();
            var ctxPtr = NativeMethods.LastErrorContext();
            var msg = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(ctxPtr) ?? "EngineCacheBackend failed";
            throw new XbergException(ec, msg);
        }
        var returnValue = new CacheBackend(nativeResult);
        return returnValue;
    }
    ...
    /// </summary>
    public ProgressSink ProgressSink()
    {
        var nativeResult = NativeMethods.EngineProgressSink(
            Handle
        );
        if (nativeResult == IntPtr.Zero)
        {
            var ec = NativeMethods.LastErrorCode();
            var ctxPtr = NativeMethods.LastErrorContext();
            var msg = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(ctxPtr) ?? "EngineProgressSink failed";
            throw new XbergException(ec, msg);
        }
        var returnValue = new ProgressSink(nativeResult);
        return returnValue;
    }
    ...
    {
        var nativeResult = NativeMethods.EngineModelProvider(
            Handle
        );
        if (nativeResult == IntPtr.Zero)
        {
            var ec = NativeMethods.LastErrorCode();
            var ctxPtr = NativeMethods.LastErrorContext();
            var msg = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(ctxPtr) ?? "EngineModelProvider failed";
            throw new XbergException(ec, msg);
        }
        var returnValue = new ModelProvider(nativeResult);
        return returnValue;
    }
}
