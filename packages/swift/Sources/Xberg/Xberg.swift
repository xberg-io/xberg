...
// alef:hash:20be33d4322d6b8c15965241bf903a3d390c435be4d6429102d9105461e65391
// swift-format-ignore-file

import Foundation
import RustBridge
public typealias MetaSchema = RustBridge.MetaSchema
public typealias Registry = RustBridge.Registry
public typealias TokenCounter = RustBridge.TokenCounter

/// Codable representation of an arbitrary JSON value (`serde_json::Value`). Emitted only when a
/// generated DTO has a `Map<String, Json>` field -- that value position cannot decode through the
/// `String`-holds-raw-JSON-text convention used for a bare Json field elsewhere in this module,
/// because the real wire value is not always a JSON string.
...
internal extension AccelerationConfig {
    init(_ rb: RustBridge.AccelerationConfigRef) throws {
        self.provider = try { let rawValue = rb.provider().toString(); guard let value = ExecutionProviderType(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown ExecutionProviderType variant", source: rawValue) }; return value }()
...
public struct Attributes: Codable, Sendable, Hashable {
    /// Element ID (#identifier)
    public let id: String?
    /// CSS classes (.class1 .class2)
    public let classes: [String]
    /// Key-value pairs (key="value")
    public let keyValues: [KeyValueAttribute]
    public init(id: String? = nil, classes: [String], keyValues: [KeyValueAttribute]) {
        self.id = id
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.keyValues = try container.decodeIfPresent([KeyValueAttribute].self, forKey: .keyValues) ?? []
    }
}

// MARK: - Internal FFI conversions for Attributes
internal extension Attributes {
    init(_ rb: RustBridge.AttributesRef) throws {
        self.id = rb.id()?.toString()
        self.classes = rb.classes().map { $0.as_str().toString() }
        self.keyValues = try rb.keyValues().map { (s: RustStringRef) -> KeyValueAttribute in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(KeyValueAttribute.self, from: d) }
    }

    func intoRust() throws -> RustBridge.Attributes {
        let __classes = RustVec<RustString>()
        for __elem in self.classes { __classes.push(value: RustString(__elem)) }
        let __keyValues = RustVec<RustBridge.KeyValueAttribute>()
        for __elem in self.keyValues { __keyValues.push(value: try __elem.intoRust()) }
        return RustBridge.Attributes(self.id.map(RustString.init), __classes, __keyValues)
    }
}
...
public struct BedrockConfig: Codable, Sendable, Hashable {
    /// AWS region (e.g. `"us-east-1"`).
    public let region: String?
    /// Cross-region inference profile prefix (e.g. `"us"`).
    public let crossRegionPrefix: String?
    ...
    public init(region: String? = nil, crossRegionPrefix: String? = nil, accessKeyId: String? = nil, secretAccessKey: String? = nil, sessionToken: String? = nil) {
        self.region = region
    ...
    private enum CodingKeys: String, CodingKey {
        case region = "region"
        case crossRegionPrefix = "cross_region_prefix"
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.crossRegionPrefix = try container.decodeIfPresent(String.self, forKey: .crossRegionPrefix) ?? nil
        self.accessKeyId = try container.decodeIfPresent(String.self, forKey: .accessKeyId) ?? nil
        self.secretAccessKey = try container.decodeIfPresent(String.self, forKey: .secretAccessKey) ?? nil
        self.sessionToken = try container.decodeIfPresent(String.self, forKey: .sessionToken) ?? nil
    }
}

// MARK: - Internal FFI conversions for BedrockConfig
internal extension BedrockConfig {
    init(_ rb: RustBridge.BedrockConfigRef) throws {
        self.region = rb.region()?.toString()
        self.crossRegionPrefix = rb.crossRegionPrefix()?.toString()
        self.accessKeyId = rb.accessKeyId()?.toString()
        self.secretAccessKey = rb.secretAccessKey()?.toString()
        self.sessionToken = rb.sessionToken()?.toString()
    }

    func intoRust() throws -> RustBridge.BedrockConfig {
        return RustBridge.BedrockConfig(self.region.map(RustString.init), self.crossRegionPrefix.map(RustString.init), self.accessKeyId.map(RustString.init), self.secretAccessKey.map(RustString.init), self.sessionToken.map(RustString.init))
    }
}
...
public struct BibtexMetadata: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.entryCount = try container.decodeIfPresent(UInt.self, forKey: .entryCount) ?? 0
        self.citationKeys = try container.decodeIfPresent([String].self, forKey: .citationKeys) ?? []
        ...
        self.entryTypes = try container.decodeIfPresent([String: UInt].self, forKey: .entryTypes) ?? nil
    }
}

// MARK: - Internal FFI conversions for BibtexMetadata
internal extension BibtexMetadata {
    init(_ rb: RustBridge.BibtexMetadataRef) throws {
        self.entryCount = rb.entryCount()
        self.citationKeys = rb.citationKeys().map { $0.as_str().toString() }
        self.authors = rb.authors().map { $0.as_str().toString() }
        self.yearRange = try rb.yearRange().map { try YearRange($0) }
        self.entryTypes = try JSONDecoder().decode([String: UInt]?.self, from: ((rb.entryTypes().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct ChunkMetadata: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.byteStart = try container.decode(UInt.self, forKey: .byteStart)
        self.byteEnd = try container.decode(UInt.self, forKey: .byteEnd)
        self.tokenCount = try container.decodeIfPresent(UInt.self, forKey: .tokenCount) ?? nil
        self.chunkIndex = try container.decode(UInt.self, forKey: .chunkIndex)
        self.totalChunks = try container.decode(UInt.self, forKey: .totalChunks)
        self.firstPage = try container.decodeIfPresent(UInt32.self, forKey: .firstPage) ?? nil
        self.lastPage = try container.decodeIfPresent(UInt32.self, forKey: .lastPage) ?? nil
        self.headingContext = try container.decodeIfPresent(HeadingContext.self, forKey: .headingContext) ?? nil
        self.headingPath = try container.decodeIfPresent([String].self, forKey: .headingPath) ?? []
        self.imageIndices = try container.decodeIfPresent([UInt32].self, forKey: .imageIndices) ?? []
        self.nodeIds = try container.decodeIfPresent([String].self, forKey: .nodeIds) ?? []
        self.pageSpans = try container.decodeIfPresent([PageSpan].self, forKey: .pageSpans) ?? []
        self.classifications = try container.decodeIfPresent([ClassificationLabel].self, forKey: .classifications) ?? []
    }
}
...
public struct Citation: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.locator = try container.decodeIfPresent(String.self, forKey: .locator) ?? nil
        self.excerpt = try container.decodeIfPresent(String.self, forKey: .excerpt) ?? nil
    }
}
...
public struct CodeDataNode: Codable, Sendable, Hashable {
    /// Whether this node is a key/value pair, XML element, or sequence item.
    public let kind: CodeDataNodeKind
    ...
    /// other kinds.
    public let attributes: [CodeDataAttribute]
    /// Children for nested containers and XML element bodies.
    public let children: [CodeDataNode]
    ...
    public let byteEnd: UInt
    public init(kind: CodeDataNodeKind, key: String? = nil, value: String? = nil, attributes: [CodeDataAttribute], children: [CodeDataNode], byteStart: UInt, byteEnd: UInt) {
        self.kind = kind
        self.key = key
        self.value = value
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.kind = try container.decode(CodeDataNodeKind.self, forKey: .kind)
        ...
        self.attributes = try container.decodeIfPresent([CodeDataAttribute].self, forKey: .attributes) ?? []
        self.children = try container.decodeIfPresent([CodeDataNode].self, forKey: .children) ?? []
        self.byteStart = try container.decode(UInt.self, forKey: .byteStart)
        self.byteEnd = try container.decode(UInt.self, forKey: .byteEnd)
    }
}

// MARK: - Internal FFI conversions for CodeDataNode
internal extension CodeDataNode {
    init(_ rb: RustBridge.CodeDataNodeRef) throws {
        self.kind = try { let rawValue = rb.kind().toString(); guard let value = CodeDataNodeKind(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown CodeDataNodeKind variant", source: rawValue) }; return value }()
        self.key = rb.key()?.toString()
        self.value = rb.value()?.toString()
        self.attributes = try rb.attributes().map { (s: RustStringRef) -> CodeDataAttribute in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(CodeDataAttribute.self, from: d) }
        self.children = try rb.children().map { (s: RustStringRef) -> CodeDataNode in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(CodeDataNode.self, from: d) }
        self.byteStart = rb.byteStart()
        self.byteEnd = rb.byteEnd()
    }

    func intoRust() throws -> RustBridge.CodeDataNode {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
...
public struct CodeMetadata: Codable, Sendable, Hashable {
    ...
    public let data: CodeDataNode?
    public init(chunks: [CodeChunkInfo], data: CodeDataNode? = nil) {
        self.chunks = chunks
        self.data = data
    }
    private enum CodingKeys: String, CodingKey {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.data = try container.decodeIfPresent(CodeDataNode.self, forKey: .data) ?? nil
    }
}
...
internal extension CodeMetadata {
    init(_ rb: RustBridge.CodeMetadataRef) throws {
        ...
        self.data = try rb.data().map { try CodeDataNode($0) }
    }
...
/// };
/// ```
public struct ConcurrencyConfig: Codable, Sendable, Hashable {
    ...
    public let maxConcurrentOcr: UInt?
    public init(maxThreads: UInt? = nil, maxConcurrentOcr: UInt? = nil) {
        self.maxThreads = maxThreads
        self.maxConcurrentOcr = maxConcurrentOcr
    }
    private enum CodingKeys: String, CodingKey {
        case maxThreads = "max_threads"
        case maxConcurrentOcr = "max_concurrent_ocr"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.maxThreads = try container.decodeIfPresent(UInt.self, forKey: .maxThreads) ?? nil
        self.maxConcurrentOcr = try container.decodeIfPresent(UInt.self, forKey: .maxConcurrentOcr) ?? nil
    }
}

// MARK: - Internal FFI conversions for ConcurrencyConfig
internal extension ConcurrencyConfig {
    init(_ rb: RustBridge.ConcurrencyConfigRef) throws {
        self.maxThreads = rb.maxThreads()
        self.maxConcurrentOcr = rb.maxConcurrentOcr()
    }

    func intoRust() throws -> RustBridge.ConcurrencyConfig {
        return RustBridge.ConcurrencyConfig(self.maxThreads, self.maxConcurrentOcr)
    }
}
...
public struct ContentConfig: Codable, Sendable, Hashable {
    ...
    public init(outputFormat: String = "markdown", preprocessingPreset: String = "standard", removeNavigation: Bool = true, removeForms: Bool = true, stripTags: [String], preserveTags: [String], excludeSelectors: [String] = ["noscript"], skipImages: Bool = false, maxDepth: UInt? = nil, wrap: Bool = false, wrapWidth: UInt = 80, includeDocumentStructure: Bool = true) {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.excludeSelectors = try container.decodeIfPresent([String].self, forKey: .excludeSelectors) ?? ["noscript"]
...
public struct ContentFilterConfig: Codable, Sendable, Hashable {
    ...
    public init(includeHeaders: Bool = false, includeFooters: Bool = false, includeFootnotes: Bool = false, stripRepeatingText: Bool = true, includeWatermarks: Bool = false) {
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case includeFootnotes = "include_footnotes"
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.includeFootnotes = try container.decodeIfPresent(Bool.self, forKey: .includeFootnotes) ?? false
...
internal extension ContentFilterConfig {
    init(_ rb: RustBridge.ContentFilterConfigRef) throws {
        ...
        self.includeFootnotes = rb.includeFootnotes()
    ...
    func intoRust() throws -> RustBridge.ContentFilterConfig {
        return RustBridge.ContentFilterConfig(self.includeHeaders, self.includeFooters, self.includeFootnotes, self.stripRepeatingText, self.includeWatermarks)
    }
}
...
/// };
/// ```
public struct CsvConfig: Codable, Sendable, Hashable {
    ...
    public let commentPrefixes: [String]
    public init(delimiter: String? = nil, commentPrefixes: [String]) {
        self.delimiter = delimiter
        self.commentPrefixes = commentPrefixes
    }
    private enum CodingKeys: String, CodingKey {
        case delimiter = "delimiter"
        case commentPrefixes = "comment_prefixes"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.delimiter = try container.decodeIfPresent(String.self, forKey: .delimiter) ?? nil
        self.commentPrefixes = try container.decodeIfPresent([String].self, forKey: .commentPrefixes) ?? []
    }
}

// MARK: - Internal FFI conversions for CsvConfig
internal extension CsvConfig {
    init(_ rb: RustBridge.CsvConfigRef) throws {
        self.delimiter = rb.delimiter()?.toString()
        self.commentPrefixes = rb.commentPrefixes().map { $0.as_str().toString() }
    }

    func intoRust() throws -> RustBridge.CsvConfig {
        let __commentPrefixes = RustVec<RustString>()
        for __elem in self.commentPrefixes { __commentPrefixes.push(value: RustString(__elem)) }
        return RustBridge.CsvConfig(self.delimiter.map(RustString.init), __commentPrefixes)
    }
}
...
internal extension CsvMetadata {
    init(_ rb: RustBridge.CsvMetadataRef) throws {
        ...
        self.columnTypes = try JSONDecoder().decode([String]?.self, from: ((rb.columnTypes().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct DeepseekOcrBackendOptions: Codable, Sendable, Hashable {
    ...
    public let device: CandleDevicePreference?
    ...
    public let dtype: CandleDeepseekOcrDtype?
    public init(modelPath: String? = nil, modelId: String? = nil, hfRevision: String? = nil, cacheDir: String? = nil, device: CandleDevicePreference? = nil, version: UInt32? = nil, dtype: CandleDeepseekOcrDtype? = nil) {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.modelPath = try container.decodeIfPresent(String.self, forKey: .modelPath) ?? nil
        self.modelId = try container.decodeIfPresent(String.self, forKey: .modelId) ?? nil
        self.hfRevision = try container.decodeIfPresent(String.self, forKey: .hfRevision) ?? nil
        self.cacheDir = try container.decodeIfPresent(String.self, forKey: .cacheDir) ?? nil
        self.device = try container.decodeIfPresent(CandleDevicePreference.self, forKey: .device) ?? nil
        self.version = try container.decodeIfPresent(UInt32.self, forKey: .version) ?? nil
        self.dtype = try container.decodeIfPresent(CandleDeepseekOcrDtype.self, forKey: .dtype) ?? nil
    }
}

// MARK: - Internal FFI conversions for DeepseekOcrBackendOptions
internal extension DeepseekOcrBackendOptions {
    init(_ rb: RustBridge.DeepseekOcrBackendOptionsRef) throws {
        self.modelPath = rb.modelPath()?.toString()
        self.modelId = rb.modelId()?.toString()
        self.hfRevision = rb.hfRevision()?.toString()
        self.cacheDir = rb.cacheDir()?.toString()
        self.device = rb.device().flatMap { CandleDevicePreference(rawValue: $0.toString()) }
        self.version = rb.version()
        self.dtype = rb.dtype().flatMap { CandleDeepseekOcrDtype(rawValue: $0.toString()) }
    }

    func intoRust() throws -> RustBridge.DeepseekOcrBackendOptions {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.deepseekOcrBackendOptionsFromJson(json)
    }
}
...
public struct DiffOptions: Codable, Sendable, Hashable {
    ...
    public init(includeMetadata: Bool = true, includeEmbedded: Bool = true, maxContentChars: UInt? = nil) {
...
public struct DjotAttributeGroup: Codable, Sendable, Hashable {
    /// Element identifier used by the Djot attribute map.
    public let identifier: String
    /// Attributes associated with the element.
    public let attributes: Attributes
    public init(identifier: String, attributes: Attributes) {
        self.identifier = identifier
        self.attributes = attributes
    }
    private enum CodingKeys: String, CodingKey {
...
// MARK: - Internal FFI conversions for DjotAttributeGroup
internal extension DjotAttributeGroup {
    init(_ rb: RustBridge.DjotAttributeGroupRef) throws {
        self.identifier = rb.identifier().toString()
        self.attributes = try Attributes(rb.attributes())
    }

    func intoRust() throws -> RustBridge.DjotAttributeGroup {
        return RustBridge.DjotAttributeGroup(RustString(self.identifier), try self.attributes.intoRust())
    }
}
...
public struct DjotImage: Codable, Sendable, Hashable {
    ...
    /// Element attributes
    public let attributes: Attributes?
    public init(src: String, alt: String, title: String? = nil, attributes: Attributes? = nil) {
        self.src = src
        self.alt = alt
        self.title = title
        self.attributes = attributes
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.title = try container.decodeIfPresent(String.self, forKey: .title) ?? nil
        self.attributes = try container.decodeIfPresent(Attributes.self, forKey: .attributes) ?? nil
    }
}
...
internal extension DjotImage {
    init(_ rb: RustBridge.DjotImageRef) throws {
        ...
        self.title = rb.title()?.toString()
        self.attributes = try rb.attributes().map { try Attributes($0) }
    }
...
public struct DjotLink: Codable, Sendable, Hashable {
    ...
    /// Element attributes
    public let attributes: Attributes?
    public init(url: String, text: String, title: String? = nil, attributes: Attributes? = nil) {
        self.url = url
        self.text = text
        self.title = title
        self.attributes = attributes
...
public struct DoctorCheck: Codable, Sendable, Hashable {
    ...
    public init(name: String, status: ProbeStatus, message: String) {
        self.name = name
        self.status = status
...
// MARK: - Internal FFI conversions for DoctorCheck
internal extension DoctorCheck {
    init(_ rb: RustBridge.DoctorCheckRef) throws {
        self.name = rb.name().toString()
        self.status = try { let rawValue = rb.status().toString(); guard let value = ProbeStatus(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown ProbeStatus variant", source: rawValue) }; return value }()
        self.message = rb.message().toString()
    }

    func intoRust() throws -> RustBridge.DoctorCheck {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.doctorCheckFromJson(json)
    }
}
...
public struct DoctorReport: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.checks = try container.decodeIfPresent([DoctorCheck].self, forKey: .checks) ?? []
    }
    ...
    public func isOk() throws  -> Bool {
        ...
        let resultJson = try RustBridge.doctorReportIsOkFromJson(selfString).toString()
...
// MARK: - Internal FFI conversions for DoctorReport
internal extension DoctorReport {
    init(_ rb: RustBridge.DoctorReportRef) throws {
        self.checks = try rb.checks().map { (s: RustStringRef) -> DoctorCheck in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(DoctorCheck.self, from: d) }
    }

    func intoRust() throws -> RustBridge.DoctorReport {
        let __checks = RustVec<RustBridge.DoctorCheck>()
        for __elem in self.checks { __checks.push(value: try __elem.intoRust()) }
        return RustBridge.DoctorReport(__checks)
    }
}
...
internal extension DocumentBoundary {
    init(_ rb: RustBridge.DocumentBoundaryRef) throws {
        ...
        self.confidence = rb.confidence()
        self.reason = try { let rawValue = rb.reason().toString(); guard let value = BoundaryReason(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown BoundaryReason variant", source: rawValue) }; return value }()
    }
...
public struct DocumentNode: Codable, Sendable, Hashable {
    ...
    public init(id: String, content: NodeContent, parent: UInt32? = nil, children: [UInt32], contentLayer: ContentLayer, page: UInt32? = nil, pageEnd: UInt32? = nil, bbox: BoundingBox? = nil, annotations: [TextAnnotation], attributes: [String: String]? = nil) {
        self.id = id
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.content = try container.decode(NodeContent.self, forKey: .content)
        self.parent = try container.decodeIfPresent(UInt32.self, forKey: .parent) ?? nil
        self.children = try container.decodeIfPresent([UInt32].self, forKey: .children) ?? []
        self.contentLayer = try container.decode(ContentLayer.self, forKey: .contentLayer)
        self.page = try container.decodeIfPresent(UInt32.self, forKey: .page) ?? nil
        self.pageEnd = try container.decodeIfPresent(UInt32.self, forKey: .pageEnd) ?? nil
        self.bbox = try container.decodeIfPresent(BoundingBox.self, forKey: .bbox) ?? nil
        self.annotations = try container.decodeIfPresent([TextAnnotation].self, forKey: .annotations) ?? []
        self.attributes = try container.decodeIfPresent([String: String].self, forKey: .attributes) ?? nil
    }
}

// MARK: - Internal FFI conversions for DocumentNode
internal extension DocumentNode {
    init(_ rb: RustBridge.DocumentNodeRef) throws {
        self.id = rb.id().toString()
        self.content = try JSONDecoder().decode(NodeContent.self, from: ((rb.content().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.parent = rb.parent()
        self.children = Array(rb.children())
        self.contentLayer = try { let rawValue = rb.contentLayer().toString(); guard let value = ContentLayer(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown ContentLayer variant", source: rawValue) }; return value }()
        self.page = rb.page()
        self.pageEnd = rb.pageEnd()
        self.bbox = try rb.bbox().map { try BoundingBox($0) }
        self.annotations = try rb.annotations().map { (s: RustStringRef) -> TextAnnotation in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(TextAnnotation.self, from: d) }
        self.attributes = try JSONDecoder().decode([String: String]?.self, from: ((rb.attributes().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
internal extension DocumentRelationship {
    init(_ rb: RustBridge.DocumentRelationshipRef) throws {
        ...
        self.kind = try { let rawValue = rb.kind().toString(); guard let value = RelationshipKind(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown RelationshipKind variant", source: rawValue) }; return value }()
    }
...
internal extension DocumentRevision {
    init(_ rb: RustBridge.DocumentRevisionRef) throws {
        ...
        self.kind = try { let rawValue = rb.kind().toString(); guard let value = RevisionKind(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown RevisionKind variant", source: rawValue) }; return value }()
...
public struct DocumentStructure: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.nodes = try container.decodeIfPresent([DocumentNode].self, forKey: .nodes) ?? []
        self.sourceFormat = try container.decodeIfPresent(String.self, forKey: .sourceFormat) ?? nil
        self.relationships = try container.decodeIfPresent([DocumentRelationship].self, forKey: .relationships) ?? []
        self.nodeTypes = try container.decodeIfPresent([String].self, forKey: .nodeTypes) ?? []
    }
    // MARK: - Instance Methods
    public func finalizeNodeTypes() throws  {
        let jsonSelf = try JSONEncoder().encode(self)
        let selfString = String(data: jsonSelf, encoding: .utf8) ?? "{}"
        _ = try RustBridge.documentStructureFinalizeNodeTypesFromJson(selfString)
    }

    public func isEmpty() throws  -> Bool {
        let jsonSelf = try JSONEncoder().encode(self)
        let selfString = String(data: jsonSelf, encoding: .utf8) ?? "{}"
        let resultJson = try RustBridge.documentStructureIsEmptyFromJson(selfString).toString()
        let data = resultJson.data(using: .utf8) ?? Data()
        return try JSONDecoder().decode(Bool.self, from: data)
    }
...
internal extension DocumentStructure {
    init(_ rb: RustBridge.DocumentStructureRef) throws {
        self.nodes = try rb.nodes().map { (s: RustStringRef) -> DocumentNode in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(DocumentNode.self, from: d) }
        self.sourceFormat = rb.sourceFormat()?.toString()
        self.relationships = try rb.relationships().map { (s: RustStringRef) -> DocumentRelationship in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(DocumentRelationship.self, from: d) }
        self.nodeTypes = rb.nodeTypes().map { $0.as_str().toString() }
    }

    func intoRust() throws -> RustBridge.DocumentStructure {
        let __nodes = RustVec<RustBridge.DocumentNode>()
        for __elem in self.nodes { __nodes.push(value: try __elem.intoRust()) }
        let __relationships = RustVec<RustBridge.DocumentRelationship>()
        for __elem in self.relationships { __relationships.push(value: try __elem.intoRust()) }
        let __nodeTypes = RustVec<RustString>()
        for __elem in self.nodeTypes { __nodeTypes.push(value: RustString(__elem)) }
        return RustBridge.DocumentStructure(__nodes, self.sourceFormat.map(RustString.init), __relationships, __nodeTypes)
    }
}
...
public struct DocumentSummary: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.tokenCount = try container.decodeIfPresent(UInt32.self, forKey: .tokenCount) ?? nil
    }
}
...
internal extension DocumentSummary {
    init(_ rb: RustBridge.DocumentSummaryRef) throws {
        self.text = rb.text().toString()
        self.strategy = try { let rawValue = rb.strategy().toString(); guard let value = SummaryStrategy(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown SummaryStrategy variant", source: rawValue) }; return value }()
        self.tokenCount = rb.tokenCount()
    }
...
public struct DocxMetadata: Codable, Sendable, Hashable {
    ...
    public let customProperties: [String: JSONValue]?
    public init(coreProperties: CoreProperties? = nil, appProperties: DocxAppProperties? = nil, customProperties: [String: JSONValue]? = nil) {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.coreProperties = try container.decodeIfPresent(CoreProperties.self, forKey: .coreProperties) ?? nil
        self.appProperties = try container.decodeIfPresent(DocxAppProperties.self, forKey: .appProperties) ?? nil
        self.customProperties = try container.decodeIfPresent([String: JSONValue].self, forKey: .customProperties) ?? nil
    }
}

// MARK: - Internal FFI conversions for DocxMetadata
internal extension DocxMetadata {
    init(_ rb: RustBridge.DocxMetadataRef) throws {
        self.coreProperties = try rb.coreProperties().map { try CoreProperties($0) }
        self.appProperties = try rb.appProperties().map { try DocxAppProperties($0) }
        self.customProperties = try JSONDecoder().decode([String: JSONValue]?.self, from: ((rb.customProperties().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
internal extension Element {
    init(_ rb: RustBridge.ElementRef) throws {
        self.elementId = rb.elementId().toString()
        self.elementType = try { let rawValue = rb.elementType().toString(); guard let value = ElementType(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown ElementType variant", source: rawValue) }; return value }()
        self.text = rb.text().toString()
        self.metadata = try ElementMetadata(rb.metadata())
    }
...
public struct ElementMetadata: Codable, Sendable, Hashable {
    ...
    public let coordinates: BoundingBox?
    /// Position index in the element sequence
    public let elementIndex: UInt?
    /// Additional custom metadata
    public let additional: [String: String]
    public init(pageNumber: UInt32? = nil, filename: String? = nil, coordinates: BoundingBox? = nil, elementIndex: UInt? = nil, additional: [String: String]) {
...
internal extension ElementMetadata {
    init(_ rb: RustBridge.ElementMetadataRef) throws {
        self.pageNumber = rb.pageNumber()
        self.filename = rb.filename()?.toString()
        self.coordinates = try rb.coordinates().map { try BoundingBox($0) }
        self.elementIndex = rb.elementIndex()
        self.additional = try JSONDecoder().decode([String: String].self, from: ((rb.additional().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct Entity: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.category = try container.decode(EntityCategory.self, forKey: .category)
        self.text = try container.decode(String.self, forKey: .text)
        self.start = try container.decode(UInt32.self, forKey: .start)
        self.end = try container.decode(UInt32.self, forKey: .end)
        self.confidence = try container.decodeIfPresent(Float.self, forKey: .confidence) ?? nil
    }
}
...
internal extension ExcelMetadata {
    init(_ rb: RustBridge.ExcelMetadataRef) throws {
        ...
        self.sheetNames = try JSONDecoder().decode([String]?.self, from: ((rb.sheetNames().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct ExcelWorkbook: Codable, Sendable, Hashable {
    ...
    public let revisions: [DocumentRevision]?
    public init(sheets: [ExcelSheet], metadata: [String: String], revisions: [DocumentRevision]? = nil) {
        self.sheets = sheets
        self.metadata = metadata
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.sheets = try container.decode([ExcelSheet].self, forKey: .sheets)
        self.metadata = try container.decode([String: String].self, forKey: .metadata)
        self.revisions = try container.decodeIfPresent([DocumentRevision].self, forKey: .revisions) ?? nil
    }
}

// MARK: - Internal FFI conversions for ExcelWorkbook
internal extension ExcelWorkbook {
    init(_ rb: RustBridge.ExcelWorkbookRef) throws {
        self.sheets = try rb.sheets().map { (s: RustStringRef) -> ExcelSheet in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(ExcelSheet.self, from: d) }
        self.metadata = try JSONDecoder().decode([String: String].self, from: ((rb.metadata().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.revisions = try JSONDecoder().decode([DocumentRevision]?.self, from: ((rb.revisions().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct ExtractedUri: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.page = try container.decodeIfPresent(UInt32.self, forKey: .page) ?? nil
        self.kind = try container.decode(UriKind.self, forKey: .kind)
    }
}
...
internal extension ExtractedUri {
    init(_ rb: RustBridge.ExtractedUriRef) throws {
        ...
        self.label = rb.label()?.toString()
        self.page = rb.page()
        self.kind = try { let rawValue = rb.kind().toString(); guard let value = UriKind(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown UriKind variant", source: rawValue) }; return value }()
    }
...
internal extension ExtractionConfidence {
    init(_ rb: RustBridge.ExtractionConfidenceRef) throws {
        ...
        self.schemaCompliance = try { let rawValue = rb.schemaCompliance().toString(); guard let value = SchemaCompliance(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown SchemaCompliance variant", source: rawValue) }; return value }()
...
internal extension Footnote {
    init(_ rb: RustBridge.FootnoteRef) throws {
        self.label = rb.label().toString()
        self.content = try rb.content().map { (s: RustStringRef) -> FormattedBlock in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(FormattedBlock.self, from: d) }
    }
...
public struct FootnoteConfig: Codable, Sendable, Hashable {
    ...
    public init(parseCitations: Bool = true) {
...
public struct FormattedBlock: Codable, Sendable, Hashable {
    ...
    public init(blockType: BlockType, level: UInt? = nil, inlineContent: [InlineElement], attributes: Attributes? = nil, language: String? = nil, code: String? = nil, children: [FormattedBlock]) {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.level = try container.decodeIfPresent(UInt.self, forKey: .level) ?? nil
        self.inlineContent = try container.decode([InlineElement].self, forKey: .inlineContent)
        self.attributes = try container.decodeIfPresent(Attributes.self, forKey: .attributes) ?? nil
        self.language = try container.decodeIfPresent(String.self, forKey: .language) ?? nil
        self.code = try container.decodeIfPresent(String.self, forKey: .code) ?? nil
        self.children = try container.decodeIfPresent([FormattedBlock].self, forKey: .children) ?? []
    }
}

// MARK: - Internal FFI conversions for FormattedBlock
internal extension FormattedBlock {
    init(_ rb: RustBridge.FormattedBlockRef) throws {
        self.blockType = try { let rawValue = rb.blockType().toString(); guard let value = BlockType(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown BlockType variant", source: rawValue) }; return value }()
        self.level = rb.level()
        self.inlineContent = try rb.inlineContent().map { (s: RustStringRef) -> InlineElement in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(InlineElement.self, from: d) }
        self.attributes = try rb.attributes().map { try Attributes($0) }
        self.language = rb.language()?.toString()
        self.code = rb.code()?.toString()
        self.children = try rb.children().map { (s: RustStringRef) -> FormattedBlock in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(FormattedBlock.self, from: d) }
    }
...
public struct GeoJsonExtractionConfig: Codable, Sendable, Hashable {
    ...
    public let includeFullCoordinates: Bool
    public init(includeFullCoordinates: Bool) {
        self.includeFullCoordinates = includeFullCoordinates
    }
    private enum CodingKeys: String, CodingKey {
        case includeFullCoordinates = "include_full_coordinates"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.includeFullCoordinates = try container.decodeIfPresent(Bool.self, forKey: .includeFullCoordinates) ?? false
    }
}

// MARK: - Internal FFI conversions for GeoJsonExtractionConfig
internal extension GeoJsonExtractionConfig {
    init(_ rb: RustBridge.GeoJsonExtractionConfigRef) throws {
        self.includeFullCoordinates = rb.includeFullCoordinates()
    }

    func intoRust() throws -> RustBridge.GeoJsonExtractionConfig {
        return RustBridge.GeoJsonExtractionConfig(self.includeFullCoordinates)
    }
}

/// Runtime options accepted by the `candle-glm-ocr` backend.
public struct GlmOcrBackendOptions: Codable, Sendable, Hashable {
    /// Optional recognition task; the backend constructor's task is used when absent.
    public let task: GlmOcrTaskKind?
    /// Optional per-call device override.
    public let device: CandleDevicePreference?
    /// Optional whole-page or layout-paired dispatch mode.
    public let layoutMode: GlmOcrLayoutMode?
    /// Whether chart regions use chart understanding instead of captioning.
    public let enableChartUnderstanding: Bool?
    /// Optional Hugging Face cache root.
    public let cacheDir: String?
    public init(task: GlmOcrTaskKind? = nil, device: CandleDevicePreference? = nil, layoutMode: GlmOcrLayoutMode? = nil, enableChartUnderstanding: Bool? = nil, cacheDir: String? = nil) {
        self.task = task
        self.device = device
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.task = try container.decodeIfPresent(GlmOcrTaskKind.self, forKey: .task) ?? nil
        self.device = try container.decodeIfPresent(CandleDevicePreference.self, forKey: .device) ?? nil
        self.layoutMode = try container.decodeIfPresent(GlmOcrLayoutMode.self, forKey: .layoutMode) ?? nil
        self.enableChartUnderstanding = try container.decodeIfPresent(Bool.self, forKey: .enableChartUnderstanding) ?? nil
        self.cacheDir = try container.decodeIfPresent(String.self, forKey: .cacheDir) ?? nil
    }
}

// MARK: - Internal FFI conversions for GlmOcrBackendOptions
internal extension GlmOcrBackendOptions {
    init(_ rb: RustBridge.GlmOcrBackendOptionsRef) throws {
        self.task = rb.task().flatMap { GlmOcrTaskKind(rawValue: $0.toString()) }
        self.device = rb.device().flatMap { CandleDevicePreference(rawValue: $0.toString()) }
        self.layoutMode = rb.layoutMode().flatMap { GlmOcrLayoutMode(rawValue: $0.toString()) }
        self.enableChartUnderstanding = rb.enableChartUnderstanding()
        self.cacheDir = rb.cacheDir()?.toString()
    }

    func intoRust() throws -> RustBridge.GlmOcrBackendOptions {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.glmOcrBackendOptionsFromJson(json)
    }
}
...
public struct GridCell: Codable, Sendable, Hashable {
    ...
    public init(content: String, row: UInt32, col: UInt32, rowSpan: UInt32 = 1, colSpan: UInt32 = 1, isHeader: Bool, bbox: BoundingBox? = nil, headingLevel: UInt8? = nil, styleName: String? = nil) {
        self.content = content
        self.row = row
        self.col = col
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.rowSpan = try container.decodeIfPresent(UInt32.self, forKey: .rowSpan) ?? 1
        self.colSpan = try container.decodeIfPresent(UInt32.self, forKey: .colSpan) ?? 1
        self.isHeader = try container.decode(Bool.self, forKey: .isHeader)
        self.bbox = try container.decodeIfPresent(BoundingBox.self, forKey: .bbox) ?? nil
        self.headingLevel = try container.decodeIfPresent(UInt8.self, forKey: .headingLevel) ?? nil
        self.styleName = try container.decodeIfPresent(String.self, forKey: .styleName) ?? nil
    }
}
...
internal extension GridCell {
    init(_ rb: RustBridge.GridCellRef) throws {
        ...
        self.bbox = try rb.bbox().map { try BoundingBox($0) }
        self.headingLevel = rb.headingLevel()
        self.styleName = rb.styleName()?.toString()
    }
...
public struct HeaderMetadata: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.text = try container.decode(String.self, forKey: .text)
        self.id = try container.decodeIfPresent(String.self, forKey: .id) ?? nil
        self.depth = try container.decode(UInt32.self, forKey: .depth)
        self.htmlOffset = try container.decode(UInt32.self, forKey: .htmlOffset)
    }
}
...
public struct HeuristicsConfig: Codable, Sendable, Hashable {
    ...
    public init(enablePdfTextHeuristics: Bool = true, textLayerThreshold: Float = 0.7, fileSizeThresholdBytes: UInt64 = 10485760, pageCountThreshold: UInt32 = 50, targetPagesPerChunk: UInt32 = 10, maxPagesPerChunk: UInt32 = 25, diskProcessingThresholdBytes: UInt64 = 52428800, minCharsPerPage: UInt32 = 50, maxXlsxSheetCount: UInt32 = 200, maxXlsxWorkbookCells: UInt64 = 5000000, maxPptxEmbeddedCount: UInt32 = 50) {
...
public struct HierarchicalBlock: Codable, Sendable, Hashable {
    ...
    public let bbox: HierarchicalBoundingBox?
    public init(text: String, fontSize: Float, level: String, bbox: HierarchicalBoundingBox? = nil) {
        self.text = text
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.bbox = try container.decodeIfPresent(HierarchicalBoundingBox.self, forKey: .bbox) ?? nil
    }
}
...
internal extension HierarchicalBlock {
    init(_ rb: RustBridge.HierarchicalBlockRef) throws {
        ...
        self.bbox = try rb.bbox().map { try HierarchicalBoundingBox($0) }
    }
...
public struct HierarchicalBoundingBox: Codable, Sendable, Hashable {
    /// Left coordinate.
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.left = try container.decodeIfPresent(Float.self, forKey: .left) ?? 0
        self.top = try container.decodeIfPresent(Float.self, forKey: .top) ?? 0
        self.right = try container.decodeIfPresent(Float.self, forKey: .right) ?? 0
        self.bottom = try container.decodeIfPresent(Float.self, forKey: .bottom) ?? 0
    }
}

// MARK: - Internal FFI conversions for HierarchicalBoundingBox
internal extension HierarchicalBoundingBox {
    init(_ rb: RustBridge.HierarchicalBoundingBoxRef) throws {
        self.left = rb.left()
        self.top = rb.top()
        self.right = rb.right()
    ...
    func intoRust() throws -> RustBridge.HierarchicalBoundingBox {
        return RustBridge.HierarchicalBoundingBox(self.left, self.top, self.right, self.bottom)
    }
}
...
public struct HierarchyConfig: Codable, Sendable, Hashable {
    ...
    public init(enabled: Bool = true, kClusters: UInt = 3, includeBbox: Bool = true) {
...
public struct HtmlMetadata: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.baseHref = try container.decodeIfPresent(String.self, forKey: .baseHref) ?? nil
        self.language = try container.decodeIfPresent(String.self, forKey: .language) ?? nil
        self.textDirection = try container.decodeIfPresent(TextDirection.self, forKey: .textDirection) ?? nil
        self.openGraph = try container.decodeIfPresent([String: String].self, forKey: .openGraph) ?? [:]
        self.twitterCard = try container.decodeIfPresent([String: String].self, forKey: .twitterCard) ?? [:]
        self.metaTags = try container.decodeIfPresent([String: String].self, forKey: .metaTags) ?? [:]
        self.headers = try container.decodeIfPresent([HeaderMetadata].self, forKey: .headers) ?? []
        self.links = try container.decodeIfPresent([LinkMetadata].self, forKey: .links) ?? []
        self.images = try container.decodeIfPresent([ImageMetadataType].self, forKey: .images) ?? []
        self.structuredData = try container.decodeIfPresent([StructuredData].self, forKey: .structuredData) ?? []
    }
}

// MARK: - Internal FFI conversions for HtmlMetadata
internal extension HtmlMetadata {
    init(_ rb: RustBridge.HtmlMetadataRef) throws {
        self.title = rb.title()?.toString()
        self.description = rb.description()?.toString()
        self.keywords = rb.keywords().map { $0.as_str().toString() }
        self.author = rb.author()?.toString()
        ...
        self.language = rb.language()?.toString()
        self.textDirection = rb.textDirection().flatMap { TextDirection(rawValue: $0.toString()) }
        self.openGraph = try JSONDecoder().decode([String: String].self, from: ((rb.openGraph().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.twitterCard = try JSONDecoder().decode([String: String].self, from: ((rb.twitterCard().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.metaTags = try JSONDecoder().decode([String: String].self, from: ((rb.metaTags().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.headers = try rb.headers().map { (s: RustStringRef) -> HeaderMetadata in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(HeaderMetadata.self, from: d) }
        self.links = try rb.links().map { (s: RustStringRef) -> LinkMetadata in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(LinkMetadata.self, from: d) }
        self.images = try rb.images().map { (s: RustStringRef) -> ImageMetadataType in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(ImageMetadataType.self, from: d) }
        self.structuredData = try rb.structuredData().map { (s: RustStringRef) -> StructuredData in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(StructuredData.self, from: d) }
    }
...
public struct ImageDimensions: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.width = try container.decodeIfPresent(UInt32.self, forKey: .width) ?? 0
        self.height = try container.decodeIfPresent(UInt32.self, forKey: .height) ?? 0
    }
}
...
public struct ImageDpi: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.horizontal = try container.decodeIfPresent(Double.self, forKey: .horizontal) ?? 0
        self.vertical = try container.decodeIfPresent(Double.self, forKey: .vertical) ?? 0
    }
}

// MARK: - Internal FFI conversions for ImageDpi
internal extension ImageDpi {
    init(_ rb: RustBridge.ImageDpiRef) throws {
        self.horizontal = rb.horizontal()
        self.vertical = rb.vertical()
    }

    func intoRust() throws -> RustBridge.ImageDpi {
        return RustBridge.ImageDpi(self.horizontal, self.vertical)
    }
}
...
public struct ImageExtractionConfig: Codable, Sendable, Hashable {
    ...
    public init(extractImages: Bool = true, targetDpi: Int32 = 300, maxImageDimension: Int32 = 4096, injectPlaceholders: Bool = true, autoAdjustDpi: Bool = true, minDpi: Int32 = 72, maxDpi: Int32 = 600, maxImagesPerPage: UInt32? = nil, classify: Bool = false, includePageRasters: Bool = false, runOcrOnImages: Bool = true, ocrTextOnly: Bool = false, appendOcrText: Bool = false, outputFormat: ImageOutputFormat, svg: SvgOptions, includeDataBase64: Bool = false) {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.svg = try container.decodeIfPresent(SvgOptions.self, forKey: .svg) ?? SvgOptions()
...
public struct ImageMetadata: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.height = try container.decodeIfPresent(UInt32.self, forKey: .height) ?? 0
        self.format = try container.decodeIfPresent(String.self, forKey: .format) ?? ""
        self.exif = try container.decodeIfPresent([String: String].self, forKey: .exif) ?? [:]
    }
}

// MARK: - Internal FFI conversions for ImageMetadata
internal extension ImageMetadata {
    init(_ rb: RustBridge.ImageMetadataRef) throws {
        self.width = rb.width()
        self.height = rb.height()
        self.format = rb.format().toString()
        self.exif = try JSONDecoder().decode([String: String].self, from: ((rb.exif().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct ImageMetadataType: Codable, Sendable, Hashable {
    ...
    public let attributes: [KeyValueAttribute]
    public init(src: String, alt: String? = nil, title: String? = nil, dimensions: ImageDimensions? = nil, imageType: ImageType, attributes: [KeyValueAttribute]) {
        self.src = src
        self.alt = alt
        self.title = title
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.title = try container.decodeIfPresent(String.self, forKey: .title) ?? nil
        self.dimensions = try container.decodeIfPresent(ImageDimensions.self, forKey: .dimensions) ?? nil
        self.imageType = try container.decode(ImageType.self, forKey: .imageType)
        self.attributes = try container.decode([KeyValueAttribute].self, forKey: .attributes)
    }
}
...
internal extension ImageMetadataType {
    init(_ rb: RustBridge.ImageMetadataTypeRef) throws {
        ...
        self.alt = rb.alt()?.toString()
        self.title = rb.title()?.toString()
        self.dimensions = try rb.dimensions().map { try ImageDimensions($0) }
        self.imageType = try { let rawValue = rb.imageType().toString(); guard let value = ImageType(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown ImageType variant", source: rawValue) }; return value }()
        self.attributes = try rb.attributes().map { (s: RustStringRef) -> KeyValueAttribute in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(KeyValueAttribute.self, from: d) }
    }
...
public struct ImagePreprocessingConfig: Codable, Sendable, Hashable {
    ...
    public init(targetDpi: Int32 = 300, autoRotate: Bool = false, deskew: Bool = true, denoise: Bool = false, contrastEnhance: Bool = false, binarizationMethod: String = "otsu", invertColors: Bool = false) {
        self.targetDpi = targetDpi
...
public struct ImagePreprocessingMetadata: Codable, Sendable, Hashable {
    ...
    public let originalDpi: ImageDpi
    ...
    public let newDimensions: PixelDimensions?
    ...
    public init(originalDimensions: PixelDimensions, originalDpi: ImageDpi, targetDpi: Int32, scaleFactor: Double, autoAdjusted: Bool, finalDpi: Int32, newDimensions: PixelDimensions? = nil, resampleMethod: String, dimensionClamped: Bool, calculatedDpi: Int32? = nil, skippedResize: Bool, resizeError: String? = nil) {
...
internal extension ImagePreprocessingMetadata {
    init(_ rb: RustBridge.ImagePreprocessingMetadataRef) throws {
        self.originalDimensions = try PixelDimensions(rb.originalDimensions())
        self.originalDpi = try ImageDpi(rb.originalDpi())
        self.targetDpi = rb.targetDpi()
        self.scaleFactor = rb.scaleFactor()
        self.autoAdjusted = rb.autoAdjusted()
        self.finalDpi = rb.finalDpi()
        self.newDimensions = try rb.newDimensions().map { try PixelDimensions($0) }
...
public struct InlineElement: Codable, Sendable, Hashable {
    ...
    public let metadata: [String: String]?
    public init(elementType: InlineType, content: String, attributes: Attributes? = nil, metadata: [String: String]? = nil) {
        self.elementType = elementType
        self.content = content
        self.attributes = attributes
        self.metadata = metadata
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.elementType = try container.decode(InlineType.self, forKey: .elementType)
        self.content = try container.decode(String.self, forKey: .content)
        self.attributes = try container.decodeIfPresent(Attributes.self, forKey: .attributes) ?? nil
        self.metadata = try container.decodeIfPresent([String: String].self, forKey: .metadata) ?? nil
    }
}

// MARK: - Internal FFI conversions for InlineElement
internal extension InlineElement {
    init(_ rb: RustBridge.InlineElementRef) throws {
        self.elementType = try { let rawValue = rb.elementType().toString(); guard let value = InlineType(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown InlineType variant", source: rawValue) }; return value }()
        self.content = rb.content().toString()
        self.attributes = try rb.attributes().map { try Attributes($0) }
        self.metadata = try JSONDecoder().decode([String: String]?.self, from: ((rb.metadata().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct JatsMetadata: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.copyright = try container.decodeIfPresent(String.self, forKey: .copyright) ?? nil
        self.license = try container.decodeIfPresent(String.self, forKey: .license) ?? nil
        self.historyDates = try container.decodeIfPresent([String: String].self, forKey: .historyDates) ?? [:]
        self.contributorRoles = try container.decodeIfPresent([ContributorRole].self, forKey: .contributorRoles) ?? []
    }
}

// MARK: - Internal FFI conversions for JatsMetadata
internal extension JatsMetadata {
    init(_ rb: RustBridge.JatsMetadataRef) throws {
        self.copyright = rb.copyright()?.toString()
        self.license = rb.license()?.toString()
        self.historyDates = try JSONDecoder().decode([String: String].self, from: ((rb.historyDates().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.contributorRoles = try rb.contributorRoles().map { (s: RustStringRef) -> ContributorRole in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(ContributorRole.self, from: d) }
    }
...
public struct KeyValueAttribute: Codable, Sendable, Hashable {
    /// Attribute name.
    public let key: String
    /// Attribute value.
    public let value: String
...
// MARK: - Internal FFI conversions for KeyValueAttribute
internal extension KeyValueAttribute {
    init(_ rb: RustBridge.KeyValueAttributeRef) throws {
        self.key = rb.key().toString()
        self.value = rb.value().toString()
    }

    func intoRust() throws -> RustBridge.KeyValueAttribute {
        return RustBridge.KeyValueAttribute(RustString(self.key), RustString(self.value))
    }
}
...
public struct Keyword: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.positions = try container.decodeIfPresent([UInt].self, forKey: .positions) ?? nil
    }
}
...
internal extension Keyword {
    init(_ rb: RustBridge.KeywordRef) throws {
        self.text = rb.text().toString()
        self.score = rb.score()
        self.algorithm = try { let rawValue = rb.algorithm().toString(); guard let value = KeywordAlgorithm(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown KeywordAlgorithm variant", source: rawValue) }; return value }()
        self.positions = try JSONDecoder().decode([UInt]?.self, from: ((rb.positions().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct KeywordConfig: Codable, Sendable, Hashable {
    ...
    public let ngramRange: NgramRange
    ...
    public init(algorithm: KeywordAlgorithm, maxKeywords: UInt = 10, minScore: Float = 0.0, ngramRange: NgramRange, language: String? = "en", yakeParams: YakeParams? = nil, rakeParams: RakeParams? = nil) {
        self.algorithm = algorithm
...
internal extension KeywordConfig {
    init(_ rb: RustBridge.KeywordConfigRef) throws {
        ...
        self.ngramRange = try NgramRange(rb.ngramRange())
        self.language = rb.language()?.toString()
...
public struct LanguageConfidence: Codable, Sendable, Hashable {
    ...
    public init(language: String, confidence: Double, proportion: Double, script: String, reliable: Bool) {
        self.language = language
        self.confidence = confidence
...
// MARK: - Internal FFI conversions for LanguageConfidence
internal extension LanguageConfidence {
    init(_ rb: RustBridge.LanguageConfidenceRef) throws {
        self.language = rb.language().toString()
        self.confidence = rb.confidence()
        self.proportion = rb.proportion()
        self.script = rb.script().toString()
        self.reliable = rb.reliable()
    }

    func intoRust() throws -> RustBridge.LanguageConfidence {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.languageConfidenceFromJson(json)
    }
}
...
public struct LanguageDetectionConfig: Codable, Sendable, Hashable {
    ...
    public init(enabled: Bool = true, minConfidence: Double = 0.8, detectMultiple: Bool = false) {
        self.enabled = enabled
...
internal extension LayoutDetection {
    init(_ rb: RustBridge.LayoutDetectionRef) throws {
        self.className = try { let rawValue = rb.className().toString(); guard let value = LayoutClass(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown LayoutClass variant", source: rawValue) }; return value }()
        self.confidence = rb.confidence()
...
public struct LayoutDetectionConfig: Codable, Sendable, Hashable {
    ...
    /// cannot help.
    public let strategy: LayoutStrategy
    ...
    public let enableChartUnderstanding: Bool
    public init(strategy: LayoutStrategy, confidenceThreshold: Float? = nil, applyHeuristics: Bool = true, tableModel: TableModel, formulaModel: FormulaModel? = nil, tableOverlapPreference: TableOverlapPreference, acceleration: AccelerationConfig? = nil, enableChartUnderstanding: Bool = false) {
        self.strategy = strategy
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.strategy = try container.decode(LayoutStrategy.self, forKey: .strategy)
...
internal extension LayoutDetectionConfig {
    init(_ rb: RustBridge.LayoutDetectionConfigRef) throws {
        self.strategy = try { let rawValue = rb.strategy().toString(); guard let value = LayoutStrategy(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown LayoutStrategy variant", source: rawValue) }; return value }()
        self.confidenceThreshold = rb.confidenceThreshold()
        self.applyHeuristics = rb.applyHeuristics()
        self.tableModel = try { let rawValue = rb.tableModel().toString(); guard let value = TableModel(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown TableModel variant", source: rawValue) }; return value }()
        self.formulaModel = rb.formulaModel().flatMap { FormulaModel(rawValue: $0.toString()) }
        self.tableOverlapPreference = try { let rawValue = rb.tableOverlapPreference().toString(); guard let value = TableOverlapPreference(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown TableOverlapPreference variant", source: rawValue) }; return value }()
...
public struct LinkMetadata: Codable, Sendable, Hashable {
    ...
    /// Additional attributes as key-value pairs.
    public let attributes: [KeyValueAttribute]
    public init(href: String, text: String, title: String? = nil, linkType: LinkType, rel: [String], attributes: [KeyValueAttribute]) {
        self.href = href
        self.text = text
        self.title = title
...
internal extension LinkMetadata {
    init(_ rb: RustBridge.LinkMetadataRef) throws {
        ...
        self.text = rb.text().toString()
        self.title = rb.title()?.toString()
        self.linkType = try { let rawValue = rb.linkType().toString(); guard let value = LinkType(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown LinkType variant", source: rawValue) }; return value }()
...
public struct LlmBudgetConfig: Codable, Sendable, Hashable {
    /// Global spend limit in USD.
    public let globalLimit: Double?
    /// Per-model spend limits in USD, keyed by model name.
    public let modelLimits: [String: Double]?
    /// Enforcement mode: `"hard"` (reject over-budget requests) or `"soft"` (log only).
    public let enforcement: String?
    public init(globalLimit: Double? = nil, modelLimits: [String: Double]? = nil, enforcement: String? = nil) {
        self.globalLimit = globalLimit
        self.modelLimits = modelLimits
        self.enforcement = enforcement
    }
    private enum CodingKeys: String, CodingKey {
        case globalLimit = "global_limit"
        case modelLimits = "model_limits"
        case enforcement = "enforcement"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.globalLimit = try container.decodeIfPresent(Double.self, forKey: .globalLimit) ?? nil
        self.modelLimits = try container.decodeIfPresent([String: Double].self, forKey: .modelLimits) ?? nil
        self.enforcement = try container.decodeIfPresent(String.self, forKey: .enforcement) ?? nil
    }
}

// MARK: - Internal FFI conversions for LlmBudgetConfig
internal extension LlmBudgetConfig {
    init(_ rb: RustBridge.LlmBudgetConfigRef) throws {
        self.globalLimit = rb.globalLimit()
        self.modelLimits = try JSONDecoder().decode([String: Double]?.self, from: ((rb.modelLimits().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.enforcement = rb.enforcement()?.toString()
    }

    func intoRust() throws -> RustBridge.LlmBudgetConfig {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.llmBudgetConfigFromJson(json)
    }
}
...
public struct LlmCacheConfig: Codable, Sendable, Hashable {
    /// Maximum number of cached entries.
    public let maxEntries: UInt?
    ...
    public let backendConfig: [String: String]?
    public init(maxEntries: UInt? = nil, ttlSeconds: UInt64? = nil, backend: String? = nil, backendConfig: [String: String]? = nil) {
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case backendConfig = "backend_config"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.maxEntries = try container.decodeIfPresent(UInt.self, forKey: .maxEntries) ?? nil
        self.ttlSeconds = try container.decodeIfPresent(UInt64.self, forKey: .ttlSeconds) ?? nil
        self.backend = try container.decodeIfPresent(String.self, forKey: .backend) ?? nil
        self.backendConfig = try container.decodeIfPresent([String: String].self, forKey: .backendConfig) ?? nil
    }
}

// MARK: - Internal FFI conversions for LlmCacheConfig
internal extension LlmCacheConfig {
    init(_ rb: RustBridge.LlmCacheConfigRef) throws {
        self.maxEntries = rb.maxEntries()
        self.ttlSeconds = rb.ttlSeconds()
        self.backend = rb.backend()?.toString()
        self.backendConfig = try JSONDecoder().decode([String: String]?.self, from: ((rb.backendConfig().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }

    func intoRust() throws -> RustBridge.LlmCacheConfig {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.llmCacheConfigFromJson(json)
    }
}
...
public struct LlmProviderConfig: Codable, Sendable, Hashable {
    ...
    public init(name: String, baseUrl: String, authHeader: String? = nil, modelPrefixes: [String]) {
        self.name = name
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.authHeader = try container.decodeIfPresent(String.self, forKey: .authHeader) ?? nil
        self.modelPrefixes = try container.decodeIfPresent([String].self, forKey: .modelPrefixes) ?? []
    }
}

// MARK: - Internal FFI conversions for LlmProviderConfig
internal extension LlmProviderConfig {
    init(_ rb: RustBridge.LlmProviderConfigRef) throws {
        self.name = rb.name().toString()
        self.baseUrl = rb.baseUrl().toString()
        self.authHeader = rb.authHeader()?.toString()
        self.modelPrefixes = rb.modelPrefixes().map { $0.as_str().toString() }
    }

    func intoRust() throws -> RustBridge.LlmProviderConfig {
        let __modelPrefixes = RustVec<RustString>()
        for __elem in self.modelPrefixes { __modelPrefixes.push(value: RustString(__elem)) }
        return RustBridge.LlmProviderConfig(RustString(self.name), RustString(self.baseUrl), self.authHeader.map(RustString.init), __modelPrefixes)
    }
}
...
public struct LlmRateLimitConfig: Codable, Sendable, Hashable {
    /// Requests per minute limit.
    public let rpm: UInt32?
    /// Tokens per minute limit.
    public let tpm: UInt64?
    /// Rate limit window, in seconds.
    public let windowSeconds: UInt64?
    public init(rpm: UInt32? = nil, tpm: UInt64? = nil, windowSeconds: UInt64? = nil) {
        self.rpm = rpm
        self.tpm = tpm
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.rpm = try container.decodeIfPresent(UInt32.self, forKey: .rpm) ?? nil
        self.tpm = try container.decodeIfPresent(UInt64.self, forKey: .tpm) ?? nil
        self.windowSeconds = try container.decodeIfPresent(UInt64.self, forKey: .windowSeconds) ?? nil
    }
}

// MARK: - Internal FFI conversions for LlmRateLimitConfig
internal extension LlmRateLimitConfig {
    init(_ rb: RustBridge.LlmRateLimitConfigRef) throws {
        self.rpm = rb.rpm()
        self.tpm = rb.tpm()
        self.windowSeconds = rb.windowSeconds()
    }

    func intoRust() throws -> RustBridge.LlmRateLimitConfig {
        return RustBridge.LlmRateLimitConfig(self.rpm, self.tpm, self.windowSeconds)
    }
}
...
public struct MarkdownCodeBlock: Codable, Sendable, Hashable {
...
// MARK: - Internal FFI conversions for MarkdownCodeBlock
internal extension MarkdownCodeBlock {
    init(_ rb: RustBridge.MarkdownCodeBlockRef) throws {
        self.language = rb.language().toString()
        self.code = rb.code().toString()
    }

    func intoRust() throws -> RustBridge.MarkdownCodeBlock {
        return RustBridge.MarkdownCodeBlock(RustString(self.language), RustString(self.code))
    }
}

/// A link extracted from Markdown.
public struct MarkdownLink: Codable, Sendable, Hashable {
    /// Visible link text.
    public let text: String
    /// Link destination.
    public let url: String
...
// MARK: - Internal FFI conversions for MarkdownLink
internal extension MarkdownLink {
    init(_ rb: RustBridge.MarkdownLinkRef) throws {
        self.text = rb.text().toString()
        self.url = rb.url().toString()
    }

    func intoRust() throws -> RustBridge.MarkdownLink {
        return RustBridge.MarkdownLink(RustString(self.text), RustString(self.url))
    }
}
...
public struct MultidocThresholds: Codable, Sendable, Hashable {
    ...
    public init(densityShiftThreshold: Float = 0.3, bigramOverlapMin: Float = 0.1) {
...
public struct NgramRange: Codable, Sendable, Hashable {
    /// Minimum number of words in a candidate.
    public let min: UInt
    /// Maximum number of words in a candidate.
    public let max: UInt
    public init(min: UInt = 1, max: UInt = 3) {
        self.min = min
        self.max = max
    }
    private enum CodingKeys: String, CodingKey {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.min = try container.decodeIfPresent(UInt.self, forKey: .min) ?? 1
        self.max = try container.decodeIfPresent(UInt.self, forKey: .max) ?? 3
    }
}

// MARK: - Internal FFI conversions for NgramRange
internal extension NgramRange {
    init(_ rb: RustBridge.NgramRangeRef) throws {
        self.min = rb.min()
        self.max = rb.max()
    }

    func intoRust() throws -> RustBridge.NgramRange {
        return RustBridge.NgramRange(self.min, self.max)
    }
}
...
/// means.
public struct OcrBackendCapabilities: Codable, Sendable, Hashable {
    ...
    public init(name: String, supportedLanguages: [String]) {
        self.name = name
...
// MARK: - Internal FFI conversions for OcrBackendCapabilities
internal extension OcrBackendCapabilities {
    init(_ rb: RustBridge.OcrBackendCapabilitiesRef) throws {
        self.name = rb.name().toString()
        self.supportedLanguages = rb.supportedLanguages().map { $0.as_str().toString() }
    }

    func intoRust() throws -> RustBridge.OcrBackendCapabilities {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.ocrBackendCapabilitiesFromJson(json)
    }
}
...
public struct OcrElement: Codable, Sendable, Hashable {
    ...
    public let backendMetadata: [String: JSONValue]
    public init(text: String, geometry: OcrBoundingGeometry, confidence: OcrConfidence, level: OcrElementLevel, rotation: OcrRotation? = nil, pageNumber: UInt32 = 1, parentId: String? = nil, backendMetadata: [String: JSONValue]) {
        self.text = text
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.text = try container.decodeIfPresent(String.self, forKey: .text) ?? ""
        self.geometry = try container.decode(OcrBoundingGeometry.self, forKey: .geometry)
        self.confidence = try container.decode(OcrConfidence.self, forKey: .confidence)
        self.level = try container.decode(OcrElementLevel.self, forKey: .level)
        self.rotation = try container.decodeIfPresent(OcrRotation.self, forKey: .rotation) ?? nil
        self.pageNumber = try container.decodeIfPresent(UInt32.self, forKey: .pageNumber) ?? 1
        self.parentId = try container.decodeIfPresent(String.self, forKey: .parentId) ?? nil
        self.backendMetadata = try container.decodeIfPresent([String: JSONValue].self, forKey: .backendMetadata) ?? [:]
    }
}

// MARK: - Internal FFI conversions for OcrElement
internal extension OcrElement {
    init(_ rb: RustBridge.OcrElementRef) throws {
        self.text = rb.text().toString()
        self.geometry = try JSONDecoder().decode(OcrBoundingGeometry.self, from: ((rb.geometry().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.confidence = try OcrConfidence(rb.confidence())
        self.level = try { let rawValue = rb.level().toString(); guard let value = OcrElementLevel(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown OcrElementLevel variant", source: rawValue) }; return value }()
        self.rotation = try rb.rotation().map { try OcrRotation($0) }
        self.pageNumber = rb.pageNumber()
        self.parentId = rb.parentId()?.toString()
        self.backendMetadata = try JSONDecoder().decode([String: JSONValue].self, from: ((rb.backendMetadata().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
internal extension OcrElementConfig {
    init(_ rb: RustBridge.OcrElementConfigRef) throws {
        ...
        self.minLevel = try { let rawValue = rb.minLevel().toString(); guard let value = OcrElementLevel(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown OcrElementLevel variant", source: rawValue) }; return value }()
        self.minConfidence = rb.minConfidence()
...
public struct OcrExtractionResult: Codable, Sendable, Hashable {
    ...
    public init(content: String, mimeType: String, metadata: [String: JSONValue], tables: [OcrTable], ocrElements: [OcrElement]? = nil) {
        self.content = content
        self.mimeType = mimeType
        self.metadata = metadata
        self.tables = tables
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.tables = try container.decodeIfPresent([OcrTable].self, forKey: .tables) ?? []
        self.ocrElements = try container.decodeIfPresent([OcrElement].self, forKey: .ocrElements) ?? nil
    }
}

// MARK: - Internal FFI conversions for OcrExtractionResult
internal extension OcrExtractionResult {
    init(_ rb: RustBridge.OcrExtractionResultRef) throws {
        self.content = rb.content().toString()
        self.mimeType = rb.mimeType().toString()
        self.metadata = try JSONDecoder().decode([String: JSONValue].self, from: ((rb.metadata().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.tables = try rb.tables().map { (s: RustStringRef) -> OcrTable in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(OcrTable.self, from: d) }
        self.ocrElements = try JSONDecoder().decode([OcrElement]?.self, from: ((rb.ocrElements().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct OcrPoint: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.x = try container.decodeIfPresent(UInt32.self, forKey: .x) ?? 0
        self.y = try container.decodeIfPresent(UInt32.self, forKey: .y) ?? 0
    }
}

// MARK: - Internal FFI conversions for OcrPoint
internal extension OcrPoint {
    init(_ rb: RustBridge.OcrPointRef) throws {
        self.x = rb.x()
        self.y = rb.y()
    }

    func intoRust() throws -> RustBridge.OcrPoint {
        return RustBridge.OcrPoint(self.x, self.y)
    }
}
...
public struct OcrQualityThresholds: Codable, Sendable, Hashable {
    ...
    public let maxOcrOutputFragmentedWordRatio: Double
    ...
    public let minOcrMeanConfidence: Double
    ...
    public let minWordsForOcrOutputCheck: UInt
    ...
    public let maxOcrOutputDictInvalidWordRatio: Double
    ...
    public let discardSuspectedOcrNoise: Bool
    ...
    public let enablePlausibilityOcrRouting: Bool
    ...
    public let minReliableLanguageChunkRatio: Double
    public init(minTotalNonWhitespace: UInt = 64, minNonWhitespacePerPage: Double = 32.0, minMeaningfulWordLen: UInt = 4, minMeaningfulWords: UInt = 3, minAlnumRatio: Double = 0.3, minGarbageChars: UInt = 5, maxFragmentedWordRatio: Double = 0.6, criticalFragmentedWordRatio: Double = 0.8, maxOcrOutputFragmentedWordRatio: Double = 0.35, minOcrMeanConfidence: Double = 75.0, minWordsForOcrOutputCheck: UInt = 20, maxOcrOutputDictInvalidWordRatio: Double = 1.01, discardSuspectedOcrNoise: Bool = false, minAvgWordLength: Double = 2.0, minWordsForAvgLengthCheck: UInt = 50, minConsecutiveRepeatRatio: Double = 0.08, minWordsForRepeatCheck: UInt = 50, substantiveMinChars: UInt = 100, nonTextMinChars: UInt = 20, alnumWsRatioThreshold: Double = 0.4, pipelineMinQuality: Double = 0.5, minUndecodableRatio: Double = 0.5, enableProvenanceOcrRouting: Bool = true, minProvenanceFallbackRatio: Double = 0.5, enablePlausibilityOcrRouting: Bool = true, minReliableLanguageChunkRatio: Double = 0.1) {
        ...
        self.maxOcrOutputFragmentedWordRatio = maxOcrOutputFragmentedWordRatio
        self.minOcrMeanConfidence = minOcrMeanConfidence
        self.minWordsForOcrOutputCheck = minWordsForOcrOutputCheck
        self.maxOcrOutputDictInvalidWordRatio = maxOcrOutputDictInvalidWordRatio
        self.discardSuspectedOcrNoise = discardSuspectedOcrNoise
        ...
        self.enablePlausibilityOcrRouting = enablePlausibilityOcrRouting
        self.minReliableLanguageChunkRatio = minReliableLanguageChunkRatio
    }
    private enum CodingKeys: String, CodingKey {
        ...
        case maxOcrOutputFragmentedWordRatio = "max_ocr_output_fragmented_word_ratio"
        case minOcrMeanConfidence = "min_ocr_mean_confidence"
        case minWordsForOcrOutputCheck = "min_words_for_ocr_output_check"
        case maxOcrOutputDictInvalidWordRatio = "max_ocr_output_dict_invalid_word_ratio"
        case discardSuspectedOcrNoise = "discard_suspected_ocr_noise"
        ...
        case enablePlausibilityOcrRouting = "enable_plausibility_ocr_routing"
        case minReliableLanguageChunkRatio = "min_reliable_language_chunk_ratio"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        ...
        self.maxOcrOutputFragmentedWordRatio = try container.decodeIfPresent(Double.self, forKey: .maxOcrOutputFragmentedWordRatio) ?? 0.35
        self.minOcrMeanConfidence = try container.decodeIfPresent(Double.self, forKey: .minOcrMeanConfidence) ?? 75.0
        self.minWordsForOcrOutputCheck = try container.decodeIfPresent(UInt.self, forKey: .minWordsForOcrOutputCheck) ?? 20
        self.maxOcrOutputDictInvalidWordRatio = try container.decodeIfPresent(Double.self, forKey: .maxOcrOutputDictInvalidWordRatio) ?? 1.01
        self.discardSuspectedOcrNoise = try container.decodeIfPresent(Bool.self, forKey: .discardSuspectedOcrNoise) ?? false
        ...
        self.minUndecodableRatio = try container.decodeIfPresent(Double.self, forKey: .minUndecodableRatio) ?? 0.5
        self.enableProvenanceOcrRouting = try container.decodeIfPresent(Bool.self, forKey: .enableProvenanceOcrRouting) ?? true
        self.minProvenanceFallbackRatio = try container.decodeIfPresent(Double.self, forKey: .minProvenanceFallbackRatio) ?? 0.5
        self.enablePlausibilityOcrRouting = try container.decodeIfPresent(Bool.self, forKey: .enablePlausibilityOcrRouting) ?? true
        self.minReliableLanguageChunkRatio = try container.decodeIfPresent(Double.self, forKey: .minReliableLanguageChunkRatio) ?? 0.1
    }
}
...
internal extension OcrQualityThresholds {
    init(_ rb: RustBridge.OcrQualityThresholdsRef) throws {
        ...
        self.maxOcrOutputFragmentedWordRatio = rb.maxOcrOutputFragmentedWordRatio()
        self.minOcrMeanConfidence = rb.minOcrMeanConfidence()
        self.minWordsForOcrOutputCheck = rb.minWordsForOcrOutputCheck()
        self.maxOcrOutputDictInvalidWordRatio = rb.maxOcrOutputDictInvalidWordRatio()
        self.discardSuspectedOcrNoise = rb.discardSuspectedOcrNoise()
        ...
        self.enablePlausibilityOcrRouting = rb.enablePlausibilityOcrRouting()
        self.minReliableLanguageChunkRatio = rb.minReliableLanguageChunkRatio()
    }

    func intoRust() throws -> RustBridge.OcrQualityThresholds {
        return RustBridge.OcrQualityThresholds(self.minTotalNonWhitespace, self.minNonWhitespacePerPage, self.minMeaningfulWordLen, self.minMeaningfulWords, self.minAlnumRatio, self.minGarbageChars, self.maxFragmentedWordRatio, self.criticalFragmentedWordRatio, self.maxOcrOutputFragmentedWordRatio, self.minOcrMeanConfidence, self.minWordsForOcrOutputCheck, self.maxOcrOutputDictInvalidWordRatio, self.discardSuspectedOcrNoise, self.minAvgWordLength, self.minWordsForAvgLengthCheck, self.minConsecutiveRepeatRatio, self.minWordsForRepeatCheck, self.substantiveMinChars, self.nonTextMinChars, self.alnumWsRatioThreshold, self.pipelineMinQuality, self.minUndecodableRatio, self.enableProvenanceOcrRouting, self.minProvenanceFallbackRatio, self.enablePlausibilityOcrRouting, self.minReliableLanguageChunkRatio)
    }
}
...
public struct OcrRotation: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.angleDegrees = try container.decode(Double.self, forKey: .angleDegrees)
        self.confidence = try container.decodeIfPresent(Double.self, forKey: .confidence) ?? nil
    }
}
...
public struct OcrTable: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.cells = try container.decode([[String]].self, forKey: .cells)
        self.markdown = try container.decode(String.self, forKey: .markdown)
        self.pageNumber = try container.decode(UInt32.self, forKey: .pageNumber)
        self.boundingBox = try container.decodeIfPresent(OcrTableBoundingBox.self, forKey: .boundingBox) ?? nil
    }
}
...
public struct PaddleOcrVlBackendOptions: Codable, Sendable, Hashable {
    /// Optional per-call recognition task; the backend constructor's task is used when absent.
    public let task: PaddleOcrVlTaskKind?
    ...
    /// Optional per-call device override.
    public let device: CandleDevicePreference?
    public init(task: PaddleOcrVlTaskKind? = nil, modelPath: String? = nil, modelId: String? = nil, hfRevision: String? = nil, cacheDir: String? = nil, device: CandleDevicePreference? = nil) {
        self.task = task
        self.modelPath = modelPath
        self.modelId = modelId
        self.hfRevision = hfRevision
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.task = try container.decodeIfPresent(PaddleOcrVlTaskKind.self, forKey: .task) ?? nil
        self.modelPath = try container.decodeIfPresent(String.self, forKey: .modelPath) ?? nil
        self.modelId = try container.decodeIfPresent(String.self, forKey: .modelId) ?? nil
        self.hfRevision = try container.decodeIfPresent(String.self, forKey: .hfRevision) ?? nil
        self.cacheDir = try container.decodeIfPresent(String.self, forKey: .cacheDir) ?? nil
...
// MARK: - Internal FFI conversions for PaddleOcrVlBackendOptions
internal extension PaddleOcrVlBackendOptions {
    init(_ rb: RustBridge.PaddleOcrVlBackendOptionsRef) throws {
        self.task = rb.task().flatMap { PaddleOcrVlTaskKind(rawValue: $0.toString()) }
        self.modelPath = rb.modelPath()?.toString()
        self.modelId = rb.modelId()?.toString()
        self.hfRevision = rb.hfRevision()?.toString()
        self.cacheDir = rb.cacheDir()?.toString()
    ...
    func intoRust() throws -> RustBridge.PaddleOcrVlBackendOptions {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.paddleOcrVlBackendOptionsFromJson(json)
    }
}
...
public struct PageConfig: Codable, Sendable, Hashable {
    ...
    public init(extractPages: Bool = false, insertPageMarkers: Bool = false, markerFormat: String = "\n\n<!-- PAGE {page_num} -->\n\n") {
...
public struct PageContent: Codable, Sendable, Hashable {
    ...
    public let ocrConfidence: PageOcrConfidence?
    public init(pageNumber: UInt32, content: String, tables: [Table], imageIndices: [UInt32], imagePreprocessing: ImagePreprocessingMetadata? = nil, hierarchy: PageHierarchy? = nil, isBlank: Bool? = nil, layoutRegions: [LayoutRegion]? = nil, speakerNotes: String? = nil, sectionName: String? = nil, sheetName: String? = nil, ocrConfidence: PageOcrConfidence? = nil) {
        self.pageNumber = pageNumber
        self.content = content
        self.tables = tables
        self.imageIndices = imageIndices
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.pageNumber = try container.decode(UInt32.self, forKey: .pageNumber)
        self.content = try container.decode(String.self, forKey: .content)
        self.tables = try container.decodeIfPresent([Table].self, forKey: .tables) ?? []
        self.imageIndices = try container.decodeIfPresent([UInt32].self, forKey: .imageIndices) ?? []
        self.imagePreprocessing = try container.decodeIfPresent(ImagePreprocessingMetadata.self, forKey: .imagePreprocessing) ?? nil
        self.hierarchy = try container.decodeIfPresent(PageHierarchy.self, forKey: .hierarchy) ?? nil
        self.isBlank = try container.decodeIfPresent(Bool.self, forKey: .isBlank) ?? nil
        self.layoutRegions = try container.decodeIfPresent([LayoutRegion].self, forKey: .layoutRegions) ?? nil
        self.speakerNotes = try container.decodeIfPresent(String.self, forKey: .speakerNotes) ?? nil
        self.sectionName = try container.decodeIfPresent(String.self, forKey: .sectionName) ?? nil
        self.sheetName = try container.decodeIfPresent(String.self, forKey: .sheetName) ?? nil
        self.ocrConfidence = try container.decodeIfPresent(PageOcrConfidence.self, forKey: .ocrConfidence) ?? nil
    }
}
...
internal extension PageContent {
    init(_ rb: RustBridge.PageContentRef) throws {
        ...
        self.imageIndices = Array(rb.imageIndices())
        self.imagePreprocessing = try rb.imagePreprocessing().map { try ImagePreprocessingMetadata($0) }
        ...
        self.ocrConfidence = try rb.ocrConfidence().map { try PageOcrConfidence($0) }
    }
...
public struct PageHierarchy: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.blockCount = try container.decode(UInt32.self, forKey: .blockCount)
        self.blocks = try container.decodeIfPresent([HierarchicalBlock].self, forKey: .blocks) ?? []
    }
}
...
public struct PageInfo: Codable, Sendable, Hashable {
    ...
    public init(number: UInt32, title: String? = nil, dimensions: PageDimensions? = nil, imageCount: UInt32? = nil, tableCount: UInt32? = nil, hidden: Bool? = nil, isBlank: Bool? = nil, hasVectorGraphics: Bool) {
        self.number = number
        self.title = title
        self.dimensions = dimensions
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.number = try container.decode(UInt32.self, forKey: .number)
        self.title = try container.decodeIfPresent(String.self, forKey: .title) ?? nil
        self.dimensions = try container.decodeIfPresent(PageDimensions.self, forKey: .dimensions) ?? nil
        ...
        self.hidden = try container.decodeIfPresent(Bool.self, forKey: .hidden) ?? nil
        self.isBlank = try container.decodeIfPresent(Bool.self, forKey: .isBlank) ?? nil
        self.hasVectorGraphics = try container.decodeIfPresent(Bool.self, forKey: .hasVectorGraphics) ?? false
    }
}
...
public struct PageOcrConfidence: Codable, Sendable, Hashable {
    ...
    public let backend: String
    public init(score: Double? = nil, wordCount: UInt32, backend: String) {
        self.score = score
...
// MARK: - Internal FFI conversions for PageOcrConfidence
internal extension PageOcrConfidence {
    init(_ rb: RustBridge.PageOcrConfidenceRef) throws {
        self.score = rb.score()
        self.wordCount = rb.wordCount()
        self.backend = rb.backend().toString()
    }

    func intoRust() throws -> RustBridge.PageOcrConfidence {
        return RustBridge.PageOcrConfidence(self.score, self.wordCount, RustString(self.backend))
    }
}
...
public struct PageSpan: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.page = try container.decode(UInt32.self, forKey: .page)
        self.bbox = try container.decodeIfPresent(BoundingBox.self, forKey: .bbox) ?? nil
    }
}
...
public struct PageStructure: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.totalCount = try container.decode(UInt32.self, forKey: .totalCount)
        self.unitType = try container.decode(PageUnitType.self, forKey: .unitType)
        self.boundaries = try container.decodeIfPresent([PageBoundary].self, forKey: .boundaries) ?? nil
        self.pages = try container.decodeIfPresent([PageInfo].self, forKey: .pages) ?? nil
    }
}
...
internal extension PageStructure {
    init(_ rb: RustBridge.PageStructureRef) throws {
        ...
        self.unitType = try { let rawValue = rb.unitType().toString(); guard let value = PageUnitType(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown PageUnitType variant", source: rawValue) }; return value }()
...
public struct PdfAnnotation: Codable, Sendable, Hashable {
    ...
    public let quadPoints: [BoundingBox]?
    ...
    public init(annotationType: PdfAnnotationType, content: String? = nil, pageNumber: UInt32, boundingBox: BoundingBox? = nil, author: String? = nil, modified: String? = nil, color: String? = nil, subject: String? = nil, quadPoints: [BoundingBox]? = nil, markedText: String? = nil) {
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case markedText = "marked_text"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.annotationType = try container.decode(PdfAnnotationType.self, forKey: .annotationType)
        ...
        self.subject = try container.decodeIfPresent(String.self, forKey: .subject) ?? nil
        self.quadPoints = try container.decodeIfPresent([BoundingBox].self, forKey: .quadPoints) ?? nil
        self.markedText = try container.decodeIfPresent(String.self, forKey: .markedText) ?? nil
    }
}
...
internal extension PdfAnnotation {
    init(_ rb: RustBridge.PdfAnnotationRef) throws {
        self.annotationType = try { let rawValue = rb.annotationType().toString(); guard let value = PdfAnnotationType(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown PdfAnnotationType variant", source: rawValue) }; return value }()
        self.content = rb.content()?.toString()
        self.pageNumber = rb.pageNumber()
        self.boundingBox = try rb.boundingBox().map { try BoundingBox($0) }
        self.author = rb.author()?.toString()
        self.modified = rb.modified()?.toString()
        self.color = rb.color()?.toString()
        self.subject = rb.subject()?.toString()
        self.quadPoints = try JSONDecoder().decode([BoundingBox]?.self, from: ((rb.quadPoints().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.markedText = rb.markedText()?.toString()
    }
...
public struct PdfConfig: Codable, Sendable, Hashable {
    ...
    public init(extractImages: Bool = false, extractTables: Bool = true, passwords: [String]? = nil, extractMetadata: Bool = true, hierarchy: HierarchyConfig? = nil, extractAnnotations: Bool = false, topMarginFraction: Float? = nil, bottomMarginFraction: Float? = nil, allowSingleColumnTables: Bool = false, ocrInlineImages: Bool = false, extractFormFields: Bool = true, readingOrder: Bool = false, backend: PdfBackend) {
        self.extractImages = extractImages
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.backend = try container.decode(PdfBackend.self, forKey: .backend)
    }
    // MARK: - Instance Methods
    public func validate() throws  {
        let jsonSelf = try JSONEncoder().encode(self)
        let selfString = String(data: jsonSelf, encoding: .utf8) ?? "{}"
        _ = try RustBridge.pdfConfigValidateFromJson(selfString)
    }
...
internal extension PdfConfig {
    init(_ rb: RustBridge.PdfConfigRef) throws {
        ...
        self.passwords = try JSONDecoder().decode([String]?.self, from: ((rb.passwords().toString()).data(using: .utf8) ?? Data("null".utf8)))
        ...
        self.backend = try { let rawValue = rb.backend().toString(); guard let value = PdfBackend(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown PdfBackend variant", source: rawValue) }; return value }()
    }
...
public struct PdfFormField: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.fieldType = try container.decode(FormFieldType.self, forKey: .fieldType)
        self.value = try container.decodeIfPresent(String.self, forKey: .value) ?? nil
        self.defaultValue = try container.decodeIfPresent(String.self, forKey: .defaultValue) ?? nil
        self.flags = try container.decode(UInt32.self, forKey: .flags)
        self.page = try container.decodeIfPresent(UInt32.self, forKey: .page) ?? nil
        self.bbox = try container.decodeIfPresent(BoundingBox.self, forKey: .bbox) ?? nil
        self.maxLength = try container.decodeIfPresent(UInt32.self, forKey: .maxLength) ?? nil
        self.tooltip = try container.decodeIfPresent(String.self, forKey: .tooltip) ?? nil
    }
}
...
internal extension PdfFormField {
    init(_ rb: RustBridge.PdfFormFieldRef) throws {
        ...
        self.fieldType = try { let rawValue = rb.fieldType().toString(); guard let value = FormFieldType(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown FormFieldType variant", source: rawValue) }; return value }()
        self.value = rb.value()?.toString()
...
public struct PdfMetadata: Codable, Sendable, Hashable {
    ...
    public let fabricatedTextPages: [UInt32]?
    ...
    /// when no page qualifies.
    public let implausibleTextPages: [UInt32]?
    ...
    public let layoutGatedPages: [UInt32]?
    ...
    public let layoutGateReasons: [String]?
    public init(pdfVersion: String? = nil, producer: String? = nil, isEncrypted: Bool? = nil, width: Int64? = nil, height: Int64? = nil, pageCount: UInt32? = nil, scannedConfidence: Float? = nil, scannedPages: [UInt32]? = nil, fabricatedTextPages: [UInt32]? = nil, implausibleTextPages: [UInt32]? = nil, layoutGatedPages: [UInt32]? = nil, layoutGateReasons: [String]? = nil) {
        ...
        self.fabricatedTextPages = fabricatedTextPages
        self.implausibleTextPages = implausibleTextPages
        self.layoutGatedPages = layoutGatedPages
        self.layoutGateReasons = layoutGateReasons
    }
    private enum CodingKeys: String, CodingKey {
        ...
        case fabricatedTextPages = "fabricated_text_pages"
        case implausibleTextPages = "implausible_text_pages"
        case layoutGatedPages = "layout_gated_pages"
        case layoutGateReasons = "layout_gate_reasons"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        ...
        self.fabricatedTextPages = try container.decodeIfPresent([UInt32].self, forKey: .fabricatedTextPages) ?? nil
        self.implausibleTextPages = try container.decodeIfPresent([UInt32].self, forKey: .implausibleTextPages) ?? nil
        self.layoutGatedPages = try container.decodeIfPresent([UInt32].self, forKey: .layoutGatedPages) ?? nil
        self.layoutGateReasons = try container.decodeIfPresent([String].self, forKey: .layoutGateReasons) ?? nil
    }
}
...
internal extension PdfMetadata {
    init(_ rb: RustBridge.PdfMetadataRef) throws {
        ...
        self.scannedPages = try JSONDecoder().decode([UInt32]?.self, from: ((rb.scannedPages().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.fabricatedTextPages = try JSONDecoder().decode([UInt32]?.self, from: ((rb.fabricatedTextPages().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.implausibleTextPages = try JSONDecoder().decode([UInt32]?.self, from: ((rb.implausibleTextPages().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.layoutGatedPages = try JSONDecoder().decode([UInt32]?.self, from: ((rb.layoutGatedPages().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.layoutGateReasons = try JSONDecoder().decode([String]?.self, from: ((rb.layoutGateReasons().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct PixelDimensions: Codable, Sendable, Hashable {
    /// Width in pixels.
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.width = try container.decodeIfPresent(UInt.self, forKey: .width) ?? 0
        self.height = try container.decodeIfPresent(UInt.self, forKey: .height) ?? 0
    }
}

// MARK: - Internal FFI conversions for PixelDimensions
internal extension PixelDimensions {
    init(_ rb: RustBridge.PixelDimensionsRef) throws {
        self.width = rb.width()
        self.height = rb.height()
    }

    func intoRust() throws -> RustBridge.PixelDimensions {
        return RustBridge.PixelDimensions(self.width, self.height)
    }
}
...
public struct PostProcessorConfig: Codable, Sendable, Hashable {
    ...
    public init(enabled: Bool = true, enabledProcessors: [String]? = nil, disabledProcessors: [String]? = nil, enabledSet: [String]? = nil, disabledSet: [String]? = nil) {
        self.enabled = enabled
...
internal extension PostProcessorConfig {
    init(_ rb: RustBridge.PostProcessorConfigRef) throws {
        self.enabled = rb.enabled()
        self.enabledProcessors = try JSONDecoder().decode([String]?.self, from: ((rb.enabledProcessors().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.disabledProcessors = try JSONDecoder().decode([String]?.self, from: ((rb.disabledProcessors().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.enabledSet = try JSONDecoder().decode([String]?.self, from: ((rb.enabledSet().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.disabledSet = try JSONDecoder().decode([String]?.self, from: ((rb.disabledSet().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct PresentationHyperlink: Codable, Sendable, Hashable {
    /// Link destination.
    public let url: String
...
// MARK: - Internal FFI conversions for PresentationHyperlink
internal extension PresentationHyperlink {
    init(_ rb: RustBridge.PresentationHyperlinkRef) throws {
        self.url = rb.url().toString()
        self.label = rb.label()?.toString()
    }

    func intoRust() throws -> RustBridge.PresentationHyperlink {
        return RustBridge.PresentationHyperlink(RustString(self.url), self.label.map(RustString.init))
    }
}
...
internal extension PresetSummary {
    init(_ rb: RustBridge.PresetSummaryRef) throws {
        ...
        self.description = rb.description().toString()
        self.category = try { let rawValue = rb.category().toString(); guard let value = PresetCategory(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown PresetCategory variant", source: rawValue) }; return value }()
        self.tags = rb.tags().map { $0.as_str().toString() }
        self.preferredCallMode = try { let rawValue = rb.preferredCallMode().toString(); guard let value = CallMode(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown CallMode variant", source: rawValue) }; return value }()
...
public struct QrCode: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.confidence = try container.decodeIfPresent(Float.self, forKey: .confidence) ?? nil
        self.bbox = try container.decodeIfPresent(QrBoundingBox.self, forKey: .bbox) ?? nil
    }
}
...
public struct RakeParams: Codable, Sendable, Hashable {
    ...
    public init(minWordLength: UInt = 1, maxWordsPerPhrase: UInt = 3) {
...
internal extension RedactionFinding {
    init(_ rb: RustBridge.RedactionFindingRef) throws {
        ...
        self.end = rb.end()
        self.category = try JSONDecoder().decode(PiiCategory.self, from: ((rb.category().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.strategy = try { let rawValue = rb.strategy().toString(); guard let value = RedactionStrategy(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown RedactionStrategy variant", source: rawValue) }; return value }()
...
public struct RedactionPattern: Codable, Sendable, Hashable {
    ...
    public init(label: String, pattern: String, caseSensitive: Bool = false) {
        self.label = label
...
public struct RedactionTerm: Codable, Sendable, Hashable {
    ...
    public let caseSensitive: Bool
    public init(label: String, value: String, caseSensitive: Bool = false) {
        self.label = label
        self.value = value
        self.caseSensitive = caseSensitive
    }
...
public struct SecurityLimits: Codable, Sendable, Hashable {
    ...
    public init(maxArchiveSize: UInt = 524288000, maxCompressionRatio: UInt = 100, maxFilesInArchive: UInt = 10000, maxNestingDepth: UInt = 1024, maxEntityLength: UInt = 1048576, maxContentSize: UInt = 104857600, maxIterations: UInt = 10000000, maxXmlDepth: UInt = 1024, maxTableCells: UInt = 100000, maxPages: UInt? = nil) {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.maxPages = try container.decodeIfPresent(UInt.self, forKey: .maxPages) ?? nil
    }
}
...
internal extension SecurityLimits {
    ...
    func intoRust() throws -> RustBridge.SecurityLimits {
        return RustBridge.SecurityLimits(self.maxArchiveSize, self.maxCompressionRatio, self.maxFilesInArchive, self.maxNestingDepth, self.maxEntityLength, self.maxContentSize, self.maxIterations, self.maxXmlDepth, self.maxTableCells, self.maxPages)
    }
}
...
public struct ServerConfig: Codable, Sendable, Hashable {
    ...
    public let jobTimeoutSecs: UInt64
    public init(host: String = "127.0.0.1", port: UInt16 = 8000, corsOrigins: [String], maxRequestBodyBytes: UInt = 104857600, maxMultipartFieldBytes: UInt = 104857600, jobTimeoutSecs: UInt64 = 600) {
        self.host = host
        self.port = port
        self.corsOrigins = corsOrigins
        self.maxRequestBodyBytes = maxRequestBodyBytes
        self.maxMultipartFieldBytes = maxMultipartFieldBytes
        self.jobTimeoutSecs = jobTimeoutSecs
    }
    private enum CodingKeys: String, CodingKey {
        case host = "host"
        case port = "port"
        ...
        case jobTimeoutSecs = "job_timeout_secs"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.host = try container.decodeIfPresent(String.self, forKey: .host) ?? "127.0.0.1"
        self.port = try container.decodeIfPresent(UInt16.self, forKey: .port) ?? 8000
        self.corsOrigins = try container.decodeIfPresent([String].self, forKey: .corsOrigins) ?? []
        self.maxRequestBodyBytes = try container.decodeIfPresent(UInt.self, forKey: .maxRequestBodyBytes) ?? 104857600
        self.maxMultipartFieldBytes = try container.decodeIfPresent(UInt.self, forKey: .maxMultipartFieldBytes) ?? 104857600
        self.jobTimeoutSecs = try container.decodeIfPresent(UInt64.self, forKey: .jobTimeoutSecs) ?? 600
    }
    // MARK: - Instance Methods
...
internal extension ServerConfig {
    init(_ rb: RustBridge.ServerConfigRef) throws {
        ...
        self.jobTimeoutSecs = rb.jobTimeoutSecs()
    }
    ...
    func intoRust() throws -> RustBridge.ServerConfig {
        ...
        return RustBridge.ServerConfig(RustString(self.host), self.port, __corsOrigins, self.maxRequestBodyBytes, self.maxMultipartFieldBytes, self.jobTimeoutSecs)
    }
}
...
public struct SsrfPolicy: Codable, Sendable, Hashable {
    ...
    public let allowlist: [HostMatcher]
    ...
    public let schemeAllowlist: [String]
    public init(denyPrivate: Bool = true, allowlist: [HostMatcher], maxRedirects: UInt8 = 5, schemeAllowlist: [String] = ["http", "https"]) {
        self.denyPrivate = denyPrivate
        self.allowlist = allowlist
        self.maxRedirects = maxRedirects
        self.schemeAllowlist = schemeAllowlist
    }
    private enum CodingKeys: String, CodingKey {
        case denyPrivate = "deny_private"
        case allowlist = "allowlist"
        case maxRedirects = "max_redirects"
        case schemeAllowlist = "scheme_allowlist"
    }
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.denyPrivate = try container.decodeIfPresent(Bool.self, forKey: .denyPrivate) ?? true
        self.allowlist = try container.decodeIfPresent([HostMatcher].self, forKey: .allowlist) ?? []
        self.maxRedirects = try container.decodeIfPresent(UInt8.self, forKey: .maxRedirects) ?? 5
        self.schemeAllowlist = try container.decodeIfPresent([String].self, forKey: .schemeAllowlist) ?? ["http", "https"]
    }
}
...
internal extension SsrfPolicy {
    init(_ rb: RustBridge.SsrfPolicyRef) throws {
        ...
        self.allowlist = try rb.allowlist().map { (s: RustStringRef) -> HostMatcher in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(HostMatcher.self, from: d) }
        self.maxRedirects = rb.maxRedirects()
        self.schemeAllowlist = rb.schemeAllowlist().map { $0.as_str().toString() }
    }

    func intoRust() throws -> RustBridge.SsrfPolicy {
        let __allowlist = RustVec<RustBridge.HostMatcher>()
        for __elem in self.allowlist { __allowlist.push(value: try __elem.intoRust()) }
        let __schemeAllowlist = RustVec<RustString>()
        for __elem in self.schemeAllowlist { __schemeAllowlist.push(value: RustString(__elem)) }
        return RustBridge.SsrfPolicy(self.denyPrivate, __allowlist, self.maxRedirects, __schemeAllowlist)
    }
}
...
public struct StructuredData: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.dataType = try container.decode(StructuredDataType.self, forKey: .dataType)
        self.rawJson = try container.decode(String.self, forKey: .rawJson)
        self.schemaType = try container.decodeIfPresent(String.self, forKey: .schemaType) ?? nil
    }
}
...
internal extension StructuredData {
    init(_ rb: RustBridge.StructuredDataRef) throws {
        self.dataType = try { let rawValue = rb.dataType().toString(); guard let value = StructuredDataType(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown StructuredDataType variant", source: rawValue) }; return value }()
...
public struct SvgOptions: Codable, Sendable, Hashable {
    ...
    public init(sanitize: Bool = true, renderDpi: Float = 96.0) {
...
public struct Table: Codable, Sendable, Hashable {
    ...
    public let cellStyles: [TableCellStyle]
    ...
    public init(cells: [[String]], markdown: String, pageNumber: UInt32, boundingBox: BoundingBox? = nil, tableId: String? = nil, cellStyles: [TableCellStyle], columns: [String]? = nil) {
        self.cells = cells
        self.markdown = markdown
        self.pageNumber = pageNumber
        self.boundingBox = boundingBox
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.cellStyles = try container.decodeIfPresent([TableCellStyle].self, forKey: .cellStyles) ?? []
...
internal extension Table {
    init(_ rb: RustBridge.TableRef) throws {
        ...
        self.cellStyles = try rb.cellStyles().map { (s: RustStringRef) -> TableCellStyle in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(TableCellStyle.self, from: d) }
        self.columns = try JSONDecoder().decode([String]?.self, from: ((rb.columns().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct TableCell: Codable, Sendable, Hashable {
    ...
    /// Whether this is a header cell
    public let isHeader: Bool
    public init(content: String, rowSpan: UInt32 = 1, colSpan: UInt32 = 1, isHeader: Bool) {
        self.content = content
        self.rowSpan = rowSpan
        self.colSpan = colSpan
        self.isHeader = isHeader
...
public struct TableCellStyle: Codable, Sendable, Hashable {
    ...
    public let styleName: String?
    public init(row: UInt32, col: UInt32, headingLevel: UInt8? = nil, styleName: String? = nil) {
        self.row = row
        self.col = col
        self.headingLevel = headingLevel
        self.styleName = styleName
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.row = try container.decodeIfPresent(UInt32.self, forKey: .row) ?? 0
        self.col = try container.decodeIfPresent(UInt32.self, forKey: .col) ?? 0
        self.headingLevel = try container.decodeIfPresent(UInt8.self, forKey: .headingLevel) ?? nil
        self.styleName = try container.decodeIfPresent(String.self, forKey: .styleName) ?? nil
    }
}

// MARK: - Internal FFI conversions for TableCellStyle
internal extension TableCellStyle {
    init(_ rb: RustBridge.TableCellStyleRef) throws {
        self.row = rb.row()
        self.col = rb.col()
        self.headingLevel = rb.headingLevel()
        self.styleName = rb.styleName()?.toString()
    ...
    func intoRust() throws -> RustBridge.TableCellStyle {
        return RustBridge.TableCellStyle(self.row, self.col, self.headingLevel, self.styleName.map(RustString.init))
    }
}
...
public struct TesseractConfig: Codable, Sendable, Hashable {
    ...
    public let thresholdingMethod: String
    public init(language: [String] = ["eng"], psm: Int32? = nil, outputFormat: String = "markdown", oem: Int32 = 3, minConfidence: Double = 0.0, preprocessing: ImagePreprocessingConfig? = nil, enableTableDetection: Bool = true, tableMinConfidence: Double = 0.0, tableColumnThreshold: Int32 = 50, tableRowThresholdRatio: Double = 0.5, useCache: Bool = true, classifyUsePreAdaptedTemplates: Bool = true, languageModelNgramOn: Bool = true, tesseditDontBlkrejGoodWds: Bool = true, tesseditDontRowrejGoodWds: Bool = true, tesseditEnableDictCorrection: Bool = true, tesseditCharWhitelist: String = "", tesseditCharBlacklist: String = "", tesseditUsePrimaryParamsModel: Bool = true, textordSpaceSizeIsVariable: Bool = true, thresholdingMethod: String = "otsu") {
        self.language = language
        self.psm = psm
        self.outputFormat = outputFormat
        self.oem = oem
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.psm = try container.decodeIfPresent(Int32.self, forKey: .psm) ?? nil
        self.outputFormat = try container.decodeIfPresent(String.self, forKey: .outputFormat) ?? "markdown"
        ...
        self.languageModelNgramOn = try container.decodeIfPresent(Bool.self, forKey: .languageModelNgramOn) ?? true
        ...
        self.thresholdingMethod = try container.decodeIfPresent(String.self, forKey: .thresholdingMethod) ?? "otsu"
    }
}
...
internal extension TesseractConfig {
    init(_ rb: RustBridge.TesseractConfigRef) throws {
        ...
        self.thresholdingMethod = rb.thresholdingMethod().toString()
    }
...
public struct TextExtractionResult: Codable, Sendable, Hashable {
    ...
    public init(content: String, lineCount: UInt, wordCount: UInt, characterCount: UInt, headers: [String]? = nil, links: [MarkdownLink]? = nil, codeBlocks: [MarkdownCodeBlock]? = nil) {
        self.content = content
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.content = try container.decode(String.self, forKey: .content)
        self.lineCount = try container.decode(UInt.self, forKey: .lineCount)
        self.wordCount = try container.decode(UInt.self, forKey: .wordCount)
        self.characterCount = try container.decode(UInt.self, forKey: .characterCount)
        self.headers = try container.decodeIfPresent([String].self, forKey: .headers) ?? nil
        self.links = try container.decodeIfPresent([MarkdownLink].self, forKey: .links) ?? nil
        self.codeBlocks = try container.decodeIfPresent([MarkdownCodeBlock].self, forKey: .codeBlocks) ?? nil
    }
}
...
internal extension TextExtractionResult {
    init(_ rb: RustBridge.TextExtractionResultRef) throws {
        ...
        self.headers = try JSONDecoder().decode([String]?.self, from: ((rb.headers().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.links = try JSONDecoder().decode([MarkdownLink]?.self, from: ((rb.links().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.codeBlocks = try JSONDecoder().decode([MarkdownCodeBlock]?.self, from: ((rb.codeBlocks().toString()).data(using: .utf8) ?? Data("null".utf8)))
    }
...
public struct TextMetadata: Codable, Sendable, Hashable {
    ...
    public let codeBlocks: [MarkdownCodeBlock]?
    public init(lineCount: UInt32, wordCount: UInt32, characterCount: UInt32, headers: [String]? = nil, links: [MarkdownLink]? = nil, codeBlocks: [MarkdownCodeBlock]? = nil) {
        self.lineCount = lineCount
        self.wordCount = wordCount
        self.characterCount = characterCount
        self.headers = headers
...
public struct TokenReductionConfig: Codable, Sendable, Hashable {
    ...
    public init(level: ReductionLevel, languageHint: String? = nil, preserveMarkdown: Bool = false, preserveCode: Bool = true, semanticThreshold: Float = 0.3, enableParallel: Bool = true, useSimd: Bool = true, customStopwords: [String: [String]]? = nil, preservePatterns: [String], targetReduction: Float? = nil, enableSemanticClustering: Bool = false, preserveImportantWords: Bool = true) {
        self.level = level
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.level = try container.decode(ReductionLevel.self, forKey: .level)
        self.languageHint = try container.decodeIfPresent(String.self, forKey: .languageHint) ?? nil
        self.preserveMarkdown = try container.decodeIfPresent(Bool.self, forKey: .preserveMarkdown) ?? false
        self.preserveCode = try container.decodeIfPresent(Bool.self, forKey: .preserveCode) ?? true
        self.semanticThreshold = try container.decodeIfPresent(Float.self, forKey: .semanticThreshold) ?? 0.3
        self.enableParallel = try container.decodeIfPresent(Bool.self, forKey: .enableParallel) ?? true
        self.useSimd = try container.decodeIfPresent(Bool.self, forKey: .useSimd) ?? true
        self.customStopwords = try container.decodeIfPresent([String: [String]].self, forKey: .customStopwords) ?? nil
        self.preservePatterns = try container.decodeIfPresent([String].self, forKey: .preservePatterns) ?? []
        self.targetReduction = try container.decodeIfPresent(Float.self, forKey: .targetReduction) ?? nil
        self.enableSemanticClustering = try container.decodeIfPresent(Bool.self, forKey: .enableSemanticClustering) ?? false
...
internal extension TokenReductionConfig {
    init(_ rb: RustBridge.TokenReductionConfigRef) throws {
        self.level = try { let rawValue = rb.level().toString(); guard let value = ReductionLevel(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown ReductionLevel variant", source: rawValue) }; return value }()
        ...
        self.customStopwords = try JSONDecoder().decode([String: [String]]?.self, from: ((rb.customStopwords().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.preservePatterns = rb.preservePatterns().map { $0.as_str().toString() }
...
public struct TokenReductionOptions: Codable, Sendable, Hashable {
    ...
    public let preserveImportantWords: Bool
    public init(mode: String = "off", preserveImportantWords: Bool = true) {
        self.mode = mode
        self.preserveImportantWords = preserveImportantWords
    }
    private enum CodingKeys: String, CodingKey {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.mode = try container.decodeIfPresent(String.self, forKey: .mode) ?? "off"
        self.preserveImportantWords = try container.decodeIfPresent(Bool.self, forKey: .preserveImportantWords) ?? true
    }
}
...
public struct Translation: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.sourceLang = try container.decodeIfPresent(String.self, forKey: .sourceLang) ?? nil
        self.content = try container.decode(String.self, forKey: .content)
        self.formattedContent = try container.decodeIfPresent(String.self, forKey: .formattedContent) ?? nil
    }
}
...
public struct TreeSitterProcessConfig: Codable, Sendable, Hashable {
    ...
    public init(structure: Bool = true, imports: Bool = true, exports: Bool = true, comments: Bool = false, docstrings: Bool = false, symbols: Bool = false, diagnostics: Bool = false, dataExtraction: Bool = false, chunkMaxSize: UInt? = nil, contentMode: CodeContentMode) {
...
internal extension TreeSitterProcessConfig {
    init(_ rb: RustBridge.TreeSitterProcessConfigRef) throws {
        ...
        self.contentMode = try { let rawValue = rb.contentMode().toString(); guard let value = CodeContentMode(rawValue: rawValue) else { throw HeuristicsError.validation(message: "Unknown CodeContentMode variant", source: rawValue) }; return value }()
    }
...
public struct TrocrBackendOptions: Codable, Sendable, Hashable {
    /// Optional model variant; the backend constructor's variant is used when absent.
    public let variant: CandleTrocrVariant?
    /// Optional per-call device override.
    public let device: CandleDevicePreference?
    /// Optional Hugging Face cache root.
    public let cacheDir: String?
    /// Optional immutable Hugging Face model revision.
    public let hfRevision: String?
    public init(variant: CandleTrocrVariant? = nil, device: CandleDevicePreference? = nil, cacheDir: String? = nil, hfRevision: String? = nil) {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.variant = try container.decodeIfPresent(CandleTrocrVariant.self, forKey: .variant) ?? nil
        self.device = try container.decodeIfPresent(CandleDevicePreference.self, forKey: .device) ?? nil
        self.cacheDir = try container.decodeIfPresent(String.self, forKey: .cacheDir) ?? nil
        self.hfRevision = try container.decodeIfPresent(String.self, forKey: .hfRevision) ?? nil
    }
...
// MARK: - Internal FFI conversions for TrocrBackendOptions
internal extension TrocrBackendOptions {
    init(_ rb: RustBridge.TrocrBackendOptionsRef) throws {
        self.variant = rb.variant().flatMap { CandleTrocrVariant(rawValue: $0.toString()) }
        self.device = rb.device().flatMap { CandleDevicePreference(rawValue: $0.toString()) }
        self.cacheDir = rb.cacheDir()?.toString()
        self.hfRevision = rb.hfRevision()?.toString()
    }

    func intoRust() throws -> RustBridge.TrocrBackendOptions {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "{}"
        return try RustBridge.trocrBackendOptionsFromJson(json)
    }
}
...
public struct YearRange: Codable, Sendable, Hashable {
    ...
    public init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.min = try container.decodeIfPresent(UInt32.self, forKey: .min) ?? nil
        self.max = try container.decodeIfPresent(UInt32.self, forKey: .max) ?? nil
        self.years = try container.decodeIfPresent([UInt32].self, forKey: .years) ?? []
    }
}
...
public typealias AttributesRef = RustBridge.AttributesRef

public typealias AttributesRefMut = RustBridge.AttributesRefMut
...
public typealias BedrockConfigRef = RustBridge.BedrockConfigRef

public typealias BedrockConfigRefMut = RustBridge.BedrockConfigRefMut

public typealias BibtexMetadataRef = RustBridge.BibtexMetadataRef

public typealias BibtexMetadataRefMut = RustBridge.BibtexMetadataRefMut
...
public typealias CodeDataNodeRef = RustBridge.CodeDataNodeRef

public typealias CodeDataNodeRefMut = RustBridge.CodeDataNodeRefMut
...
public typealias ConcurrencyConfigRef = RustBridge.ConcurrencyConfigRef

public typealias ConcurrencyConfigRefMut = RustBridge.ConcurrencyConfigRefMut
...
public typealias CsvConfigRef = RustBridge.CsvConfigRef

public typealias CsvConfigRefMut = RustBridge.CsvConfigRefMut
...
public typealias DeepseekOcrBackendOptionsRef = RustBridge.DeepseekOcrBackendOptionsRef

public typealias DeepseekOcrBackendOptionsRefMut = RustBridge.DeepseekOcrBackendOptionsRefMut
...
public typealias DjotAttributeGroupRef = RustBridge.DjotAttributeGroupRef

public typealias DjotAttributeGroupRefMut = RustBridge.DjotAttributeGroupRefMut
...
public typealias DoctorCheckRef = RustBridge.DoctorCheckRef

public typealias DoctorCheckRefMut = RustBridge.DoctorCheckRefMut

public typealias DoctorReportRef = RustBridge.DoctorReportRef

public typealias DoctorReportRefMut = RustBridge.DoctorReportRefMut
...
public typealias DocumentNodeRef = RustBridge.DocumentNodeRef

public typealias DocumentNodeRefMut = RustBridge.DocumentNodeRefMut
...
public typealias DocumentStructureRef = RustBridge.DocumentStructureRef

public typealias DocumentStructureRefMut = RustBridge.DocumentStructureRefMut
...
public typealias DocxMetadataRef = RustBridge.DocxMetadataRef

public typealias DocxMetadataRefMut = RustBridge.DocxMetadataRefMut

public typealias ElementRef = RustBridge.ElementRef

public typealias ElementRefMut = RustBridge.ElementRefMut

public typealias ElementMetadataRef = RustBridge.ElementMetadataRef

public typealias ElementMetadataRefMut = RustBridge.ElementMetadataRefMut
...
public typealias ExcelWorkbookRef = RustBridge.ExcelWorkbookRef

public typealias ExcelWorkbookRefMut = RustBridge.ExcelWorkbookRefMut
...
public typealias FootnoteRef = RustBridge.FootnoteRef

public typealias FootnoteRefMut = RustBridge.FootnoteRefMut
...
public typealias FormattedBlockRef = RustBridge.FormattedBlockRef

public typealias FormattedBlockRefMut = RustBridge.FormattedBlockRefMut

public typealias FormulaRef = RustBridge.FormulaRef

public typealias FormulaRefMut = RustBridge.FormulaRefMut

public typealias GeoJsonExtractionConfigRef = RustBridge.GeoJsonExtractionConfigRef

public typealias GeoJsonExtractionConfigRefMut = RustBridge.GeoJsonExtractionConfigRefMut

public typealias GlmOcrBackendOptionsRef = RustBridge.GlmOcrBackendOptionsRef

public typealias GlmOcrBackendOptionsRefMut = RustBridge.GlmOcrBackendOptionsRefMut
...
public typealias HierarchicalBoundingBoxRef = RustBridge.HierarchicalBoundingBoxRef

public typealias HierarchicalBoundingBoxRefMut = RustBridge.HierarchicalBoundingBoxRefMut
...
public typealias HtmlMetadataRef = RustBridge.HtmlMetadataRef

public typealias HtmlMetadataRefMut = RustBridge.HtmlMetadataRefMut
...
public typealias ImageDpiRef = RustBridge.ImageDpiRef

public typealias ImageDpiRefMut = RustBridge.ImageDpiRefMut
...
public typealias ImageMetadataRef = RustBridge.ImageMetadataRef

public typealias ImageMetadataRefMut = RustBridge.ImageMetadataRefMut
...
public typealias InlineElementRef = RustBridge.InlineElementRef

public typealias InlineElementRefMut = RustBridge.InlineElementRefMut

public typealias JatsMetadataRef = RustBridge.JatsMetadataRef

public typealias JatsMetadataRefMut = RustBridge.JatsMetadataRefMut

public typealias KeyValueAttributeRef = RustBridge.KeyValueAttributeRef

public typealias KeyValueAttributeRefMut = RustBridge.KeyValueAttributeRefMut
...
public typealias LanguageConfidenceRef = RustBridge.LanguageConfidenceRef

public typealias LanguageConfidenceRefMut = RustBridge.LanguageConfidenceRefMut
...
public typealias LlmBudgetConfigRef = RustBridge.LlmBudgetConfigRef

public typealias LlmBudgetConfigRefMut = RustBridge.LlmBudgetConfigRefMut

public typealias LlmCacheConfigRef = RustBridge.LlmCacheConfigRef

public typealias LlmCacheConfigRefMut = RustBridge.LlmCacheConfigRefMut

public typealias LlmProviderConfigRef = RustBridge.LlmProviderConfigRef

public typealias LlmProviderConfigRefMut = RustBridge.LlmProviderConfigRefMut

public typealias LlmRateLimitConfigRef = RustBridge.LlmRateLimitConfigRef

public typealias LlmRateLimitConfigRefMut = RustBridge.LlmRateLimitConfigRefMut
...
public typealias MarkdownCodeBlockRef = RustBridge.MarkdownCodeBlockRef

public typealias MarkdownCodeBlockRefMut = RustBridge.MarkdownCodeBlockRefMut

public typealias MarkdownLinkRef = RustBridge.MarkdownLinkRef

public typealias MarkdownLinkRefMut = RustBridge.MarkdownLinkRefMut
...
public typealias NgramRangeRef = RustBridge.NgramRangeRef

public typealias NgramRangeRefMut = RustBridge.NgramRangeRefMut

public typealias OcrBackendCapabilitiesRef = RustBridge.OcrBackendCapabilitiesRef

public typealias OcrBackendCapabilitiesRefMut = RustBridge.OcrBackendCapabilitiesRefMut
...
public typealias OcrElementRef = RustBridge.OcrElementRef

public typealias OcrElementRefMut = RustBridge.OcrElementRefMut
...
public typealias OcrExtractionResultRef = RustBridge.OcrExtractionResultRef

public typealias OcrExtractionResultRefMut = RustBridge.OcrExtractionResultRefMut

public typealias OcrMetadataRef = RustBridge.OcrMetadataRef

public typealias OcrMetadataRefMut = RustBridge.OcrMetadataRefMut

public typealias OcrPointRef = RustBridge.OcrPointRef

public typealias OcrPointRefMut = RustBridge.OcrPointRefMut
...
public typealias PaddleOcrVlBackendOptionsRef = RustBridge.PaddleOcrVlBackendOptionsRef

public typealias PaddleOcrVlBackendOptionsRefMut = RustBridge.PaddleOcrVlBackendOptionsRefMut
...
public typealias PageOcrConfidenceRef = RustBridge.PageOcrConfidenceRef

public typealias PageOcrConfidenceRefMut = RustBridge.PageOcrConfidenceRefMut
...
public typealias PixelDimensionsRef = RustBridge.PixelDimensionsRef

public typealias PixelDimensionsRefMut = RustBridge.PixelDimensionsRefMut
...
public typealias PresentationHyperlinkRef = RustBridge.PresentationHyperlinkRef

public typealias PresentationHyperlinkRefMut = RustBridge.PresentationHyperlinkRefMut
...
public typealias TableCellStyleRef = RustBridge.TableCellStyleRef

public typealias TableCellStyleRefMut = RustBridge.TableCellStyleRefMut
...
public typealias TokenReductionConfigRef = RustBridge.TokenReductionConfigRef

public typealias TokenReductionConfigRefMut = RustBridge.TokenReductionConfigRefMut
...
public typealias TrocrBackendOptionsRef = RustBridge.TrocrBackendOptionsRef

public typealias TrocrBackendOptionsRefMut = RustBridge.TrocrBackendOptionsRefMut
...
public enum AnnotationKind: Codable, Sendable, Hashable {
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let type = try container.decode(String.self, forKey: .annotationType)
        switch type {
        case "bold":
            self = .bold
        case "italic":
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        switch self {
        case .bold:
            try container.encode("bold", forKey: .annotationType)
        case .italic:
            try container.encode("italic", forKey: .annotationType)
        case .underline:
            try container.encode("underline", forKey: .annotationType)
        case .strikethrough:
            try container.encode("strikethrough", forKey: .annotationType)
        case .code:
            try container.encode("code", forKey: .annotationType)
        case .`subscript`:
            try container.encode("subscript", forKey: .annotationType)
        case .superscript:
            try container.encode("superscript", forKey: .annotationType)
        case .link(let url, let title):
            try container.encode("link", forKey: .annotationType)
            try container.encode(url, forKey: .url)
            try container.encodeIfPresent(title, forKey: .title)

        case .highlight:
            try container.encode("highlight", forKey: .annotationType)
        case .color(let value):
            try container.encode("color", forKey: .annotationType)
            try container.encode(value, forKey: .value)

        case .fontSize(let value):
            try container.encode("font_size", forKey: .annotationType)
            try container.encode(value, forKey: .value)

        case .custom(let name, let value):
            try container.encode("custom", forKey: .annotationType)
...
public enum CandleDeepseekOcrDtype: String, Codable, Sendable, Hashable {
    case auto
    case f32
    case f16
    case bf16
}
extension CandleDeepseekOcrDtype {
    func intoRust() throws -> RustBridge.CandleDeepseekOcrDtype {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.candleDeepseekOcrDtypeFromJson(json)
    }
}

/// Device selection shared by the typed candle backend option objects.
public enum CandleDevicePreference: String, Codable, Sendable, Hashable {
    case auto
    case cpu
    case cuda
    case metal
}
extension CandleDevicePreference {
    func intoRust() throws -> RustBridge.CandleDevicePreference {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.candleDevicePreferenceFromJson(json)
    }
}

/// TrOCR model variant accepted by `candle-trocr` backend options.
public enum CandleTrocrVariant: String, Codable, Sendable, Hashable {
    case basePrinted = "base-printed"
    case largePrinted = "large-printed"
    case baseHandwritten = "base-handwritten"
    case largeHandwritten = "large-handwritten"
}
extension CandleTrocrVariant {
    func intoRust() throws -> RustBridge.CandleTrocrVariant {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.candleTrocrVariantFromJson(json)
    }
}
...
public enum ConfidenceSemantics: Codable, Sendable, Hashable {
    /// Validated to track legibility on a known scale — usable as an absolute quality gate.
    case legibility(scaleMax: Double)
    ...
    public init(from decoder: Decoder) throws {
        ...
        case "Legibility":
            ...
            self = .legibility(
                scaleMax: try nested.decode(Double.self, forKey: __AlefExternalTagKey("scale_max"))
            )
        default:
            throw DecodingError.dataCorrupted(
    ...
    public func encode(to encoder: Encoder) throws {
        switch self {
        case .legibility(let scaleMax):
            var container = encoder.container(keyedBy: __AlefExternalTagKey.self)
            var nested = container.nestedContainer(keyedBy: __AlefExternalTagKey.self, forKey: __AlefExternalTagKey("Legibility"))
            try nested.encode(scaleMax, forKey: __AlefExternalTagKey("scale_max"))
        case .uncalibrated:
            var single = encoder.singleValueContainer()
            try single.encode("Uncalibrated")
        case .none:
            var single = encoder.singleValueContainer()
...
extension ConfidenceSemantics {
    func intoRust() throws -> RustBridge.ConfidenceSemantics {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.confidenceSemanticsFromJson(json)
    }
}

/// Content filter applied to each crawled page before it reaches the result.
public enum ContentFilterKind: String, Codable, Sendable, Hashable {
    /// Keep only pages scoring at or above `bm25_threshold` for `bm25_query`.
    case bm25
}
extension ContentFilterKind {
    func intoRust() throws -> RustBridge.ContentFilterKind {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.contentFilterKindFromJson(json)
    }
}
...
public enum CrawlStrategyKind: String, Codable, Sendable, Hashable {
    ...
    case bestFirst = "best_first"
    /// Like `BestFirst`, but stops once newly crawled pages stop contributing new terms.
    case adaptive
}
extension CrawlStrategyKind {
    func intoRust() throws -> RustBridge.CrawlStrategyKind {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.crawlStrategyKindFromJson(json)
    }
}
...
/// token itself.
public enum CredentialProviderConfig: Codable, Sendable, Hashable {
    /// Azure AD OAuth2 client-credentials flow (Azure OpenAI / Azure Cognitive Services).
    case azureAd(tenantId: String, clientId: String, clientSecret: String, scope: String?)
    ...
    /// follows.
    case vertexOauth2(serviceAccountKeyFile: String, scope: String?)
    ...
    case vertexAdc(scope: String?)
    /// AWS STS `AssumeRoleWithWebIdentity` (EKS IRSA / OIDC federation) for Bedrock.
    case bedrockWebIdentity(roleArn: String, tokenFile: String, sessionName: String?, region: String?)
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case scope
        case serviceAccountKeyFile = "service_account_key_file"
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch type {
        case "azure_ad":
            self = .azureAd(tenantId: try container.decode(String.self, forKey: .tenantId), clientId: try container.decode(String.self, forKey: .clientId), clientSecret: try container.decode(String.self, forKey: .clientSecret), scope: try container.decodeIfPresent(String.self, forKey: .scope))
        case "vertex_oauth2":
            self = .vertexOauth2(serviceAccountKeyFile: try container.decode(String.self, forKey: .serviceAccountKeyFile), scope: try container.decodeIfPresent(String.self, forKey: .scope))
        case "vertex_adc":
            self = .vertexAdc(scope: try container.decodeIfPresent(String.self, forKey: .scope))
        case "bedrock_web_identity":
            self = .bedrockWebIdentity(roleArn: try container.decode(String.self, forKey: .roleArn), tokenFile: try container.decode(String.self, forKey: .tokenFile), sessionName: try container.decodeIfPresent(String.self, forKey: .sessionName), region: try container.decodeIfPresent(String.self, forKey: .region))
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .type,
                in: container,
                debugDescription: "Unknown CredentialProviderConfig type: \(type)"
            )
        }
    }
    ...
    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        switch self {
        case .azureAd(let tenantId, let clientId, let clientSecret, let scope):
            try container.encode("azure_ad", forKey: .type)
            try container.encode(tenantId, forKey: .tenantId)
            try container.encode(clientId, forKey: .clientId)
            try container.encode(clientSecret, forKey: .clientSecret)
            try container.encodeIfPresent(scope, forKey: .scope)

        case .vertexOauth2(let serviceAccountKeyFile, let scope):
            try container.encode("vertex_oauth2", forKey: .type)
            try container.encode(serviceAccountKeyFile, forKey: .serviceAccountKeyFile)
            try container.encodeIfPresent(scope, forKey: .scope)

        case .vertexAdc(let scope):
            try container.encode("vertex_adc", forKey: .type)
            try container.encodeIfPresent(scope, forKey: .scope)

        case .bedrockWebIdentity(let roleArn, let tokenFile, let sessionName, let region):
            try container.encode("bedrock_web_identity", forKey: .type)
            try container.encode(roleArn, forKey: .roleArn)
            try container.encode(tokenFile, forKey: .tokenFile)
...
extension CredentialProviderConfig {
    /// Returns the serde wire tag identifying this variant -- the same value
    /// the pre-promotion opaque binding's `to_string()` returned -- dropping
    /// any associated payload.
    public func toString() -> String {
        ...
        case .azureAd:
            return "azure_ad"
        case .vertexOauth2:
            return "vertex_oauth2"
        case .vertexAdc:
            return "vertex_adc"
        case .bedrockWebIdentity:
            return "bedrock_web_identity"
        }
    }
}
extension CredentialProviderConfig {
    func intoRust() throws -> RustBridge.CredentialProviderConfig {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.credentialProviderConfigFromJson(json)
    }
}
...
public enum DiffLine: Codable, Sendable, Hashable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch tag {
        case "context":
            self = .context(field0: try container.decode(String.self, forKey: .text))
        case "added":
            self = .added(field0: try container.decode(String.self, forKey: .text))
        case "removed":
            self = .removed(field0: try container.decode(String.self, forKey: .text))
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .kind,
                in: container,
                debugDescription: "Unknown DiffLine tag: \(tag)"
            )
        }
    }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .context(let field0):
            ...
            try container.encode(field0, forKey: .text)
...
public enum DocumentContentEncoding: String, Codable, Sendable, Hashable {
    /// Populate `DownloadedDocument.content_base64` with a base64-encoded copy.
    case base64
}
extension DocumentContentEncoding {
    func intoRust() throws -> RustBridge.DocumentContentEncoding {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.documentContentEncodingFromJson(json)
    }
}
...
public enum EntityCategory: Codable, Sendable, Hashable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch key.stringValue {
        case "custom":
            self = .custom(field0: try container.decode(String.self, forKey: key))
        default:
            throw DecodingError.dataCorrupted(
                .init(codingPath: decoder.codingPath, debugDescription: "unknown variant key \(key.stringValue)"))
        }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .person:
            var single = encoder.singleValueContainer()
            try single.encode("person")
        case .organization:
            var single = encoder.singleValueContainer()
            try single.encode("organization")
        case .location:
            var single = encoder.singleValueContainer()
            try single.encode("location")
        case .date:
            var single = encoder.singleValueContainer()
            try single.encode("date")
        case .time:
            var single = encoder.singleValueContainer()
            try single.encode("time")
        case .money:
            var single = encoder.singleValueContainer()
            try single.encode("money")
        case .percent:
            var single = encoder.singleValueContainer()
            try single.encode("percent")
        case .email:
            var single = encoder.singleValueContainer()
            try single.encode("email")
        case .phone:
            var single = encoder.singleValueContainer()
            try single.encode("phone")
        case .url:
            var single = encoder.singleValueContainer()
            try single.encode("url")
        case .custom(let value):
            var container = encoder.container(keyedBy: __AlefExternalTagKey.self)
            try container.encode(value, forKey: __AlefExternalTagKey("custom"))
        }
    }
}
...
public enum FormatMetadata: Codable, Sendable, Hashable {
    ...
    case audio(field0: AudioMetadata)
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let type = try container.decode(String.self, forKey: .formatType)
        switch type {
        case "pdf":
            self = .pdf(field0: try PdfMetadata(from: decoder))
        case "docx":
            self = .docx(field0: try DocxMetadata(from: decoder))
        case "excel":
            self = .excel(field0: try ExcelMetadata(from: decoder))
        case "email":
            self = .email(field0: try EmailMetadata(from: decoder))
        case "pptx":
            self = .pptx(field0: try PptxMetadata(from: decoder))
        case "archive":
            self = .archive(field0: try ArchiveMetadata(from: decoder))
        case "image":
            self = .image(field0: try ImageMetadata(from: decoder))
        case "xml":
            self = .xml(field0: try XmlMetadata(from: decoder))
        case "text":
            self = .text(field0: try TextMetadata(from: decoder))
        case "html":
            self = .html(field0: try HtmlMetadata(from: decoder))
        case "ocr":
            self = .ocr(field0: try OcrMetadata(from: decoder))
        case "csv":
            self = .csv(field0: try CsvMetadata(from: decoder))
        case "bibtex":
            self = .bibtex(field0: try BibtexMetadata(from: decoder))
        case "citation":
            self = .citation(field0: try CitationMetadata(from: decoder))
        case "fiction_book":
            self = .fictionBook(field0: try FictionBookMetadata(from: decoder))
        case "dbf":
            self = .dbf(field0: try DbfMetadata(from: decoder))
        case "jats":
            self = .jats(field0: try JatsMetadata(from: decoder))
        case "epub":
            self = .epub(field0: try EpubMetadata(from: decoder))
        case "pst":
            self = .pst(field0: try PstMetadata(from: decoder))
        case "audio":
            self = .audio(field0: try AudioMetadata(from: decoder))
        case "code":
            self = .code(field0: try CodeMetadata(from: decoder))
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .formatType,
                in: container,
                debugDescription: "Unknown FormatMetadata type: \(type)"
            )
        }
    }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        switch self {
        case .pdf(let field0):
            try container.encode("pdf", forKey: .formatType)
            try field0.encode(to: encoder)

        case .docx(let field0):
            try container.encode("docx", forKey: .formatType)
            try field0.encode(to: encoder)

        case .excel(let field0):
            try container.encode("excel", forKey: .formatType)
            try field0.encode(to: encoder)

        case .email(let field0):
            try container.encode("email", forKey: .formatType)
            try field0.encode(to: encoder)

        case .pptx(let field0):
            try container.encode("pptx", forKey: .formatType)
            try field0.encode(to: encoder)

        case .archive(let field0):
            try container.encode("archive", forKey: .formatType)
            try field0.encode(to: encoder)

        case .image(let field0):
            try container.encode("image", forKey: .formatType)
            try field0.encode(to: encoder)

        case .xml(let field0):
            try container.encode("xml", forKey: .formatType)
            try field0.encode(to: encoder)

        case .text(let field0):
            try container.encode("text", forKey: .formatType)
            try field0.encode(to: encoder)

        case .html(let field0):
            try container.encode("html", forKey: .formatType)
            try field0.encode(to: encoder)

        case .ocr(let field0):
            try container.encode("ocr", forKey: .formatType)
            try field0.encode(to: encoder)

        case .csv(let field0):
            try container.encode("csv", forKey: .formatType)
            try field0.encode(to: encoder)

        case .bibtex(let field0):
            try container.encode("bibtex", forKey: .formatType)
            try field0.encode(to: encoder)

        case .citation(let field0):
            try container.encode("citation", forKey: .formatType)
            try field0.encode(to: encoder)

        case .fictionBook(let field0):
            try container.encode("fiction_book", forKey: .formatType)
            try field0.encode(to: encoder)

        case .dbf(let field0):
            try container.encode("dbf", forKey: .formatType)
            try field0.encode(to: encoder)

        case .jats(let field0):
            try container.encode("jats", forKey: .formatType)
            try field0.encode(to: encoder)

        case .epub(let field0):
            try container.encode("epub", forKey: .formatType)
            try field0.encode(to: encoder)

        case .pst(let field0):
            try container.encode("pst", forKey: .formatType)
            try field0.encode(to: encoder)

        case .audio(let field0):
            try container.encode("audio", forKey: .formatType)
            try field0.encode(to: encoder)

        case .code(let field0):
            try container.encode("code", forKey: .formatType)
            try field0.encode(to: encoder)
...
public enum GlmOcrLayoutMode: String, Codable, Sendable, Hashable {
    case wholePage = "whole_page"
    case paired
}
extension GlmOcrLayoutMode {
    func intoRust() throws -> RustBridge.GlmOcrLayoutMode {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.glmOcrLayoutModeFromJson(json)
    }
}

/// Task accepted by `candle-glm-ocr` backend options.
public enum GlmOcrTaskKind: String, Codable, Sendable, Hashable {
    case ocr
    case table
    case formula
    case chart
...
extension GlmOcrTaskKind {
    func intoRust() throws -> RustBridge.GlmOcrTaskKind {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.glmOcrTaskKindFromJson(json)
    }
}
...
public enum HostMatcher: Codable, Sendable, Hashable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        case "suffix":
            self = .suffix(value: try container.decode(String.self, forKey: .value))
        case "cidr":
            self = .cidr(value: try container.decode(String.self, forKey: .value))
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .type,
                in: container,
                debugDescription: "Unknown HostMatcher type: \(type)"
            )
        }
    }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .suffix(let value):
            try container.encode("suffix", forKey: .type)
            try container.encode(value, forKey: .value)

        case .cidr(let value):
            try container.encode("cidr", forKey: .type)
            try container.encode(value, forKey: .value)

        }
    }
}
extension HostMatcher {
    /// Returns the serde wire tag identifying this variant -- the same value
    /// the pre-promotion opaque binding's `to_string()` returned -- dropping
    /// any associated payload.
    public func toString() -> String {
...
extension HostMatcher {
    func intoRust() throws -> RustBridge.HostMatcher {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.hostMatcherFromJson(json)
    }
}
...
public typealias HtmlToMarkdownRsOutputFormat = RustBridge.HtmlToMarkdownRsOutputFormat
...
/// (JSON, TOML, YAML).
public enum LayoutStrategy: String, Codable, Sendable, Hashable {
...
extension LayoutStrategy {
    func intoRust() throws -> RustBridge.LayoutStrategy {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.layoutStrategyFromJson(json)
    }
}
...
public enum MimeDetectionPolicy: String, Codable, Sendable, Hashable {
    /// Prefer a supported content signature, falling back to the filename extension.
    case preferContent = "prefer_content"
    ...
    case trustExtension = "trust_extension"
    /// Ignore the filename extension and require content-based detection.
    case contentOnly = "content_only"
}
extension MimeDetectionPolicy {
    func intoRust() throws -> RustBridge.MimeDetectionPolicy {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.mimeDetectionPolicyFromJson(json)
    }
}
...
extension NoChunkingReason {
    ...
    public func toString() -> String {
        ...
        case .fewPages:
            return "FewPages"
        case .textLayerDetected:
            return "TextLayerDetected"
        case .formatNotChunkable:
            return "FormatNotChunkable"
        case .chunkingDisabled:
...
public enum NodeContent: Codable, Sendable, Hashable {
    ...
    case metadataBlock(entries: [KeyValueAttribute])
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let type = try container.decode(String.self, forKey: .nodeType)
        switch type {
        case "title":
        ...
        case "comment":
            self = .comment(text: try container.decode(String.self, forKey: .text))
        case "group":
        ...
        case "metadata_block":
            self = .metadataBlock(entries: try container.decode([KeyValueAttribute].self, forKey: .entries))
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .nodeType,
                in: container,
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        switch self {
        case .title(let text):
            try container.encode("title", forKey: .nodeType)
            try container.encode(text, forKey: .text)

        case .heading(let level, let text):
            try container.encode("heading", forKey: .nodeType)
            try container.encode(level, forKey: .level)
            try container.encode(text, forKey: .text)

        case .paragraph(let text):
            try container.encode("paragraph", forKey: .nodeType)
            try container.encode(text, forKey: .text)

        case .list(let ordered):
            try container.encode("list", forKey: .nodeType)
            try container.encode(ordered, forKey: .ordered)

        case .listItem(let text):
            try container.encode("list_item", forKey: .nodeType)
            try container.encode(text, forKey: .text)

        case .table(let grid):
            try container.encode("table", forKey: .nodeType)
            try container.encode(grid, forKey: .grid)

        case .image(let description, let imageIndex, let src):
            try container.encode("image", forKey: .nodeType)
            try container.encodeIfPresent(description, forKey: .description)
            try container.encodeIfPresent(imageIndex, forKey: .imageIndex)
            try container.encodeIfPresent(src, forKey: .src)

        case .code(let text, let language):
            try container.encode("code", forKey: .nodeType)
            try container.encode(text, forKey: .text)
            try container.encodeIfPresent(language, forKey: .language)

        case .quote:
            try container.encode("quote", forKey: .nodeType)
        case .formula(let text):
            try container.encode("formula", forKey: .nodeType)
            try container.encode(text, forKey: .text)

        case .footnote(let text):
            try container.encode("footnote", forKey: .nodeType)
            try container.encode(text, forKey: .text)

        case .comment(let text):
            try container.encode("comment", forKey: .nodeType)
            try container.encode(text, forKey: .text)

        case .group(let label, let headingLevel, let headingText):
            try container.encode("group", forKey: .nodeType)
        ...
        case .pageBreak:
            try container.encode("page_break", forKey: .nodeType)
        case .slide(let number, let title):
            try container.encode("slide", forKey: .nodeType)
            try container.encode(number, forKey: .number)
            try container.encodeIfPresent(title, forKey: .title)

        case .definitionList:
            try container.encode("definition_list", forKey: .nodeType)
        case .definitionItem(let term, let definition):
            try container.encode("definition_item", forKey: .nodeType)
            try container.encode(term, forKey: .term)
            try container.encode(definition, forKey: .definition)

        case .citation(let key, let text):
            try container.encode("citation", forKey: .nodeType)
            try container.encode(key, forKey: .key)
            try container.encode(text, forKey: .text)

        case .admonition(let kind, let title):
            try container.encode("admonition", forKey: .nodeType)
            try container.encode(kind, forKey: .kind)
            try container.encodeIfPresent(title, forKey: .title)

        case .rawBlock(let format, let content):
            try container.encode("raw_block", forKey: .nodeType)
            try container.encode(format, forKey: .format)
            try container.encode(content, forKey: .content)

        case .metadataBlock(let entries):
            try container.encode("metadata_block", forKey: .nodeType)
...
public enum OcrBoundingGeometry: Codable, Sendable, Hashable {
    ...
    case quadrilateral(points: [OcrPoint])
    ...
    public init(from decoder: Decoder) throws {
        ...
        case "quadrilateral":
            self = .quadrilateral(points: try container.decode([OcrPoint].self, forKey: .points))
        default:
            throw DecodingError.dataCorruptedError(
                forKey: .type,
                in: container,
...
public enum OutputFormat: Codable, Sendable, Hashable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .markdown:
            var single = encoder.singleValueContainer()
            try single.encode("markdown")
        case .djot:
            var single = encoder.singleValueContainer()
            try single.encode("djot")
        case .html:
            var single = encoder.singleValueContainer()
            try single.encode("html")
        case .json:
            var single = encoder.singleValueContainer()
            try single.encode("json")
        case .docTags:
            var single = encoder.singleValueContainer()
            try single.encode("doctags")
        case .custom(let value):
            var single = encoder.singleValueContainer()
...
public enum PaddleOcrVlTaskKind: String, Codable, Sendable, Hashable {
    case ocr
    case table
    case formula
    case chart
}
extension PaddleOcrVlTaskKind {
    func intoRust() throws -> RustBridge.PaddleOcrVlTaskKind {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.paddleOcrVlTaskKindFromJson(json)
    }
}
...
public enum PageOrientationHandling: String, Codable, Sendable, Hashable {
    /// Reconstructs reading order regardless of page rotation — safe to hand a
    /// raster in any orientation.
    case selfCorrecting = "SelfCorrecting"
    /// Recognises rotated text correctly but emits blocks in raw raster order,
    /// so the caller must reorder.
    case recognisesRotatedText = "RecognisesRotatedText"
    /// Requires an upright raster; rotated text produces garbage.
    case requiresUpright = "RequiresUpright"
}
extension PageOrientationHandling {
    func intoRust() throws -> RustBridge.PageOrientationHandling {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.pageOrientationHandlingFromJson(json)
    }
}
...
public enum PdfBackend: String, Codable, Sendable, Hashable {
...
extension PdfBackend {
    func intoRust() throws -> RustBridge.PdfBackend {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.pdfBackendFromJson(json)
    }
}
...
public enum PiiCategory: Codable, Sendable, Hashable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .ssn:
            var single = encoder.singleValueContainer()
            try single.encode("ssn")
        case .creditCard:
            var single = encoder.singleValueContainer()
            try single.encode("credit_card")
        case .postalCode:
            var single = encoder.singleValueContainer()
            try single.encode("postal_code")
        case .ipAddress:
            var single = encoder.singleValueContainer()
            try single.encode("ip_address")
        case .iban:
            var single = encoder.singleValueContainer()
            try single.encode("iban")
        case .swiftBic:
            var single = encoder.singleValueContainer()
            try single.encode("swift_bic")
        case .dateOfBirth:
            var single = encoder.singleValueContainer()
            try single.encode("date_of_birth")
        case .person:
            var single = encoder.singleValueContainer()
            try single.encode("person")
        case .organization:
...
public enum ProbeStatus: String, Codable, Sendable, Hashable {
...
extension ProbeStatus {
    func intoRust() throws -> RustBridge.ProbeStatus {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.probeStatusFromJson(json)
    }
}
...
public func classifyChunksOwned(_ resultJson: String, _ configJson: String) async throws -> ExtractedDocument {
    let result = try extractedDocumentFromJson(resultJson)
    let config = try chunkClassificationConfigFromJson(configJson)
    return try await classifyChunksOwned(result: result, config: config)
}
public func doctor(_ configJson: String) throws -> DoctorReport {
    let config = try extractionConfigFromJson(configJson)
    return try doctor(config: config)
}
...
public func attributesFromJson(_ json: String) throws -> Attributes {
    let data = json.data(using: .utf8) ?? Data()
...
public func bedrockConfigFromJson(_ json: String) throws -> BedrockConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(BedrockConfig.self, from: data)
}
public func bibtexMetadataFromJson(_ json: String) throws -> BibtexMetadata {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(BibtexMetadata.self, from: data)
}
...
public func codeDataNodeFromJson(_ json: String) throws -> CodeDataNode {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CodeDataNode.self, from: data)
}
...
public func concurrencyConfigFromJson(_ json: String) throws -> ConcurrencyConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ConcurrencyConfig.self, from: data)
}
...
public func csvConfigFromJson(_ json: String) throws -> CsvConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CsvConfig.self, from: data)
}
...
public func deepseekOcrBackendOptionsFromJson(_ json: String) throws -> DeepseekOcrBackendOptions {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(DeepseekOcrBackendOptions.self, from: data)
}
...
public func djotAttributeGroupFromJson(_ json: String) throws -> DjotAttributeGroup {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(DjotAttributeGroup.self, from: data)
}
...
public func doctorCheckFromJson(_ json: String) throws -> DoctorCheck {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(DoctorCheck.self, from: data)
}
public func doctorReportFromJson(_ json: String) throws -> DoctorReport {
    let data = json.data(using: .utf8) ?? Data()
...
public func documentNodeFromJson(_ json: String) throws -> DocumentNode {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(DocumentNode.self, from: data)
}
...
public func documentStructureFromJson(_ json: String) throws -> DocumentStructure {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(DocumentStructure.self, from: data)
}
...
public func docxMetadataFromJson(_ json: String) throws -> DocxMetadata {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(DocxMetadata.self, from: data)
}
...
public func elementMetadataFromJson(_ json: String) throws -> ElementMetadata {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ElementMetadata.self, from: data)
}
...
public func excelWorkbookFromJson(_ json: String) throws -> ExcelWorkbook {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ExcelWorkbook.self, from: data)
}
...
public func footnoteFromJson(_ json: String) throws -> Footnote {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(Footnote.self, from: data)
}
...
public func formattedBlockFromJson(_ json: String) throws -> FormattedBlock {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(FormattedBlock.self, from: data)
}
public func formulaFromJson(_ json: String) throws -> Formula {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(Formula.self, from: data)
}
public func geoJsonExtractionConfigFromJson(_ json: String) throws -> GeoJsonExtractionConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(GeoJsonExtractionConfig.self, from: data)
}
public func glmOcrBackendOptionsFromJson(_ json: String) throws -> GlmOcrBackendOptions {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(GlmOcrBackendOptions.self, from: data)
}
...
public func hierarchicalBoundingBoxFromJson(_ json: String) throws -> HierarchicalBoundingBox {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(HierarchicalBoundingBox.self, from: data)
}
...
public func imageDpiFromJson(_ json: String) throws -> ImageDpi {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ImageDpi.self, from: data)
}
...
public func inlineElementFromJson(_ json: String) throws -> InlineElement {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(InlineElement.self, from: data)
}
public func jatsMetadataFromJson(_ json: String) throws -> JatsMetadata {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(JatsMetadata.self, from: data)
}
public func keyValueAttributeFromJson(_ json: String) throws -> KeyValueAttribute {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(KeyValueAttribute.self, from: data)
}
...
public func languageConfidenceFromJson(_ json: String) throws -> LanguageConfidence {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(LanguageConfidence.self, from: data)
}
...
public func llmBudgetConfigFromJson(_ json: String) throws -> LlmBudgetConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(LlmBudgetConfig.self, from: data)
}
public func llmCacheConfigFromJson(_ json: String) throws -> LlmCacheConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(LlmCacheConfig.self, from: data)
}
public func llmConfigFromJson(_ json: String) throws -> LlmConfig {
    return try RustBridge.llmConfigFromJson(json)
}
public func llmProviderConfigFromJson(_ json: String) throws -> LlmProviderConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(LlmProviderConfig.self, from: data)
}
public func llmRateLimitConfigFromJson(_ json: String) throws -> LlmRateLimitConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(LlmRateLimitConfig.self, from: data)
}
...
public func markdownCodeBlockFromJson(_ json: String) throws -> MarkdownCodeBlock {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(MarkdownCodeBlock.self, from: data)
}
public func markdownLinkFromJson(_ json: String) throws -> MarkdownLink {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(MarkdownLink.self, from: data)
}
...
public func ngramRangeFromJson(_ json: String) throws -> NgramRange {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(NgramRange.self, from: data)
}
public func ocrBackendCapabilitiesFromJson(_ json: String) throws -> OcrBackendCapabilities {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(OcrBackendCapabilities.self, from: data)
}
...
public func ocrElementFromJson(_ json: String) throws -> OcrElement {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(OcrElement.self, from: data)
}
...
public func ocrPointFromJson(_ json: String) throws -> OcrPoint {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(OcrPoint.self, from: data)
}
...
public func paddleOcrVlBackendOptionsFromJson(_ json: String) throws -> PaddleOcrVlBackendOptions {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(PaddleOcrVlBackendOptions.self, from: data)
}
...
public func pageOcrConfidenceFromJson(_ json: String) throws -> PageOcrConfidence {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(PageOcrConfidence.self, from: data)
}
...
public func pixelDimensionsFromJson(_ json: String) throws -> PixelDimensions {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(PixelDimensions.self, from: data)
}
...
public func presentationHyperlinkFromJson(_ json: String) throws -> PresentationHyperlink {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(PresentationHyperlink.self, from: data)
}
...
public func tableCellStyleFromJson(_ json: String) throws -> TableCellStyle {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(TableCellStyle.self, from: data)
}
...
public func tokenReductionConfigFromJson(_ json: String) throws -> TokenReductionConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(TokenReductionConfig.self, from: data)
}
...
public func trocrBackendOptionsFromJson(_ json: String) throws -> TrocrBackendOptions {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(TrocrBackendOptions.self, from: data)
}
...
public func candleDeepseekOcrDtypeFromJson(_ json: String) throws -> CandleDeepseekOcrDtype {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CandleDeepseekOcrDtype.self, from: data)
}
public func candleDevicePreferenceFromJson(_ json: String) throws -> CandleDevicePreference {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CandleDevicePreference.self, from: data)
}
public func candleTrocrVariantFromJson(_ json: String) throws -> CandleTrocrVariant {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CandleTrocrVariant.self, from: data)
}
...
public func confidenceSemanticsFromJson(_ json: String) throws -> ConfidenceSemantics {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ConfidenceSemantics.self, from: data)
}
public func contentFilterKindFromJson(_ json: String) throws -> ContentFilterKind {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(ContentFilterKind.self, from: data)
}
...
public func crawlStrategyKindFromJson(_ json: String) throws -> CrawlStrategyKind {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CrawlStrategyKind.self, from: data)
}
public func credentialProviderConfigFromJson(_ json: String) throws -> CredentialProviderConfig {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(CredentialProviderConfig.self, from: data)
}
public func diffLineFromJson(_ json: String) throws -> DiffLine {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(DiffLine.self, from: data)
}
public func documentContentEncodingFromJson(_ json: String) throws -> DocumentContentEncoding {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(DocumentContentEncoding.self, from: data)
}
...
public func glmOcrLayoutModeFromJson(_ json: String) throws -> GlmOcrLayoutMode {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(GlmOcrLayoutMode.self, from: data)
}
public func glmOcrTaskKindFromJson(_ json: String) throws -> GlmOcrTaskKind {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(GlmOcrTaskKind.self, from: data)
}
public func hostMatcherFromJson(_ json: String) throws -> HostMatcher {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(HostMatcher.self, from: data)
}
...
public func layoutStrategyFromJson(_ json: String) throws -> LayoutStrategy {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(LayoutStrategy.self, from: data)
}
...
public func mimeDetectionPolicyFromJson(_ json: String) throws -> MimeDetectionPolicy {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(MimeDetectionPolicy.self, from: data)
}
...
public func paddleOcrVlTaskKindFromJson(_ json: String) throws -> PaddleOcrVlTaskKind {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(PaddleOcrVlTaskKind.self, from: data)
}
public func pageOrientationHandlingFromJson(_ json: String) throws -> PageOrientationHandling {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(PageOrientationHandling.self, from: data)
}
...
public func pdfBackendFromJson(_ json: String) throws -> PdfBackend {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(PdfBackend.self, from: data)
}
...
public func classifyChunksOwned(result: ExtractedDocument, config: ChunkClassificationConfig) async throws -> ExtractedDocument {
    return try await Task.detached(priority: .userInitiated) {
        let result = try RustBridge.classifyChunksOwned(result, config)
        return result
    }.value
}
...
public func doctor(config: ExtractionConfig) throws -> DoctorReport {
    let _rb = RustBridge.doctor(config)
    return try DoctorReport(_rb)
}
...
/// }
/// ```
public func listOcrBackendCapabilities() throws -> [OcrBackendCapabilities] {
    return try RustBridge.listOcrBackendCapabilities().map { ref in try OcrBackendCapabilities(ref) }
}
...
/// ```
public func ocrBackendSupportsLanguage(backend: String, language: String) throws -> Bool {
        let _rb_backend = RustString(backend)
        let _rb_language = RustString(language)
    return try RustBridge.ocrBackendSupportsLanguage(_rb_backend, _rb_language)
}
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.Attributes: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.AudioMetadata: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.BBox: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.BedrockConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ConcurrencyConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.CsvConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.DeepseekOcrBackendOptions: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.DjotAttributeGroup: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.DoctorCheck: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.DoctorReport: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.GeoJsonExtractionConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.GlmOcrBackendOptions: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.HierarchicalBoundingBox: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.ImageDpi: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.KeyValueAttribute: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.Keyword: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.KeywordConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.LanguageConfidence: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.LlmBudgetConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.LlmCacheConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.LlmConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.LlmProviderConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.LlmRateLimitConfig: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.LlmUsage: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.MapResult: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.MarkdownCodeBlock: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.MarkdownLink: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.NgramRange: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.OcrBackendCapabilities: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.OcrPoint: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.PaddleOcrVlBackendOptions: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.PageOcrConfidence: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.PixelDimensions: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.PresentationHyperlink: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.TableCellStyle: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
...
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
extension RustBridge.TrocrBackendOptions: @unchecked Sendable {}
// swift-bridge opaque type used across Task.detached boundaries — Rust type is Send + Sync.
