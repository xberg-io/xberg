...
public final class SwiftDocumentExtractorBox {
    ...
    public func alef_extract(input: RustString, config: RustString) -> String {
        do {
            let input_decoded = try JSONDecoder().decode(ExtractInput.self, from: Data(input.toString().utf8))
            let config_decoded = try JSONDecoder().decode(ExtractionConfig.self, from: Data(config.toString().utf8))
            let result = try bridge.extract(input: input_decoded, config: config_decoded)
