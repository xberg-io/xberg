...
impl From<xberg::BatchBytesItem> for BatchBytesItem {
    fn from(v: xberg::BatchBytesItem) -> Self {
        BatchBytesItem {
...
impl From<xberg::BatchFileItem> for BatchFileItem {
    fn from(v: xberg::BatchFileItem) -> Self {
        BatchFileItem {
...
pub async fn extract_bytes(
    ...
    xberg::extract_bytes(&content, &mime_type, &xberg::ExtractionConfig::from(config))
        .await
...
pub fn extract_bytes_sync(
    ...
    xberg::extract_bytes_sync(&content, &mime_type, &xberg::ExtractionConfig::from(config))
        .map(ExtractionResult::from)
        .map_err(|e| e.to_string())
}
...
pub fn batch_extract_files_sync(
    ...
    xberg::batch_extract_files_sync(
        items.into_iter().map(xberg::BatchFileItem::from).collect::<Vec<_>>(),
        &xberg::ExtractionConfig::from(config),
    )
...
pub fn batch_extract_bytes_sync(
    ...
    xberg::batch_extract_bytes_sync(
        items.into_iter().map(xberg::BatchBytesItem::from).collect::<Vec<_>>(),
        &xberg::ExtractionConfig::from(config),
    )
    .map(|v| v.into_iter().map(ExtractionResult::from).collect::<Vec<_>>())
    .map_err(|e| e.to_string())
