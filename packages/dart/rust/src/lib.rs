...
pub fn batch_extract_bytes_sync(
    ...
    kreuzberg::batch_extract_bytes_sync(
        {
            let _ = items;
            ::std::unimplemented!("batch_extract_bytes from Dart not yet bridged")
        },
        &config,
    )
    .map(|v| v)
...
pub fn extract_email_content(
    ...
    kreuzberg::extraction::email::extract_email_content(&data, &mime_type, fallback_codepage.map(|v| v as u32))
        .map(|v| v)
        .map_err(|e| e.to_string())
}
...
pub fn reduce_tokens(
    ...
    kreuzberg::text::reduce_tokens(&text, &config, language_hint.as_deref())
        .map(|v| v.to_string())
        .map_err(|e| e.to_string())
}
...
pub fn chunk_texts_batch(
    ...
    kreuzberg::chunking::core::chunk_texts_batch(&texts, &config)
        .map(|v| v)
        .map_err(|e| e.to_string())
}
