...
def _to_rust_layout_detection_config(
    ...
    if isinstance(value, dict):
        ...
        if "table_overlap_preference" in value and value["table_overlap_preference"] is not None:
            value["table_overlap_preference"] = _coerce_enum(_rust.TableOverlapPreference, value["table_overlap_preference"])
        if "acceleration" in value and value["acceleration"] is not None:
            value["acceleration"] = _to_rust_acceleration_config(value["acceleration"])
    ...
    return _rust.LayoutDetectionConfig(
        ...
        **({"table_overlap_preference": _coerce_enum(_rust.TableOverlapPreference, value.table_overlap_preference)} if value.table_overlap_preference is not None else {}),
        acceleration=_to_rust_acceleration_config(value.acceleration),
...
def _to_rust_ner_config(value: NerConfig | dict[str, Any] | str | None) -> _rust.NerConfig | None:
    ...
    if isinstance(value, dict):
        ...
        if "hf_architecture" in value and value["hf_architecture"] is not None:
            value["hf_architecture"] = _coerce_enum(_rust.GlinerArchitecture, value["hf_architecture"])
    ...
    return _rust.NerConfig(
        **({"backend": _coerce_enum(_rust.NerBackendKind, value.backend)} if value.backend is not None else {}),
        ...
        hf_model_file=value.hf_model_file,
        hf_tokenizer_file=value.hf_tokenizer_file,
        hf_architecture=_coerce_enum(_rust.GlinerArchitecture, value.hf_architecture),
        llm=_to_rust_llm_config(value.llm),
        model_dir=value.model_dir,
        lora_adapter_dir=value.lora_adapter_dir,
...
def _to_rust_redaction_config(value: RedactionConfig | dict[str, Any] | str | None) -> _rust.RedactionConfig | None:
    ...
    return _rust.RedactionConfig(
        ...
        **({"strategy": _coerce_enum(_rust.RedactionStrategy, value.strategy)} if value.strategy is not None else {}),
        ...
        preserve_terms=value.preserve_terms,
    )
...
) -> _rust.CitationOutput:
    """Fuse merged vision-LLM output with extracted OCR elements and metadata."""
    return _rust.fuse(merged=merged, ocr_elements=ocr_elements, element_metadata=element_metadata, emit_citations=emit_citations, match_threshold=match_threshold, fused_confidence=fused_confidence)
...
def validate_and_merge(
    ...
    return _rust.validate_and_merge(raw_responses=raw_responses, schema=schema, merge_mode=merge_mode)
...
def custom_source_from_parts(
    repo: str | None = None,
    model_file: str | None = None,
    tokenizer_file: str | None = None,
    architecture: GlinerArchitecture | None = None,
) -> CustomGlinerSource | None:
    """Build a [`CustomGlinerSource`] from optional config fields."""
    return _rust.custom_source_from_parts(repo=repo, model_file=model_file, tokenizer_file=tokenizer_file, architecture=architecture)


def dedupe_overlaps(matches: list[PatternMatch]) -> list[PatternMatch]:
    """Pick the highest-priority match among overlapping spans."""
    return _rust.dedupe_overlaps(matches=matches)


def encrypt_map(
    map: RehydrationMap,  # noqa: A002
    passphrase: str,
) -> bytes:
    """Encrypt `map` with `passphrase`. Returns `XPII\x01` + salt(16) + nonce(12) + tag(16) + ci."""
    return _rust.encrypt_map(map=map, passphrase=passphrase)


def decrypt_map(blob: bytes, passphrase: str) -> _rust.RehydrationMap:
    """Decrypt a blob from [`encrypt_map`]."""
    return _rust.decrypt_map(blob=blob, passphrase=passphrase)
...
def find_subject(
    ...
    """Search a decrypted map for `query`, matching either the token or the original value (case."""
    return _rust.find_subject(map=map, query=query)
...
def forget_subject(
    ...
    """Remove every mapping whose token or original value matches `query`. Returns the removed e."""
    return _rust.forget_subject(map=map, query=query)
