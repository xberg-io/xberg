...
def _from_native_docx_metadata(native: Any) -> DocxMetadata:
    ...
    return DocxMetadata(
        core_properties=(None if native.core_properties is None else _from_native_core_properties(native.core_properties)),
        app_properties=(None if native.app_properties is None else _from_native_docx_app_properties(native.app_properties)),
...
def _from_native_file_extraction_config(native: Any) -> FileExtractionConfig:
    ...
    return FileExtractionConfig(
        ...
        content_filter=(None if native.content_filter is None else _from_native_content_filter_config(native.content_filter)),
        ...
        token_reduction=(None if native.token_reduction is None else _from_native_token_reduction_options(native.token_reduction)),
        language_detection=(None if native.language_detection is None else _from_native_language_detection_config(native.language_detection)),
        ...
        postprocessor=(None if native.postprocessor is None else _from_native_post_processor_config(native.postprocessor)),
        ...
        transcription=(None if native.transcription is None else _from_native_transcription_config(native.transcription)),
        ...
        summarization=(None if native.summarization is None else _from_native_summarization_config(native.summarization)),
...
def _from_native_ocr_config(native: Any) -> OcrConfig:
    ...
    return OcrConfig(
        ...
        backend=native.backend,
        language=native.language,
        tesseract_config=(None if native.tesseract_config is None else _from_native_tesseract_config(native.tesseract_config)),
        output_format=native.output_format,
        paddle_ocr_config=native.paddle_ocr_config,
        backend_options=native.backend_options,
        element_config=(None if native.element_config is None else _from_native_ocr_element_config(native.element_config)),
        quality_thresholds=(None if native.quality_thresholds is None else _from_native_ocr_quality_thresholds(native.quality_thresholds)),
...
def _from_native_ocr_extraction_result(native: Any) -> OcrExtractionResult:
    ...
    return OcrExtractionResult(
        ...
        tables=native.tables,
        ocr_elements=(None if native.ocr_elements is None else [_from_native_ocr_element(__v) for __v in native.ocr_elements]),
    )
...
def _from_native_tesseract_config(native: Any) -> TesseractConfig:
    ...
    return TesseractConfig(
        ...
        oem=native.oem,
        min_confidence=native.min_confidence,
        preprocessing=(None if native.preprocessing is None else _from_native_image_preprocessing_config(native.preprocessing)),
        enable_table_detection=native.enable_table_detection,
