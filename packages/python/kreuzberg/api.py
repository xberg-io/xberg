...
# alef:hash:6a4fe2948a5e1c94163221c06f7d05e0356b30d030c282c795334f07be8d159e
...
from .options import EmbeddingModelType, UriKind
...
def extract_file(path: str, config: ExtractionConfig, mime_type: str | None = None) -> ExtractionResult:
...
def extract_email_content(data: bytes, mime_type: str, fallback_codepage: int | None = None) -> EmailExtractionResult:
...
def chunk_text(text: str, config: ChunkingConfig, page_boundaries: list[PageBoundary] | None = None) -> ChunkingResult:
...
def chunk_semantic(
    text: str, config: ChunkingConfig, page_boundaries: list[PageBoundary] | None = None
) -> ChunkingResult:
...
def calculate_optimal_dpi(
    page_width: float, page_height: float, target_dpi: int, max_dimension: int, min_dpi: int, max_dpi: int
) -> int:
...
def render_pdf_page_to_png(
    pdf_bytes: bytes, page_index: int, dpi: int | None = None, password: str | None = None
) -> bytes:
