...
# alef:hash:ca69b96b34531a2f52a68c39603f40a8784e8e5d3866560aa9625a81063424c6
...
from ._kreuzberg import ByteBufferPool, HtmlOutputConfig, ImageExtractionConfig, KeywordConfig, Keyword, ImagePreprocessingConfig, EmbeddingModelType, TokenReductionConfig, OcrConfig, HierarchyConfig, OcrQualityThresholds, LlmConfig, PageConfig, RakeParams, PageBoundary, YakeParams, Element, StringBufferPool, TesseractConfig, PdfConfig, LayoutDetectionConfig, TreeSitterProcessConfig, TreeSitterConfig, TextAnnotation, LanguageDetectionConfig, OcrElement, PostProcessorConfig, OcrConfidence, ExtractionResult, SupportedFormat, ChunkingResult, ChunkingConfig, ContentFilterConfig, EmailExtractionResult, UriKind, LlmUsage, ExtractionConfig, AccelerationConfig, OcrElementConfig, DocumentStructure, Metadata, EmailConfig, EmbeddingConfig
...
def extract_file_sync_ffi(path: str, mime_type: str, config: ExtractionConfig) -> ExtractionResult:
    """FFI-friendly variant of `extract_file_sync` with concrete, non-generic parameter types."""
    _rust_config = _to_rust_extraction_config(config)
    if _rust_config is None:
        msg = "config conversion returned None"
        raise ValueError(msg)
    return _rust.extract_file_sync_ffi(path, mime_type, _rust_config)


def extract_file_ffi(path: str, mime_type: str, config: ExtractionConfig) -> ExtractionResult:
    """FFI-friendly variant of `extract_file` (async) with concrete, non-generic parameter types."""
    _rust_config = _to_rust_extraction_config(config)
    if _rust_config is None:
        msg = "config conversion returned None"
        raise ValueError(msg)
    return _rust.extract_file_ffi(path, mime_type, _rust_config)
