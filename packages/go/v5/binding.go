...
// alef:hash:a8927d9ee38e6c036726f60ed8273f95353ac7efbf691777ce17cb13bcde005d
...
// ChunkSizingCharacters ChunkSizingCharacters size measured in Unicode characters (default).
...
// ChunkSizingTokenizer ChunkSizingTokenizer size measured in tokens from a HuggingFace tokenizer.
...
// EmbeddingModelTypePreset EmbeddingModelTypePreset use a preset model configuration (recommended)
...
// EmbeddingModelTypeCustom EmbeddingModelTypeCustom use a custom ONNX model from HuggingFace
...
// EmbeddingModelTypeLlm EmbeddingModelTypeLlm provider-hosted embedding model via liter-llm.
...
// EmbeddingModelTypePlugin EmbeddingModelTypePlugin in-process embedding backend registered via the plugin system.
...
// DrawingType DrawingType whether the drawing is inline or anchored.
...
// NodeContentTitle NodeContentTitle document title.
...
// NodeContentHeading NodeContentHeading section heading with level (1-6).
...
// NodeContentParagraph NodeContentParagraph body text paragraph.
...
// NodeContentList NodeContentList list container — children are `ListItem` nodes.
...
// NodeContentListItem NodeContentListItem individual list item.
...
// NodeContentTable NodeContentTable table with structured cell grid.
...
// NodeContentImage NodeContentImage image reference.
...
// NodeContentCode NodeContentCode code block.
...
// NodeContentQuote NodeContentQuote block quote — container, children carry the quoted content.
...
// NodeContentFormula NodeContentFormula mathematical formula / equation.
...
// NodeContentFootnote NodeContentFootnote footnote reference content.
...
// NodeContentGroup NodeContentGroup logical grouping container (section, key-value area).
...
// NodeContentPageBreak NodeContentPageBreak page break marker.
...
// NodeContentSlide NodeContentSlide presentation slide container — children are the slide's content nodes.
...
// NodeContentDefinitionList NodeContentDefinitionList definition list container — children are `DefinitionItem` nodes.
...
// NodeContentDefinitionItem NodeContentDefinitionItem individual definition list entry with term and definition.
...
// NodeContentCitation NodeContentCitation citation or bibliographic reference.
...
// NodeContentAdmonition NodeContentAdmonition admonition / callout container (note, warning, tip, etc.).
...
// NodeContentRawBlock NodeContentRawBlock raw block preserved verbatim from the source format.
...
// NodeContentMetadataBlock NodeContentMetadataBlock structured metadata block (email headers, YAML frontmatter, etc.).
...
// AnnotationKindHighlight AnnotationKindHighlight highlighted text (PDF highlights, HTML `<mark>`).
...
// AnnotationKindColor AnnotationKindColor text color (CSS-compatible value, e.g. "#ff0000", "red").
...
// AnnotationKindFontSize AnnotationKindFontSize font size with units (e.g. "12pt", "1.2em", "16px").
...
// AnnotationKindCustom AnnotationKindCustom extensible annotation for format-specific styling.
...
// OcrBoundingGeometryRectangle OcrBoundingGeometryRectangle axis-aligned bounding box (typical for Tesseract output).
...
// OcrBoundingGeometryQuadrilateral OcrBoundingGeometryQuadrilateral 4-point quadrilateral for rotated/skewed text (PaddleOCR).
...
// ListEmbeddingBackends ListEmbeddingBackends list the names of all registered embedding backends.
