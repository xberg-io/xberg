...
extern int32_t goOcrBackendProcessImage(void* user_data, uint8_t* image_bytes, uintptr_t image_bytes_len, char* config, char** out_result, char** out_error);
...
extern int32_t goDocumentExtractorExtractBytes(void* user_data, uint8_t* content, uintptr_t content_len, char* mime_type, char* config, char** out_result, char** out_error);
...
func goOcrBackendProcessImage(
	...
	if image_bytes != nil {
		goImage_bytes = unsafe.Slice((*byte)(unsafe.Pointer(image_bytes)), image_bytes_len)
	}
...
func goDocumentExtractorExtractBytes(
	...
	var goContent []byte
	if content != nil {
		goContent = unsafe.Slice((*byte)(unsafe.Pointer(content)), content_len)
	}
