...
// alef:hash:2114bcf9a761d2367a51a93165cffd9af153e9d4d1760bb025258b61794b345a
...
type DocumentExtractor interface {
	...
	Shutdown() error
	// extract_bytes
	ExtractBytes(content []byte, mime_type string, config ExtractionConfig) (InternalDocument, error)

	// extract_file
	ExtractFile(path string, mime_type string, config ExtractionConfig) (InternalDocument, error)
...
type Renderer interface {
	...
	Shutdown() error
	// render
	Render(doc InternalDocument) (string, error)
...
func goRendererRender(
	...
	if !ok {
		...
		var goDoc InternalDocument
		if doc != nil {
			json.Unmarshal([]byte(C.GoString(doc)), &goDoc)
