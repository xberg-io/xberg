...
var (
	// ErrPdf is returned when PDF rendering failed.
	ErrPdf = errors.New("PDF rendering failed")
	// ErrImage is returned when image decoding failed.
	ErrImage = errors.New("image decoding failed")
	// ErrUnsupportedMime is returned when unsupported mime type.
...
// RasterizeError is a structured error type.
type RasterizeError struct {
	Code    string
	Message string
}

func (e RasterizeError) Error() string { return e.Message }
...
// NerBackendKindCandle NerBackendKindCandle gLiNER2 safetensors via Candle (CPU). Requires `ner-candle` feature.
// Set [`NerConfig::model_dir`] to a local directory containing
// `tokenizer.json` and `model.safetensors`.
NerBackendKindCandle NerBackendKind = "candle"
)


// GlinerArchitecture is an enumeration type.
type GlinerArchitecture string

const (
// GlinerArchitectureGliner1 GlinerArchitectureGliner1 span-mode GLiNER (`gliner-community`/`urchade` lineage, the pinned
// `xberg-io/gliner-models` catalog, and most GLiNER fine-tunes including
// the `knowledgator/gliner-pii-*` family).
GlinerArchitectureGliner1 GlinerArchitecture = "gliner1"
// GlinerArchitectureGliner2 GlinerArchitectureGliner2 schema-prompt GLiNER2 (`fastino/gliner2` lineage). Requires an ONNX export
...
GlinerArchitectureGliner2 GlinerArchitecture = "gliner2"
)
...
// CitationSource is an enumeration type.
type CitationSource string

const (
// CitationSourceLlm CitationSourceLlm value came from the LLM only.
CitationSourceLlm CitationSource = "llm"
// CitationSourceExtracted CitationSourceExtracted value came from extracted source data only.
CitationSourceExtracted CitationSource = "extracted"
// CitationSourceFused CitationSourceFused value fused from both LLM output and extracted source data.
CitationSourceFused CitationSource = "fused"
// CitationSourceNone CitationSourceNone no provenance could be attributed.
CitationSourceNone CitationSource = "none"
)


// Outcome is an enumeration type.
type Outcome string

const (
// OutcomeSuccess OutcomeSuccess all batches validated and merged.
OutcomeSuccess Outcome = "success"
// OutcomePartialSuccess OutcomePartialSuccess at least one batch validated; at least one failed.
OutcomePartialSuccess Outcome = "partial_success"
// OutcomeSchemaInvalid OutcomeSchemaInvalid no batch validated against the schema.
OutcomeSchemaInvalid Outcome = "schema_invalid"
// OutcomeError OutcomeError no batches were supplied, or the schema itself was invalid.
...
type NerConfig struct {
	...
	HfModelFile *string `json:"hf_model_file,omitempty"`
	// Path to the tokenizer file within `hf_repo` (e.g. `"tokenizer.json"`).
	// Required when `hf_repo` is set.
	HfTokenizerFile *string `json:"hf_tokenizer_file,omitempty"`
	...
	HfArchitecture *GlinerArchitecture `json:"hf_architecture,omitempty"`
	...
	LoraAdapterDir *string `json:"lora_adapter_dir,omitempty"`
...
// NoopCache is an opaque handle type.
type NoopCache struct {
	ptr unsafe.Pointer
}

// Free releases the resources held by this handle.
func (h *NoopCache) Free() {
	if h.ptr != nil {
		C.xberg_noop_cache_free((*C.XBERGNoopCache)(h.ptr))
		h.ptr = nil
	}
}


// ProgressEvent coarse progress event emitted during extraction.
//
...
// NoopProgressSink is an opaque handle type.
type NoopProgressSink struct {
	ptr unsafe.Pointer
}

// Free releases the resources held by this handle.
func (h *NoopProgressSink) Free() {
	if h.ptr != nil {
		C.xberg_noop_progress_sink_free((*C.XBERGNoopProgressSink)(h.ptr))
		h.ptr = nil
	}
}


// ModelID minimal identifier for a downloadable model.
//
...
// DefaultModelProvider is an opaque handle type.
type DefaultModelProvider struct {
	ptr unsafe.Pointer
}

// Free releases the resources held by this handle.
func (h *DefaultModelProvider) Free() {
	if h.ptr != nil {
		C.xberg_default_model_provider_free((*C.XBERGDefaultModelProvider)(h.ptr))
		h.ptr = nil
	}
}


// CitedField single field with its citation envelope.
type CitedField struct {
	// The field value.
...
// CitationOutput both views of the fused output: citation-wrapped and flattened.
type CitationOutput struct {
	...
	StructuredOutputFlat json.RawMessage `json:"structured_output_flat"`
}


// BuiltPrompt built prompt components ready to send to the vision model.
...
// ChunkerConfig configuration for batch packing.
//
...
type ChunkerConfig struct {
	...
	AvgTokensPerImage uint32 `json:"avg_tokens_per_image"`
	// Characters-per-token divisor used to estimate text/image byte token counts.
	CharsPerToken uint `json:"chars_per_token"`
}


// Batch batch of pages ready for a single vision-LLM call.
type Batch struct {
...
// PageImage rendered page ready for inline-base64 transport to the vision model.
...
// MarshalJSON serializes `[]byte` fields as a JSON array of integers (the format
// Rust's serde `Vec<u8>` deserializer expects) instead of Go's default base64 string.
func (v PageImage) MarshalJSON() ([]byte, error) {
	// Explicit shadow struct listing every field — embedding the original
	// would cause both base64-string and int-array entries for the same JSON
	// key. Bytes fields rendered as `[]int`; everything else copied verbatim.
	aux := struct {
	...
	aux.PngBytes = make([]int, len(v.PngBytes))
	for i, b := range v.PngBytes {
		aux.PngBytes[i] = int(b)
	}
	return json.Marshal(aux)
}


// MergedOutput merged structured output plus validation bookkeeping.
type MergedOutput struct {
	...
	PerBatchErrors []string `json:"per_batch_errors,omitempty"`
...
// ParsedDocument is an opaque handle type.
type ParsedDocument struct {
	ptr unsafe.Pointer
}

// Free releases the resources held by this handle.
func (h *ParsedDocument) Free() {
	if h.ptr != nil {
		C.xberg_parsed_document_free((*C.XBERGParsedDocument)(h.ptr))
		h.ptr = nil
	}
}


// Engine is an opaque handle type.
type Engine struct {
	ptr unsafe.Pointer
}
...
func (h *Engine) Free() {
	if h.ptr != nil {
		C.xberg_engine_free((*C.XBERGEngine)(h.ptr))
		h.ptr = nil
	}
}


// EngineBuilder is an opaque handle type.
type EngineBuilder struct {
	ptr unsafe.Pointer
}

// Free releases the resources held by this handle.
func (h *EngineBuilder) Free() {
	if h.ptr != nil {
		C.xberg_engine_builder_free((*C.XBERGEngineBuilder)(h.ptr))
		h.ptr = nil
	}
}
...
// CandleBackend is an opaque handle type.
type CandleBackend struct {
	ptr unsafe.Pointer
}

// Free releases the resources held by this handle.
func (h *CandleBackend) Free() {
	if h.ptr != nil {
		C.xberg_candle_backend_free((*C.XBERGCandleBackend)(h.ptr))
		h.ptr = nil
	}
}


// CustomGlinerSource caller-supplied override pointing GLiNER at an arbitrary Hugging Face repo
...
type CustomGlinerSource struct {
	...
	Architecture GlinerArchitecture `json:"architecture"`
}
...
// RehydrationMap is an opaque handle type.
type RehydrationMap struct {
	ptr unsafe.Pointer
}

// Free releases the resources held by this handle.
func (h *RehydrationMap) Free() {
	if h.ptr != nil {
		C.xberg_rehydration_map_free((*C.XBERGRehydrationMap)(h.ptr))
		h.ptr = nil
	}
}
...
//   - match_threshold: Minimum text-similarity score (`0.0`–`1.0`) for an OCR element to be accepted as the source of a field value
...
func Fuse(merged json.RawMessage, ocrElements []json.RawMessage, elementMetadata []json.RawMessage, emitCitations bool, matchThreshold float64, fusedConfidence float64) (*CitationOutput, error) {
	jsonBytescMerged, err := json.Marshal(merged)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal: %w", err)
	}
	cMerged := C.CString(string(jsonBytescMerged))
	defer C.free(unsafe.Pointer(cMerged))


	jsonBytescOcrElements, err := json.Marshal(ocrElements)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal: %w", err)
	}
	cOcrElements := C.CString(string(jsonBytescOcrElements))
	defer C.free(unsafe.Pointer(cOcrElements))


	jsonBytescElementMetadata, err := json.Marshal(elementMetadata)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal: %w", err)
	}
	cElementMetadata := C.CString(string(jsonBytescElementMetadata))
	defer C.free(unsafe.Pointer(cElementMetadata))


	var cEmitCitations C.int32_t
	if emitCitations {
		cEmitCitations = 1
	} else {
		cEmitCitations = 0
	...
	cMatchThreshold := C.double(float64(matchThreshold))


	cFusedConfidence := C.double(float64(fusedConfidence))


	ptr := C.xberg_fuse(cMerged, cOcrElements, cElementMetadata, cEmitCitations, cMatchThreshold, cFusedConfidence)
	defer C.xberg_citation_output_free(ptr)
		jsonPtr := C.xberg_citation_output_to_json(ptr)
	if jsonPtr == nil {
		return nil, fmt.Errorf("failed to convert to JSON")
	}
	defer C.xberg_free_string(jsonPtr)
		var result CitationOutput
	if err := json.Unmarshal([]byte(C.GoString(jsonPtr)), &result); err != nil {
		return nil, fmt.Errorf("failed to unmarshal: %w", err)
	}
	return &result, nil
...
// ValidateAndMerge validate each raw response and merge by the configured strategy.
//
// Arguments:
...
//   - merge_mode: Merging strategy # Behavior
...
func ValidateAndMerge(rawResponses []json.RawMessage, schema json.RawMessage, mergeMode MergeMode) (*MergedOutput, error) {
	jsonBytescRawResponses, err := json.Marshal(rawResponses)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal: %w", err)
	}
	cRawResponses := C.CString(string(jsonBytescRawResponses))
	defer C.free(unsafe.Pointer(cRawResponses))
	...
	cMergeModeStr := C.CString(string(mergeMode))
	defer C.free(unsafe.Pointer(cMergeModeStr))
	cMergeMode := C.xberg_merge_mode_from_str(cMergeModeStr)


	ptr := C.xberg_validate_and_merge(cRawResponses, cSchema, cMergeMode)
	defer C.xberg_merged_output_free(ptr)
		jsonPtr := C.xberg_merged_output_to_json(ptr)
	if jsonPtr == nil {
		return nil, fmt.Errorf("failed to convert to JSON")
	}
	defer C.xberg_free_string(jsonPtr)
		var result MergedOutput
	if err := json.Unmarshal([]byte(C.GoString(jsonPtr)), &result); err != nil {
		return nil, fmt.Errorf("failed to unmarshal: %w", err)
	}
	return &result, nil
...
// CustomSourceFromParts build a [`CustomGlinerSource`] from optional config fields.
//
...
func CustomSourceFromParts(repo *string, modelFile *string, tokenizerFile *string, architecture *GlinerArchitecture) (*CustomGlinerSource, error) {
	var cRepo *C.char
	if repo != nil {
	...
	var cTokenizerFile *C.char
	if tokenizerFile != nil {
		cTokenizerFile = C.CString(*tokenizerFile)
		defer C.free(unsafe.Pointer(cTokenizerFile))
	}


	cArchitectureStr := C.CString(string(architecture))
	defer C.free(unsafe.Pointer(cArchitectureStr))
	cArchitecture := C.xberg_gliner_architecture_from_str(cArchitectureStr)


	ptr := C.xberg_custom_source_from_parts(cRepo, cModelFile, cTokenizerFile, cArchitecture)
	if err := lastError(); err != nil {
		return nil, err
	}
	return func() *CustomGlinerSource {
	jsonPtr := C.xberg_custom_gliner_source_to_json(ptr)
	if jsonPtr == nil { return nil }
	defer C.xberg_free_string(jsonPtr)
	var result CustomGlinerSource
...
// DedupeOverlaps pick the highest-priority match among overlapping spans.
//
...
func DedupeOverlaps(matches []PatternMatch) ([]PatternMatch, error) {
	jsonBytescMatches, err := json.Marshal(matches)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal: %w", err)
	}
	cMatches := C.CString(string(jsonBytescMatches))
	defer C.free(unsafe.Pointer(cMatches))


	ptr := C.xberg_dedupe_overlaps(cMatches)
	if ptr == nil {
		return nil, fmt.Errorf("failed to get result")
	}
	defer C.xberg_free_string(ptr)
...
// EncryptMap encrypt `map` with `passphrase`. Returns `XPII\x01` + salt(16) + nonce(12) + tag(16) + ciphertext.
func EncryptMap(map *RehydrationMap, passphrase string) ([]byte, error) {
	...
	rc := C.xberg_encrypt_map(cMap, cPassphrase, &outPtr, &outLen, &outCap)
	if rc != 0 {
...
// DecryptMap decrypt a blob from [`encrypt_map`].
func DecryptMap(blob []byte, passphrase string) (*RehydrationMap, error) {
	var cBlob *C.uint8_t
	if len(blob) > 0 {
		var cBlobPinner runtime.Pinner
		cBlobPinner.Pin(&blob[0])
		defer cBlobPinner.Unpin()
		cBlob = (*C.uint8_t)(unsafe.Pointer(&blob[0]))
	}
	cBlobLen := C.uintptr_t(len(blob))
	...
	ptr := C.xberg_decrypt_map(cBlob, cBlobLen, cPassphrase)
	if err := lastError(); err != nil {
		if ptr != nil {
			C.xberg_rehydration_map_free(ptr)
		}
		return nil, err
	}
	return &RehydrationMap{ptr: unsafe.Pointer(ptr)}, nil
}
...
// Get is a method.
func (h *NoopCache) Get(key string) *[]byte {
	cKey := C.CString(key)
	defer C.free(unsafe.Pointer(cKey))


	ptr := C.xberg_noop_cache_get((*C.XBERGNoopCache)(unsafe.Pointer(h.ptr)), cKey)
	return func() *[]byte {
	if ptr == nil { return nil }
...
// Put is a method.
func (h *NoopCache) Put(key string, value []byte, ttl *uint64) {
	cKey := C.CString(key)
	defer C.free(unsafe.Pointer(cKey))
	...
	if len(value) > 0 {
		var cValuePinner runtime.Pinner
		cValuePinner.Pin(&value[0])
		defer cValuePinner.Unpin()
		cValue = (*C.uint8_t)(unsafe.Pointer(&value[0]))
	}
	cValueLen := C.uintptr_t(len(value))


	C.xberg_noop_cache_put((*C.XBERGNoopCache)(unsafe.Pointer(h.ptr)), cKey, cValue, cValueLen, cttl)
}


// Emit is a method.
func (h *NoopProgressSink) Emit(event ProgressEvent) error {
	jsonBytescEvent, err := json.Marshal(event)
	if err != nil {
	...
	// instance is constructed instead — semantically equivalent to None for query types
	// whose fields are all optional with serde(default).
	if string(jsonBytescEvent) == "null" {
		jsonBytescEvent = []byte("{}")
	}
	tmpStrcEvent := C.CString(string(jsonBytescEvent))
	cEvent := C.xberg_progress_event_from_json(tmpStrcEvent)
	C.free(unsafe.Pointer(tmpStrcEvent))
	if cEvent == nil {
		return fmt.Errorf("failed to create progress_event: %s", C.GoString(C.xberg_last_error_context()))
	}
	defer C.xberg_progress_event_free(cEvent)


	C.xberg_noop_progress_sink_emit((*C.XBERGNoopProgressSink)(unsafe.Pointer(h.ptr)), cEvent)
	return nil
}


// EnsureModel is a method.
func (h *DefaultModelProvider) EnsureModel(model ModelID) (string, error) {
	jsonBytescModel, err := json.Marshal(model)
	if err != nil {
		return "", fmt.Errorf("failed to marshal: %w", err)
	}
	// When the parameter is a nil pointer (Option<&T> on the Rust side), json.Marshal
	...
	// instance is constructed instead — semantically equivalent to None for query types
	// whose fields are all optional with serde(default).
	if string(jsonBytescModel) == "null" {
		jsonBytescModel = []byte("{}")
	}
	tmpStrcModel := C.CString(string(jsonBytescModel))
	cModel := C.xberg_model_id_from_json(tmpStrcModel)
	C.free(unsafe.Pointer(tmpStrcModel))
	if cModel == nil {
		return "", fmt.Errorf("failed to create model_id: %s", C.GoString(C.xberg_last_error_context()))
	}
	defer C.xberg_model_id_free(cModel)


	ptr := C.xberg_default_model_provider_ensure_model((*C.XBERGDefaultModelProvider)(unsafe.Pointer(h.ptr)), cModel)
	if err := lastError(); err != nil {
		if ptr != nil {
...
func (h *ParsedDocument) MimeType() string {
	ptr := C.xberg_parsed_document_mime_type((*C.XBERGParsedDocument)(unsafe.Pointer(h.ptr)))
	defer C.xberg_free_string(ptr)
	return C.GoString(ptr)
}


// PageCount page count (1 for non-PDF inputs).
func (h *ParsedDocument) PageCount() uint32 {
	ptr := C.xberg_parsed_document_page_count((*C.XBERGParsedDocument)(unsafe.Pointer(h.ptr)))
	return uint32(ptr)
}


// Source shared source bytes.
func (h *ParsedDocument) Source() []byte {
	ptr := C.xberg_parsed_document_source((*C.XBERGParsedDocument)(unsafe.Pointer(h.ptr)))
	return unmarshalBytes(ptr)
}


// Extract content from a single bytes or URI input.
func (h *Engine) Extract(input ExtractInput, config ExtractionConfig) (*ExtractionResult, error) {
	jsonBytescInput, err := json.Marshal(input)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal: %w", err)
	}
	...
	ptr := C.xberg_engine_extract((*C.XBERGEngine)(unsafe.Pointer(h.ptr)), cInput, cConfig)
	if err := lastError(); err != nil {
		return nil, err
	}
	defer C.xberg_extraction_result_free(ptr)
...
// ExtractBatch extract content from multiple bytes or URI inputs.
func (h *Engine) ExtractBatch(inputs []ExtractInput, config ExtractionConfig) (*ExtractionResult, error) {
	jsonBytescInputs, err := json.Marshal(inputs)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal: %w", err)
	}
	...
	ptr := C.xberg_engine_extract_batch((*C.XBERGEngine)(unsafe.Pointer(h.ptr)), cInputs, cConfig)
	if err := lastError(); err != nil {
		return nil, err
	}
	defer C.xberg_extraction_result_free(ptr)
...
func (h *EngineBuilder) Build() *Engine {
	ptr := C.xberg_engine_builder_build((*C.XBERGEngineBuilder)(unsafe.Pointer(h.ptr)))
	return &Engine{ptr: unsafe.Pointer(ptr)}
}


// Detect is a method.
func (h *CandleBackend) Detect(text string, categories []EntityCategory) ([]Entity, error) {
	...
	ptr := C.xberg_candle_backend_detect((*C.XBERGCandleBackend)(unsafe.Pointer(h.ptr)), cText, cCategories)
	if err := lastError(); err != nil {
		return nil, err
	}
	return func() []Entity {
