...
// alef:hash:6897ec8fa74037f228fe69c715539c8aac0d303f4234a9aefa8df7e28aaf0cd0
...
type ConcurrencyConfig struct {
	...
	MaxConcurrentOcr *uint `json:"max_concurrent_ocr,omitempty"`
}
...
type ExtractionConfig struct {
	...
	OcrNearEmptyFallback *bool `json:"ocr_near_empty_fallback,omitempty"`
	...
	OcrScannedPageQualityGate *bool `json:"ocr_scanned_page_quality_gate,omitempty"`
	...
	OcrEmbeddedImages *bool `json:"ocr_embedded_images,omitempty"`
...
func (s *ExtractionConfig) UnmarshalJSON(data []byte) error {
	...
	s.OcrNearEmptyFallback = raw.OcrNearEmptyFallback
	s.OcrScannedPageQualityGate = raw.OcrScannedPageQualityGate
	s.OcrEmbeddedImages = raw.OcrEmbeddedImages
...
type LlmConfig struct {
	...
	MaxResponseBytes *uint `json:"max_response_bytes,omitempty"`
}
...
func (s *LlmConfig) UnmarshalJSON(data []byte) error {
	...
	s.MaxResponseBytes = raw.MaxResponseBytes
...
// OcrBackendCapabilities registered OCR backend's declared name and language capabilities.
//
...
// means.
type OcrBackendCapabilities struct {
...
type OcrQualityThresholds struct {
	...
	EnablePlausibilityOcrRouting *bool `json:"enable_plausibility_ocr_routing,omitempty"`
	...
	MinReliableLanguageChunkRatio *float64 `json:"min_reliable_language_chunk_ratio,omitempty"`
}
...
type PdfMetadata struct {
	...
	FabricatedTextPages []uint32 `json:"fabricated_text_pages,omitempty"`
	...
	// when no page qualifies.
	ImplausibleTextPages []uint32 `json:"implausible_text_pages,omitempty"`
...
type TesseractConfig struct {
	...
	ThresholdingMethod *string `json:"thresholding_method,omitempty"`
}
...
// ListOcrBackendCapabilities list every registered OCR backend's name alongside its declared supported languages.
//
...
// cache.
//
// Errors are returned when returns an error only if the registry lock cannot be acquired in the current environment.
//
// Example:
//
...
//	}
func ListOcrBackendCapabilities() ([]OcrBackendCapabilities, error) {
	ptr := C.xberg_list_ocr_backend_capabilities()
	if err := lastError(); err != nil {
		return nil, err
	}
	if ptr == nil {
	...
	defer C.xberg_free_string(ptr)
	var result []OcrBackendCapabilities
	if err := json.Unmarshal([]byte(C.GoString(ptr)), &result); err != nil {
		return nil, fmt.Errorf("failed to unmarshal: %w", err)
	}
	return result, nil
...
// OcrBackendSupportsLanguage check whether a specific registered OCR backend supports a language.
//
...
//
// Arguments:
//   - backend: Name of a registered OCR backend, as returned by [`list_ocr_backends`]. Lookup is case-insensitive and resolves the same `paddleocr` alias as backend dispatch.
//   - language: Language code to check (e.g. `"eng"`, `"deu"`).
//
// Errors are returned when returns an error if no backend with that name (or alias) is registered.
//
// Example:
//
//	let supported = ocr_backend_supports_language("tesseract", "eng")?;
func OcrBackendSupportsLanguage(backend string, language string) (bool, error) {
	...
	ptr := C.xberg_ocr_backend_supports_language(cBackend, cLanguage)
	if err := lastError(); err != nil {
		return false, err
	}
	return ptr != 0, nil
...
// RunsOcrOnEmbeddedImages whether embedded images get OCR'd.
//
...
func (r *ExtractionConfig) RunsOcrOnEmbeddedImages() (bool, error) {
	jsonBytesRecv, err := json.Marshal(r)
	if err != nil {
		return false, fmt.Errorf("failed to marshal receiver: %w", err)
	}
	...
	defer C.xberg_extraction_config_free(cRecv)
	ptr := C.xberg_extraction_config_runs_ocr_on_embedded_images(cRecv)
	return ptr != 0, nil
}

// WantsOwnBytesInResult returns `true` when a standalone image extraction (the whole input document IS
...
// QR-code detection.
func (r *ExtractionConfig) WantsOwnBytesInResult() (bool, error) {
	jsonBytesRecv, err := json.Marshal(r)
	if err != nil {
		return false, fmt.Errorf("failed to marshal receiver: %w", err)
	}
	...
	defer C.xberg_extraction_config_free(cRecv)
	ptr := C.xberg_extraction_config_wants_own_bytes_in_result(cRecv)
	return ptr != 0, nil
}
