...
defmodule Xberg do
  ...
  @doc "Fuse merged vision-LLM output with extracted OCR elements and metadata."
  @spec fuse(String.t(), [String.t()], [String.t()], boolean(), float(), float()) :: map()
  def fuse(merged, ocr_elements, element_metadata, emit_citations, match_threshold, fused_confidence) do
    Xberg.Native.fuse(merged, ocr_elements, element_metadata, emit_citations, match_threshold, fused_confidence)
  end

  @doc "Validate each raw response and merge by the configured strategy."
  @spec validate_and_merge([String.t()], String.t(), map()) :: map()
  def validate_and_merge(raw_responses, schema, merge_mode) do
    Xberg.Native.validate_and_merge(raw_responses, schema, merge_mode)
  end
  ...
  @doc "Build a [`CustomGlinerSource`] from optional config fields."
  @spec custom_source_from_parts(keyword()) :: {:ok, map() | nil} | {:error, atom, String.t()}
  def custom_source_from_parts(opts \\ []) do
    Xberg.Native.custom_source_from_parts(
      Keyword.get(opts, :repo),
      Keyword.get(opts, :model_file),
      Keyword.get(opts, :tokenizer_file),
      Keyword.get(opts, :architecture)
    )
  end

  @doc "Pick the highest-priority match among overlapping spans."
  @spec dedupe_overlaps([map()]) :: [map()]
  def dedupe_overlaps(matches) do
    Xberg.Native.dedupe_overlaps(Jason.encode!(matches))
  end

  @doc "Encrypt `map` with `passphrase`. Returns `XPII\x01` + salt(16) + nonce(12) + tag(16) + ciphertext."
  @spec encrypt_map(reference(), String.t()) :: {:ok, binary()} | {:error, atom, String.t()}
  def encrypt_map(map, passphrase) do
    Xberg.Native.encrypt_map(map, passphrase)
  end

  @doc "Decrypt a blob from [`encrypt_map`]."
  @spec decrypt_map(binary(), String.t()) :: {:ok, reference()} | {:error, atom, String.t()}
  def decrypt_map(blob, passphrase) do
    Xberg.Native.decrypt_map(blob, passphrase)
  end

  @doc "Search a decrypted map for `query`, matching either the token or the original value (case-insensitive substring match on `original`; exact match on `token`, since tokens are structured like `\"[EMAIL_1]\"`)."
  @spec find_subject(reference(), String.t()) :: [map()]
  def find_subject(map, query) do
    Xberg.Native.find_subject(map, query)
  end

  @doc "Remove every mapping whose token or original value matches `query`. Returns the removed entries (the caller re-encrypts and persists the resulting map — this function does not touch disk)."
  @spec forget_subject(reference(), String.t()) :: [map()]
  def forget_subject(map, query) do
    Xberg.Native.forget_subject(map, query)
  end
  ...
  def noopcache_get_async(obj, key) do
    Xberg.Native.noopcache_get_async(obj, key)
  end

  def noopcache_put_async(obj, key, value, ttl) do
    Xberg.Native.noopcache_put_async(obj, key, value, ttl)
  end

  def noopprogresssink_emit(obj, event) do
    Xberg.Native.noopprogresssink_emit(obj, event)
  end

  def defaultmodelprovider_default do
    Xberg.Native.defaultmodelprovider_default()
  end

  def defaultmodelprovider_ensure_model_async(obj, model) do
    Xberg.Native.defaultmodelprovider_ensure_model_async(obj, model)
  end

  @doc "Build a memo from shared source bytes: detect the MIME type and count"
  def parseddocument_from_bytes(source) do
    Xberg.Native.parseddocument_from_bytes(source)
  end

  @doc "The detected MIME type."
  def parseddocument_mime_type(obj) do
    Xberg.Native.parseddocument_mime_type(obj)
  end

  @doc "The page count (1 for non-PDF inputs)."
  def parseddocument_page_count(obj) do
    Xberg.Native.parseddocument_page_count(obj)
  end

  @doc "The shared source bytes."
  def parseddocument_source(obj) do
    Xberg.Native.parseddocument_source(obj)
  end

  @doc "Start building an [`Engine`]."
  def engine_builder do
    Xberg.Native.engine_builder()
  end

  @doc "Construct an [`Engine`] with default configuration."
  def engine_new_default do
    Xberg.Native.engine_new_default()
  end

  @doc "Extract content from a single bytes or URI input."
  def engine_extract_async(obj, input, config) do
    Xberg.Native.engine_extract_async(obj, input, config)
  end

  @doc "Extract content from multiple bytes or URI inputs."
  def engine_extract_batch_async(obj, inputs, config) do
    Xberg.Native.engine_extract_batch_async(obj, inputs, config)
  end

  @doc "Finalize the builder into an [`Engine`], filling every unset seam with"
  def enginebuilder_build(obj) do
    Xberg.Native.enginebuilder_build(obj)
  end

  @doc "Load from a local model directory. Applies `lora_adapter_dir` if provided."
  def candlebackend_from_local(model_dir, lora_adapter_dir) do
    Xberg.Native.candlebackend_from_local(model_dir, lora_adapter_dir)
  end

  @doc "Load from in-memory model bytes (no filesystem access — required on `wasm32`,"
  def candlebackend_from_bytes(safetensors, tokenizer_json, encoder_config_json) do
    Xberg.Native.candlebackend_from_bytes(safetensors, tokenizer_json, encoder_config_json)
  end

  def candlebackend_detect_async(obj, text, categories) do
    Xberg.Native.candlebackend_detect_async(obj, text, categories)
  end
  ...
  def ibanchecksumvalidator_label(obj) do
    Xberg.Native.ibanchecksumvalidator_label(obj)
  end

  def ibanchecksumvalidator_validate(obj, entity, ctx) do
    Xberg.Native.ibanchecksumvalidator_validate(obj, entity, ctx)
  end

  def luhnvalidator_label(obj) do
    Xberg.Native.luhnvalidator_label(obj)
  end

  def luhnvalidator_validate(obj, entity, ctx) do
    Xberg.Native.luhnvalidator_validate(obj, entity, ctx)
  end
  ...
  defmodule OcrBackend.Host do
    ...
    @callback process_document(String.t(), String.t() | nil) :: {:ok, map()} | {:error, atom, String.t()}
  end
  ...
  The GenServer should dispatch `{:trait_call, ...}` messages to a module that
  implements the `DocumentExtractor.Host` behaviour.
  """
  ...
  defmodule Renderer.Host do
    ...
    @callback render_result(String.t() | nil) :: {:ok, String.t()} | {:error, atom, String.t()}
  end
  ...
  The GenServer should dispatch `{:trait_call, ...}` messages to a module that
  implements the `Renderer.Host` behaviour.
  """
