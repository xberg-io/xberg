...
// alef:hash:a3b21f7ba4a4a132f886a5d6d57fb1b2309f09bd130e1fb913dc30d5d855811a
...
pub fn reduce_tokens(text: String, config: Option<String>, language_hint: Option<String>) -> Result<String, String> {
    let config_core: Option<kreuzberg::TokenReductionConfig> = config
        ...
        .transpose()
        .map_err(|e| e.to_string())?;
    let result = kreuzberg::text::reduce_tokens(
        &text,
        config_core.as_ref().unwrap_or(&Default::default()),
...
pub struct RustlerEmbeddingBackendBridge {
    inner: rustler::env::SavedTerm,
    cached_name: String,
}

impl RustlerEmbeddingBackendBridge {
    /// Create a new bridge wrapping an Elixir term.
    ///
    /// Validates that the term provides all required methods.
    pub fn new(env: rustler::Env<'_>, elixir_term: rustler::Term<'_>) -> Self {
        let owned = rustler::OwnedEnv::new();
        let saved = owned.save(elixir_term);
        let cached_name = owned.run(|env| {
            elixir_term
...
impl kreuzberg::plugins::Plugin for RustlerEmbeddingBackendBridge {
    ...
    fn version(&self) -> String {
        self.env
            .run(|env| {
                let elixir_term = self.inner.load(env);
                let key = rustler::types::atom::Atom::from_str(env, "version").ok()?;
                let method_term: rustler::Term = rustler::Term::map_get(elixir_term, key).ok()??;
                if method_term.atom_to_string().ok().as_deref() == Some("nil") {
                    return None;
                }
                let args: Vec<rustler::Term> = Vec::new();
                let result: rustler::Term = env.call(method_term, &args).ok()?;
                result.decode::<String>().ok()
            })
            .unwrap_or_else(|| Err("method call failed".into()))
    }
    ...
    fn initialize(&self) -> std::result::Result<(), kreuzberg::KreuzbergError> {
        self.env
            .run(|env| {
                let elixir_term = self.inner.load(env);
                let key = rustler::types::atom::Atom::from_str(env, "initialize").ok()?;
                let method_term: rustler::Term = rustler::Term::map_get(elixir_term, key).ok()??;
                if method_term.atom_to_string().ok().as_deref() == Some("nil") {
                    return None;
                }
    ...
    fn shutdown(&self) -> std::result::Result<(), kreuzberg::KreuzbergError> {
        self.env
            .run(|env| {
                let elixir_term = self.inner.load(env);
                let key = rustler::types::atom::Atom::from_str(env, "shutdown").ok()?;
                let method_term: rustler::Term = rustler::Term::map_get(elixir_term, key).ok()??;
                if method_term.atom_to_string().ok().as_deref() == Some("nil") {
                    return None;
                }
...
impl kreuzberg::plugins::EmbeddingBackend for RustlerEmbeddingBackendBridge {
    fn dimensions(&self) -> usize {
        self.env
            .run(|env| {
                let elixir_term = self.inner.load(env);
                let key = rustler::types::atom::Atom::from_str(env, "dimensions").ok()?;
                let method_term: rustler::Term = rustler::Term::map_get(elixir_term, key).ok()??;
                if method_term.atom_to_string().ok().as_deref() == Some("nil") {
                    return None;
                }
    ...
    async fn embed(&self, texts: Vec<String>) -> std::result::Result<Vec<Vec<f32>>, kreuzberg::KreuzbergError> {
        let saved_term = self.inner.clone();
        ...
        tokio::task::spawn_blocking(move || {
            ...
            env.run(|env_ref| {
                let elixir_term = saved_term.load(env_ref);
                let key = rustler::types::atom::Atom::from_str(env_ref, "embed").ok()?;
                let method_term: rustler::Term = rustler::Term::map_get(elixir_term, key).ok()??;
                if method_term.atom_to_string().ok().as_deref() == Some("nil") {
                    return None;
                }
                let args: Vec<rustler::Term> = vec![format!("{:?}", texts).encode(env_ref)];
                let _result = env_ref.call(method_term, &args).ok()?;
                Some(())
            })
        })
        .await
...
#[rustler::nif]
pub fn register_embedding_backend(env: rustler::Env<'_>, elixir_backend: rustler::Term<'_>) -> rustler::Atom {
    let required = ["dimensions", "embed"];
    for method_name in &required {
        if !elixir_backend
            .map_get(rustler::types::atom::Atom::from_str(env, method_name).unwrap())
            .is_ok()
        {
    ...
    let bridge = RustlerEmbeddingBackendBridge::new(env, elixir_backend);
...
#[rustler::nif]
pub fn tessdatamanager_ensure_all_languages(resource: ResourceArc<TessdataManager>) -> Result<usize, String> {
    let result = resource
        .inner
        .as_ref()
        .clone()
