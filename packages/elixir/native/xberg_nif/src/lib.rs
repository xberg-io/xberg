...
impl NerConfig {
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            hf_repo: opts.get("hf_repo").and_then(|t| t.decode().ok()),
            hf_model_file: opts.get("hf_model_file").and_then(|t| t.decode().ok()),
            hf_tokenizer_file: opts.get("hf_tokenizer_file").and_then(|t| t.decode().ok()),
            hf_architecture: opts.get("hf_architecture").and_then(|t| t.decode().ok()),
            llm: opts.get("llm").and_then(|t| t.decode().ok()),
            model_dir: opts.get("model_dir").and_then(|t| t.decode().ok()),
            lora_adapter_dir: opts.get("lora_adapter_dir").and_then(|t| t.decode().ok()),
            custom_labels: opts
                .get("custom_labels")
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
...
impl std::panic::RefUnwindSafe for NoopCache {}

impl rustler::Resource for NoopCache {}

#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize, rustler::NifStruct)]
#[module = "Xberg.ProgressEvent"]
...
impl std::panic::RefUnwindSafe for NoopProgressSink {}

impl rustler::Resource for NoopProgressSink {}

#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize, rustler::NifStruct)]
#[module = "Xberg.ModelId"]
pub struct ModelId {
    pub kind: String,
}
...
impl std::panic::RefUnwindSafe for DefaultModelProvider {}

impl rustler::Resource for DefaultModelProvider {}

#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize, rustler::NifStruct)]
#[module = "Xberg.CitationOutput"]
...
impl ChunkerConfig {
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            max_input_tokens: opts
                .get("max_input_tokens")
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
            avg_tokens_per_image: opts
                .get("avg_tokens_per_image")
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
            chars_per_token: opts
                .get("chars_per_token")
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
        }
...
impl std::panic::RefUnwindSafe for Engine {}

impl rustler::Resource for Engine {}
...
impl std::panic::RefUnwindSafe for EngineBuilder {}

impl rustler::Resource for EngineBuilder {}
...
impl std::panic::RefUnwindSafe for CandleBackend {}

impl rustler::Resource for CandleBackend {}

#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize, rustler::NifStruct)]
#[module = "Xberg.CustomGlinerSource"]
...
impl std::panic::RefUnwindSafe for RehydrationMap {}
...
#[allow(clippy::derivable_impls)]
impl Default for GlinerArchitecture {
    fn default() -> Self {
        Self::Gliner1
    }
}
...
pub fn fuse(
    ...
    let merged_json: serde_json::Value = serde_json::from_str(&merged).map_err(|e| e.to_string())?;
    xberg::engine::structured::citations::fuse(
        merged_json,
        &ocr_elements_core,
        &element_metadata_core,
        emit_citations,
        match_threshold,
        fused_confidence,
    )
...
#[rustler::nif]
pub fn validate_and_merge(raw_responses: Vec<String>, schema: String, merge_mode: MergeMode) -> MergedOutput {
    let schema_json: serde_json::Value = serde_json::from_str(&schema).map_err(|e| e.to_string())?;
    xberg::engine::structured::schema::validate_and_merge(raw_responses, schema_json, merge_mode.into()).into()
}
...
#[rustler::nif]
pub fn dedupe_overlaps(matches: Option<String>) -> Vec<PatternMatch> {
    let matches_core: Vec<xberg::text::redaction::patterns::PatternMatch> = matches
        .and_then(|s| serde_json::from_str::<Vec<xberg::text::redaction::patterns::PatternMatch>>(&s).ok())
        .unwrap_or_default();
...
#[rustler::nif]
pub fn encrypt_map(map: rustler::ResourceArc<RehydrationMap>, passphrase: String) -> Result<Vec<u8>, String> {
    let result = xberg::text::redaction::encrypt_map(&map.inner, &passphrase).map_err(|e| e.to_string())?;
    Ok(result.to_vec())
}

/// Decrypt a blob from `encrypt_map`.
#[rustler::nif]
pub fn decrypt_map(blob: rustler::Binary, passphrase: String) -> Result<ResourceArc<RehydrationMap>, String> {
    let result = xberg::text::redaction::decrypt_map(blob.as_slice(), &passphrase).map_err(|e| e.to_string())?;
...
#[rustler::nif(schedule = "DirtyCpu")]
pub fn noopcache_get_async(resource: rustler::ResourceArc<NoopCache>, _key: String) -> Result<Option<Vec<u8>>, String> {
    std::thread::Builder::new()
        .stack_size(32 * 1024 * 1024)
        .spawn(move || {
            let rt = tokio::runtime::Runtime::new().map_err(|e| e.to_string())?;
            let result = rt.block_on(async { resource.inner.as_ref().get(&_key).await });
            Ok::<_, String>(result)
        })
        .map_err(|e| e.to_string())?
        .join()
...
#[rustler::nif(schedule = "DirtyCpu")]
pub fn noopcache_put_async(
    resource: rustler::ResourceArc<NoopCache>,
    _key: String,
    _value: Vec<u8>,
    _ttl: Option<u64>,
) -> Result<(), String> {
        ...
        .spawn(move || {
            ...
            let result = rt.block_on(async {
                resource
                    ...
                    .put(
                        &_key,
                        _value.as_slice().to_vec(),
                        std::time::Duration::from_millis(_ttl),
                    )
                    .await
            });
            Ok::<_, String>(result)
...
#[rustler::nif]
pub fn noopprogresssink_emit(resource: rustler::ResourceArc<NoopProgressSink>, _event: ProgressEvent) -> () {
    resource.inner.as_ref().emit(_event.into())
}

#[rustler::nif]
pub fn defaultmodelprovider_default() -> ResourceArc<DefaultModelProvider> {
    ResourceArc::new(DefaultModelProvider {
        inner: Arc::new(xberg::engine::seams::DefaultModelProvider::default()),
    })
}

#[rustler::nif(schedule = "DirtyCpu")]
pub fn defaultmodelprovider_ensure_model_async(
    resource: rustler::ResourceArc<DefaultModelProvider>,
    model: ModelId,
) -> Result<String, String> {
    Err(String::from("Not implemented: defaultmodelprovider_ensure_model_async"))
}

#[rustler::nif]
pub fn chunkerconfig_default() -> ChunkerConfig {
...
#[rustler::nif]
pub fn parseddocument_from_bytes(source: Vec<u8>) -> Result<ResourceArc<ParsedDocument>, String> {
    let result =
        xberg::engine::parsed::ParsedDocument::from_bytes(source.as_slice().to_vec()).map_err(|e| e.to_string())?;
...
#[rustler::nif]
pub fn parseddocument_mime_type(resource: rustler::ResourceArc<ParsedDocument>) -> String {
    resource.inner.as_ref().mime_type().into()
}

/// The page count (1 for non-PDF inputs).
#[rustler::nif]
pub fn parseddocument_page_count(resource: rustler::ResourceArc<ParsedDocument>) -> u32 {
    resource.inner.as_ref().page_count()
}

/// The shared source bytes.
#[rustler::nif]
pub fn parseddocument_source(resource: rustler::ResourceArc<ParsedDocument>) -> Vec<u8> {
    resource.inner.as_ref().source().to_vec()
}

/// Start building an `Engine`.
#[rustler::nif]
pub fn engine_builder() -> ResourceArc<EngineBuilder> {
...
#[rustler::nif]
pub fn engine_new_default() -> ResourceArc<Engine> {
    ResourceArc::new(Engine {
...
/// Extract content from a single bytes or URI input.
#[rustler::nif(schedule = "DirtyCpu")]
pub fn engine_extract_async(
    resource: rustler::ResourceArc<Engine>,
    input: Option<String>,
...
/// Extract content from multiple bytes or URI inputs.
#[rustler::nif(schedule = "DirtyCpu")]
pub fn engine_extract_batch_async(
    resource: rustler::ResourceArc<Engine>,
                ...
                let result = rt
                    .block_on(async {
                        resource
                            .inner
                            .as_ref()
                            .extract_batch(inputs, config_core.as_ref().unwrap_or(&Default::default()))
                            .await
                    })
                    .map_err(|e| e.to_string())?;
                Ok(result.into())
...
/// its in-core default.
#[rustler::nif]
pub fn enginebuilder_build(resource: rustler::ResourceArc<EngineBuilder>) -> ResourceArc<Engine> {
    ResourceArc::new(Engine {
        inner: Arc::new((*resource.inner).clone().build()),
    })
}
...
#[rustler::nif]
pub fn candlebackend_from_local(
    model_dir: String,
    lora_adapter_dir: Option<String>,
) -> Result<ResourceArc<CandleBackend>, String> {
    let result = xberg::text::ner::candle::CandleBackend::from_local(
        &std::path::PathBuf::from(model_dir),
        &std::path::PathBuf::from(lora_adapter_dir),
    )
    .map_err(|e| e.to_string())?;
    Ok(ResourceArc::new(CandleBackend {
        inner: Arc::new(result),
    }))
}
...
#[rustler::nif]
pub fn candlebackend_from_bytes(
    safetensors: Vec<u8>,
    tokenizer_json: Vec<u8>,
    encoder_config_json: Vec<u8>,
) -> Result<ResourceArc<CandleBackend>, String> {
    let result = xberg::text::ner::candle::CandleBackend::from_bytes(
        safetensors.as_slice(),
        tokenizer_json.as_slice(),
        encoder_config_json.as_slice(),
    )
    .map_err(|e| e.to_string())?;
    Ok(ResourceArc::new(CandleBackend {
        inner: Arc::new(result),
...
#[rustler::nif(schedule = "DirtyCpu")]
pub fn candlebackend_detect_async(
    resource: rustler::ResourceArc<CandleBackend>,
    text: String,
    categories: Vec<EntityCategory>,
) -> Result<Vec<Entity>, String> {
    Err(String::from("Not implemented: candlebackend_detect_async"))
}
...
impl From<ChunkerConfig> for xberg::engine::structured::chunk::ChunkerConfig {
    fn from(val: ChunkerConfig) -> Self {
        Self {
            ...
            chars_per_token: val.chars_per_token,
        }
    }
}
...
/// Convert a `xberg::engine::structured::rasterize::RasterizeError` error to a Rustler error string.
#[allow(dead_code)]
fn rasterize_error_to_rustler_err(e: xberg::engine::structured::rasterize::RasterizeError) -> String {
    e.to_string()
}
...
fn on_load(env: rustler::Env, _info: rustler::Term) -> bool {
    env.register::<NoopCache>()
        .expect("Failed to register resource type NoopCache");
    env.register::<NoopProgressSink>()
        .expect("Failed to register resource type NoopProgressSink");
    env.register::<DefaultModelProvider>()
        .expect("Failed to register resource type DefaultModelProvider");
    env.register::<ParsedDocument>()
        .expect("Failed to register resource type ParsedDocument");
    env.register::<Engine>()
        .expect("Failed to register resource type Engine");
    env.register::<EngineBuilder>()
        .expect("Failed to register resource type EngineBuilder");
    env.register::<CandleBackend>()
        .expect("Failed to register resource type CandleBackend");
    env.register::<RehydrationMap>()
