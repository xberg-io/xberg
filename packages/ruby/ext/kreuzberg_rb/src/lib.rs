...
// alef:hash:0994bc9e3644baa652c8c5bc331d24f21d60556e93a0013a64f118050e3beaca
...
fn embed_texts(args: &[magnus::Value]) -> Result<Vec<Vec<f32>>, Error> {
    ...
    let config: Option<EmbeddingConfig> = match config {
        Some(_v) if !_v.is_nil() => {
            let s: String = _v.funcall("to_json", ())?;
            let core: kreuzberg::EmbeddingConfig = serde_json::from_str(&s).map_err(|e| {
    ...
    let result = kreuzberg::embed_texts(texts, config.map(Into::into)).map_err(|e| {
        magnus::Error::new(
            unsafe { Ruby::get_unchecked() }.exception_runtime_error(),
            e.to_string(),
        )
