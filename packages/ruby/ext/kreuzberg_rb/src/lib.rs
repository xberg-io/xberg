...
// alef:hash:1e0e00e38059d0fdbb1a974707eb46e37ee13a4d59a3aaa2e9ac7cf072789b1b
...
fn batch_extract_files_sync(items: Vec<BatchFileItem>, config: magnus::Value) -> Result<Vec<ExtractionResult>, Error> {
    let items_core: Vec<kreuzberg::BatchFileItem> = {
        let s: String = items.funcall("to_json", ())?;
        serde_json::from_str(&s).map_err(|e| {
            magnus::Error::new(
                unsafe { Ruby::get_unchecked() }.exception_runtime_error(),
                e.to_string(),
...
fn embed_texts_async(texts: Vec<String>, config: magnus::Value) -> Result<Vec<Vec<f32>>, Error> {
...
fn embed_texts_async_async(texts: Vec<String>, config: magnus::Value) -> Result<Vec<Vec<f32>>, Error> {
    let config_core: kreuzberg::EmbeddingConfig = {
        let s: String = config.funcall("to_json", ())?;
        serde_json::from_str(&s).map_err(|e| {
            magnus::Error::new(
...
fn init(ruby: &Ruby) -> Result<(), Error> {
    ...
    module.define_module_function("embed_texts_async", function!(embed_texts_async, 2))?;
    module.define_module_function("embed_texts_async_async", function!(embed_texts_async_async, 2))?;
