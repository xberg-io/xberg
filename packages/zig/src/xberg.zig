...
// alef:hash:2d8cc54cbbf218aecac84f14875426d658a729b92c41360301486cdf71118c92
// alef:handle-abi:1
...
pub const ExtractionConfig = struct {
    ...
    ///
    /// `null` (the default) derives the answer from whether `Self.ocr` is set, which is
    ...
    ocr_scanned_page_quality_gate: ?bool,
...
/// means.
pub const OcrBackendCapabilities = struct {
    ...
    supported_languages: []const []const u8,
};
...
pub const PdfMetadata = struct {
    ...
    fabricated_text_pages: ?[]const u32,
    ...
    /// when no page qualifies.
    implausible_text_pages: ?[]const u32,
...
pub fn list_ocr_backend_capabilities() XbergError![]u8 {
    const _result = c.xberg_list_ocr_backend_capabilities();
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(XbergError);
    }
    if (_result == null) {
    ...
    const _result_len = c.xberg_list_ocr_backend_capabilities_len();
    return blk: {
        if (_result == null) return error.UnknownFfiError;
        const slice = _result[0.._result_len];
        defer _free_string(_result);
...
pub fn ocr_backend_supports_language(backend: []const u8, language: []const u8) XbergError!bool {
    const backend_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{backend}, 0);
    defer std.heap.c_allocator.free(backend_z);
    ...
    const _result = c.xberg_ocr_backend_supports_language(backend_z, language_z);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(XbergError);
    }
    return _result != 0;
