...
inline fn _error_with_message(comptime E: type) E {
    ...
    if (E == RasterizeError) return _from_ffi_msg_RasterizeError(msg_opt);
...
/// Map an FFI error message string to a `RasterizeError` variant by prefix-matching
/// against the upstream `#[error("...")]` templates. Falls back to the
/// first declared variant when no prefix matches.
inline fn _from_ffi_msg_RasterizeError(msg_opt: ?[]const u8) RasterizeError {
    if (msg_opt) |msg| {
        if (std.mem.startsWith(u8, msg, "pdf rendering failed:")) return error.Pdf;
        if (std.mem.startsWith(u8, msg, "image decoding failed:")) return error.Image;
        if (std.mem.startsWith(u8, msg, "unsupported mime type:")) return error.UnsupportedMime;
    }
    return _first_error(RasterizeError);
}
...
pub const NerConfig = struct {
    ...
    hf_architecture: ?GlinerArchitecture,
...
pub const GlinerArchitecture = enum {
...
pub fn extract(input: []const u8, config: []const u8) RasterizeError![]u8 {
    ...
    if (input_handle == null) return _error_with_message(RasterizeError);
    ...
    if (config_handle == null) return _error_with_message(RasterizeError);
    ...
    return blk: {
        ...
        if (_json_ptr == null) return _first_error(RasterizeError); // FFI to_json returned NULL — surface first declared variant rather than crashing on null-slice
...
pub fn extract_batch(inputs: []const u8, config: []const u8) RasterizeError![]u8 {
...
pub fn fuse(merged: []const u8, ocr_elements: []const u8, element_metadata: []const u8, emit_citations: bool, match_threshold: f64, fused_confidence: f64) error{OutOfMemory}!CitationOutput {
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    const ocr_elements_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{ocr_elements}, 0);
    defer std.heap.c_allocator.free(ocr_elements_z);
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    const element_metadata_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{element_metadata}, 0);
    defer std.heap.c_allocator.free(element_metadata_z);
    const _result = c.xberg_fuse(merged, ocr_elements_z, element_metadata_z, emit_citations, match_threshold, fused_confidence);
    return CitationOutput{ ._handle = _result.? };
}
...
pub fn validate_and_merge(raw_responses: []const u8, schema: []const u8, merge_mode: MergeMode) error{OutOfMemory}!MergedOutput {
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    const raw_responses_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{raw_responses}, 0);
    defer std.heap.c_allocator.free(raw_responses_z);
    const _result = c.xberg_validate_and_merge(raw_responses_z, schema, merge_mode);
    return MergedOutput{ ._handle = _result.? };
}
...
/// bindings.
pub fn list_embedding_backends() RasterizeError![]u8 {
    ...
    return blk: {
        if (_result == null) return _first_error(RasterizeError);
        const slice = _result[0.._result_len];
        const owned = try std.heap.c_allocator.dupe(u8, slice);
        _free_string(_result);
        break :blk owned;
...
pub fn list_document_extractors() RasterizeError![]u8 {
...
pub fn list_ocr_backends() RasterizeError![]u8 {
...
pub fn list_post_processors() RasterizeError![]u8 {
...
pub fn list_renderers() RasterizeError![]u8 {
...
///
/// Since v5.0.
pub fn list_reranker_backends() RasterizeError![]u8 {
...
pub fn list_validators() RasterizeError![]u8 {
...
pub fn custom_source_from_parts(repo: ?[]const u8, model_file: ?[]const u8, tokenizer_file: ?[]const u8, architecture: ?GlinerArchitecture) RasterizeError!?CustomGlinerSource {
    const repo_z: ?[:0]u8 = if (repo) |v| try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{v}, 0) else null;
    defer if (repo_z) |z| std.heap.c_allocator.free(z);
    const model_file_z: ?[:0]u8 = if (model_file) |v| try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{v}, 0) else null;
    defer if (model_file_z) |z| std.heap.c_allocator.free(z);
    const tokenizer_file_z: ?[:0]u8 = if (tokenizer_file) |v| try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{v}, 0) else null;
    defer if (tokenizer_file_z) |z| std.heap.c_allocator.free(z);
    const _result = c.xberg_custom_source_from_parts(if (repo_z) |z| z.ptr else null, if (model_file_z) |z| z.ptr else null, if (tokenizer_file_z) |z| z.ptr else null, architecture);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(RasterizeError);
    }
    if (_result == null) {
...
pub fn dedupe_overlaps(matches: []const u8) error{OutOfMemory}![]u8 {
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    const matches_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{matches}, 0);
    defer std.heap.c_allocator.free(matches_z);
    const _result = c.xberg_dedupe_overlaps(matches_z);
    const _result_len = c.xberg_dedupe_overlaps_len(matches_z);
    return blk: {
        const slice = _result[0.._result_len];
        const owned = try std.heap.c_allocator.dupe(u8, slice);
        _free_string(_result);
...
pub fn encrypt_map(map: RehydrationMap, passphrase: []const u8) RasterizeError![]u8 {
    const passphrase_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{passphrase}, 0);
    defer std.heap.c_allocator.free(passphrase_z);
    var _out_ptr: [*c]u8 = undefined;
    var _out_len: usize = 0;
    var _out_cap: usize = 0;
    _ = c.xberg_encrypt_map(map, passphrase_z, &_out_ptr, &_out_len, &_out_cap);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(RasterizeError);
    }
...
pub fn decrypt_map(blob: []const u8, passphrase: []const u8) RasterizeError!RehydrationMap {
    const passphrase_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{passphrase}, 0);
    defer std.heap.c_allocator.free(passphrase_z);
    const _result = c.xberg_decrypt_map(blob.ptr, blob.len, passphrase_z);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(RasterizeError);
    }
...
/// no engine-level caching, so `get` always returns `null`
/// and `put` is inert.
pub const NoopCache = struct {
    _handle: *anyopaque,

    pub fn get(self: *NoopCache, _key: []const u8) error{OutOfMemory}!?[]const u8 {
        const _key_z = try std.heap.c_allocator.dupeZ(u8, _key);
        defer std.heap.c_allocator.free(_key_z);
        const _result = c.xberg_noop_cache_get(@as(*c.XBERGNoopCache, @ptrCast(self._handle)), _key_z);
        return _result;
    }

    pub fn put(self: *NoopCache, _key: []const u8, _value: []const u8, _ttl: ?i64) error{OutOfMemory}!void {
        const _key_z = try std.heap.c_allocator.dupeZ(u8, _key);
        defer std.heap.c_allocator.free(_key_z);
        c.xberg_noop_cache_put(@as(*c.XBERGNoopCache, @ptrCast(self._handle)), _key_z, _value.ptr, _value.len, _ttl);
    }

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *NoopCache) void {
        c.xberg_noop_cache_free(@as(*c.XBERGNoopCache, @ptrCast(self._handle)));
    }
};
...
pub const ProgressEvent = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *ProgressEvent) void {
        c.xberg_progress_event_free(@as(*c.XBERGProgressEvent, @ptrCast(self._handle)));
    }
};
...
pub const NoopProgressSink = struct {
    _handle: *anyopaque,

    pub fn emit(self: *NoopProgressSink, _event: ProgressEvent) error{OutOfMemory}!void {
        c.xberg_noop_progress_sink_emit(@as(*c.XBERGNoopProgressSink, @ptrCast(self._handle)), _event);
    }

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *NoopProgressSink) void {
        c.xberg_noop_progress_sink_free(@as(*c.XBERGNoopProgressSink, @ptrCast(self._handle)));
    }
};
...
pub const ModelId = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *ModelId) void {
        c.xberg_model_id_free(@as(*c.XBERGModelId, @ptrCast(self._handle)));
    }
};

pub fn default_default_model_provider() DefaultModelProvider {
    const _handle = c.xberg_default_model_provider_default();
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.XBERGDefaultModelProvider, @ptrCast(_handle.?)) };
}
...
pub const DefaultModelProvider = struct {
    _handle: *anyopaque,

    pub fn ensure_model(self: *DefaultModelProvider, model: ModelId) (RasterizeError||error{OutOfMemory})![]u8 {
        const _result = c.xberg_default_model_provider_ensure_model(@as(*c.XBERGDefaultModelProvider, @ptrCast(self._handle)), model);
        if (_result == null) {
            return _first_error(RasterizeError);
        }
        return blk: {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *DefaultModelProvider) void {
        c.xberg_default_model_provider_free(@as(*c.XBERGDefaultModelProvider, @ptrCast(self._handle)));
    }
};

/// Both views of the fused output: citation-wrapped and flattened.
pub const CitationOutput = struct {
    _handle: *anyopaque,

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *CitationOutput) void {
        c.xberg_citation_output_free(@as(*c.XBERGCitationOutput, @ptrCast(self._handle)));
    }
};

/// Built prompt components ready to send to the vision model.
pub const BuiltPrompt = struct {
    _handle: *anyopaque,

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *BuiltPrompt) void {
        c.xberg_built_prompt_free(@as(*c.XBERGBuiltPrompt, @ptrCast(self._handle)));
    }
};

pub fn default_chunker_config() ChunkerConfig {
    const _handle = c.xberg_chunker_config_default();
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.XBERGChunkerConfig, @ptrCast(_handle.?)) };
}
...
pub const ChunkerConfig = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *ChunkerConfig) void {
        c.xberg_chunker_config_free(@as(*c.XBERGChunkerConfig, @ptrCast(self._handle)));
    }
};
...
pub const Batch = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *Batch) void {
        c.xberg_batch_free(@as(*c.XBERGBatch, @ptrCast(self._handle)));
    }
};
...
pub const PageImage = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *PageImage) void {
        c.xberg_page_image_free(@as(*c.XBERGPageImage, @ptrCast(self._handle)));
    }
};

/// Merged structured output plus validation bookkeeping.
pub const MergedOutput = struct {
    _handle: *anyopaque,

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *MergedOutput) void {
        c.xberg_merged_output_free(@as(*c.XBERGMergedOutput, @ptrCast(self._handle)));
    }
};
...
pub fn from_bytes_parsed_document(source: []const u8) ParsedDocument {
    const _handle = c.xberg_parsed_document_from_bytes(source.ptr, source.len);
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.XBERGParsedDocument, @ptrCast(_handle.?)) };
}
...
pub const ParsedDocument = struct {
    ...
    pub fn mime_type(self: *ParsedDocument) error{OutOfMemory}![]u8 {
        const _result = c.xberg_parsed_document_mime_type(@as(*c.XBERGParsedDocument, @ptrCast(self._handle)));
        return blk: {
            const slice = std.mem.span(_result);
            const owned = try std.heap.c_allocator.dupe(u8, slice);
            c.xberg_free_string(_result);
    ...
    pub fn page_count(self: *ParsedDocument) u32 {
        const _result = c.xberg_parsed_document_page_count(@as(*c.XBERGParsedDocument, @ptrCast(self._handle)));
        return _result;
    }

    /// The shared source bytes.
    pub fn source(self: *ParsedDocument) error{OutOfMemory}![]u8 {
        var _out_ptr: [*c]u8 = undefined;
        var _out_len: usize = 0;
        var _out_cap: usize = 0;
        _ = c.xberg_parsed_document_source(@as(*c.XBERGParsedDocument, @ptrCast(self._handle)), &_out_ptr, &_out_len, &_out_cap);
        const _owned = try std.heap.c_allocator.dupe(u8, _out_ptr[0.._out_len]);
        c.xberg_free_bytes(_out_ptr, _out_len, _out_cap);
        return _owned;
    }
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *ParsedDocument) void {
        c.xberg_parsed_document_free(@as(*c.XBERGParsedDocument, @ptrCast(self._handle)));
    }
};

/// Start building an `Engine`.
pub fn builder_engine() Engine {
    const _handle = c.xberg_engine_builder();
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.XBERGEngine, @ptrCast(_handle.?)) };
}

/// Construct an `Engine` with default configuration.
pub fn new_default_engine() Engine {
    const _handle = c.xberg_engine_new_default();
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.XBERGEngine, @ptrCast(_handle.?)) };
}
...
pub const Engine = struct {
    ...
    /// Extract content from a single bytes or URI input.
    pub fn extract(self: *Engine, input: []const u8, config: []const u8) (RasterizeError||error{OutOfMemory})![]u8 {
        const input_z = try std.heap.c_allocator.dupeZ(u8, input);
        defer std.heap.c_allocator.free(input_z);
        const input_handle = c.xberg_extract_input_from_json(input_z.ptr);
        if (input_handle == null) return _first_error(RasterizeError);
        defer c.xberg_extract_input_free(input_handle);
        const config_z = try std.heap.c_allocator.dupeZ(u8, config);
        defer std.heap.c_allocator.free(config_z);
        const config_handle = c.xberg_extraction_config_from_json(config_z.ptr);
        if (config_handle == null) return _first_error(RasterizeError);
        defer c.xberg_extraction_config_free(config_handle);
        const _result = c.xberg_engine_extract(@as(*c.XBERGEngine, @ptrCast(self._handle)), input_handle, config_handle);
        if (_result == null) {
            return _first_error(RasterizeError);
        }
        return blk: {
    ...
    /// Extract content from multiple bytes or URI inputs.
    pub fn extract_batch(self: *Engine, inputs: []const u8, config: []const u8) (RasterizeError||error{OutOfMemory})![]u8 {
        const inputs_z = try std.heap.c_allocator.dupeZ(u8, inputs);
        defer std.heap.c_allocator.free(inputs_z);
        const config_z = try std.heap.c_allocator.dupeZ(u8, config);
        defer std.heap.c_allocator.free(config_z);
        const config_handle = c.xberg_extraction_config_from_json(config_z.ptr);
        if (config_handle == null) return _first_error(RasterizeError);
        defer c.xberg_extraction_config_free(config_handle);
        const _result = c.xberg_engine_extract_batch(@as(*c.XBERGEngine, @ptrCast(self._handle)), inputs_z, config_handle);
        if (_result == null) {
            return _first_error(RasterizeError);
        }
        return blk: {
...
pub const EngineBuilder = struct {
    ...
    pub fn build(self: *EngineBuilder) Engine {
        const _result = c.xberg_engine_builder_build(@as(*c.XBERGEngineBuilder, @ptrCast(self._handle)));
        return Engine{ ._handle = _result.? };
    }

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *EngineBuilder) void {
        c.xberg_engine_builder_free(@as(*c.XBERGEngineBuilder, @ptrCast(self._handle)));
    }
};
...
pub fn from_local_candle_backend(model_dir: []const u8, lora_adapter_dir: ?[]const u8) error{OutOfMemory}!CandleBackend {
    const model_dir_z = try std.heap.c_allocator.dupeZ(u8, model_dir);
    defer std.heap.c_allocator.free(model_dir_z);
    const lora_adapter_dir_z = try std.heap.c_allocator.dupeZ(u8, lora_adapter_dir);
    defer std.heap.c_allocator.free(lora_adapter_dir_z);
    const _handle = c.xberg_candle_backend_from_local(model_dir_z.ptr, lora_adapter_dir_z.ptr);
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.XBERGCandleBackend, @ptrCast(_handle.?)) };
}
...
pub fn from_bytes_candle_backend(safetensors: []const u8, tokenizer_json: []const u8, encoder_config_json: []const u8) CandleBackend {
    const _handle = c.xberg_candle_backend_from_bytes(safetensors.ptr, safetensors.len, tokenizer_json.ptr, tokenizer_json.len, encoder_config_json.ptr, encoder_config_json.len);
    if (_handle == null) return _first_error(anyerror);
    return .{ ._handle = @as(*c.XBERGCandleBackend, @ptrCast(_handle.?)) };
}
...
pub const CandleBackend = struct {
    _handle: *anyopaque,

    pub fn detect(self: *CandleBackend, text: []const u8, categories: []const u8) (RasterizeError||error{OutOfMemory})![]u8 {
        ...
        const _result = c.xberg_candle_backend_detect(@as(*c.XBERGCandleBackend, @ptrCast(self._handle)), text_z, categories_z);
        if (_result == null) {
            return _first_error(RasterizeError);
        }
        return blk: {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *CandleBackend) void {
        c.xberg_candle_backend_free(@as(*c.XBERGCandleBackend, @ptrCast(self._handle)));
    }
};
...
pub const CustomGlinerSource = struct {
    _handle: *anyopaque,

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *CustomGlinerSource) void {
        c.xberg_custom_gliner_source_free(@as(*c.XBERGCustomGlinerSource, @ptrCast(self._handle)));
    }
};
...
pub const TextRedactionOutcome = struct {
    ...
    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *TextRedactionOutcome) void {
        c.xberg_text_redaction_outcome_free(@as(*c.XBERGTextRedactionOutcome, @ptrCast(self._handle)));
    }
};

/// Token → original PII text.
pub const RehydrationMap = struct {
    _handle: *anyopaque,

    /// Release the underlying FFI handle. Safe to call once per instance.
    pub fn free(self: *RehydrationMap) void {
        c.xberg_rehydration_map_free(@as(*c.XBERGRehydrationMap, @ptrCast(self._handle)));
    }
};
