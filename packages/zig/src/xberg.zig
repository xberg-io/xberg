...
pub fn extract_bytes(content: []const u8, mime_type: []const u8, config: []const u8) XbergError![]u8 {
    ...
    const _result = c.xberg_extract_bytes(content.ptr, content.len, mime_type_z, config_handle);
...
/// Returns `XbergError.UnsupportedFormat` if MIME type is not supported.
pub fn extract_file(path: []const u8, mime_type: ?[]const u8, config: []const u8) XbergError![]u8 {
    ...
    if (config_handle == null) return _error_with_message(XbergError);
    defer c.xberg_extraction_config_free(config_handle);
    const _result = c.xberg_extract_file(path_z, if (mime_type_z) |z| z.ptr else null, config_handle);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(XbergError);
    }
    if (_result == null) {
...
pub fn extract_file_sync(path: []const u8, mime_type: ?[]const u8, config: []const u8) XbergError![]u8 {
    const path_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{path}, 0);
    defer std.heap.c_allocator.free(path_z);
    const mime_type_z: ?[:0]u8 = if (mime_type) |v| try std.fmt.allocPrintSentinel(
    ...
    if (config_handle == null) return _error_with_message(XbergError);
    defer c.xberg_extraction_config_free(config_handle);
    const _result = c.xberg_extract_file_sync(path_z, if (mime_type_z) |z| z.ptr else null, config_handle);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(XbergError);
    }
    if (_result == null) {
...
pub fn extract_bytes_sync(content: []const u8, mime_type: []const u8, config: []const u8) XbergError![]u8 {
    const mime_type_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{mime_type}, 0);
    defer std.heap.c_allocator.free(mime_type_z);
    const config_z = try std.fmt.allocPrintSentinel(
    ...
    if (config_handle == null) return _error_with_message(XbergError);
    defer c.xberg_extraction_config_free(config_handle);
    const _result = c.xberg_extract_bytes_sync(content.ptr, content.len, mime_type_z, config_handle);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(XbergError);
    }
    if (_result == null) {
...
pub fn batch_extract_files_sync(items: []const u8, config: []const u8) XbergError![]u8 {
    ...
    if (config_handle == null) return _error_with_message(XbergError);
    defer c.xberg_extraction_config_free(config_handle);
    const _result = c.xberg_batch_extract_files_sync(items_z, config_handle);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(XbergError);
    }
    if (_result == null) {
    ...
    const _result_len = c.xberg_batch_extract_files_sync_len(items_z, config_handle);
    return blk: {
...
pub fn batch_extract_bytes_sync(items: []const u8, config: []const u8) XbergError![]u8 {
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    const items_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{items}, 0);
    defer std.heap.c_allocator.free(items_z);
    ...
    if (config_handle == null) return _error_with_message(XbergError);
    defer c.xberg_extraction_config_free(config_handle);
    const _result = c.xberg_batch_extract_bytes_sync(items_z, config_handle);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(XbergError);
    }
    if (_result == null) {
    ...
    const _result_len = c.xberg_batch_extract_bytes_sync_len(items_z, config_handle);
    return blk: {
        if (_result == null) return _first_error(XbergError);
        const slice = _result[0.._result_len];
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
///
/// Per-file configuration overrides:
pub fn batch_extract_files(items: []const u8, config: []const u8) XbergError![]u8 {
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    const items_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{items}, 0);
    defer std.heap.c_allocator.free(items_z);
    ...
    if (config_handle == null) return _error_with_message(XbergError);
    defer c.xberg_extraction_config_free(config_handle);
    const _result = c.xberg_batch_extract_files(items_z, config_handle);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(XbergError);
    }
    if (_result == null) {
    ...
    const _result_len = c.xberg_batch_extract_files_len(items_z, config_handle);
    return blk: {
        if (_result == null) return _first_error(XbergError);
        const slice = _result[0.._result_len];
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub fn batch_extract_bytes(items: []const u8, config: []const u8) XbergError![]u8 {
    // Vec/Map parameters are passed as JSON strings across the FFI boundary.
    const items_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{items}, 0);
    defer std.heap.c_allocator.free(items_z);
    ...
    if (config_handle == null) return _error_with_message(XbergError);
    defer c.xberg_extraction_config_free(config_handle);
    const _result = c.xberg_batch_extract_bytes(items_z, config_handle);
    if (c.xberg_last_error_code() != 0) {
        return _error_with_message(XbergError);
    }
    if (_result == null) {
    ...
    const _result_len = c.xberg_batch_extract_bytes_len(items_z, config_handle);
    return blk: {
        if (_result == null) return _first_error(XbergError);
        const slice = _result[0.._result_len];
        const owned = try std.heap.c_allocator.dupe(u8, slice);
