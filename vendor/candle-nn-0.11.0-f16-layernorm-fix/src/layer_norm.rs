...
impl Module for LayerNorm {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        // f16/wasm32 fix: the fused fast-path below (`crate::ops::layer_norm`)
        // does not go through the F32-upcast/downcast-back dance the manual
        // path a few lines down performs -- confirmed via a live browser
        // reproduction that running a DeBERTa-v2 encoder with F16 weights
        // (necessary on wasm32 to fit a large model's weights in linear
        // memory, see xberg-gliner-candle's streaming_load.rs) throws
        // "dtype mismatch in add, lhs: F32, rhs: F16" from inside this
        // module's forward pass. Skip the fused path for F16/BF16 so those
        // dtypes always take the manual path below, which IS dtype-correct
        // (upcasts to F32 for the mean/variance computation, then casts back
        // to x's original dtype before the final weight mul / bias add).
        // F32 (and other dtypes) keep using the fused path unchanged.
        let skip_fused_fast_path = matches!(x.dtype(), DType::F16 | DType::BF16);
        if !skip_fused_fast_path && x.is_contiguous() && self.remove_mean {
