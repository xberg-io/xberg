    ...
    pub fn forward(
        ...
        if self.config.position_biased_input {
            embeddings = embeddings
                ...
                .with_context(|| "DBG embeddings.position_biased add".to_string())?;
        }
        ...
        if self.config.type_vocab_size > 0 {
            embeddings = self
                ...
                .map_or_else(
                    ...
                    |token_type_embeddings| embeddings.add(&token_type_embeddings.forward(&token_type_ids)?),
                )
                .with_context(|| "DBG embeddings.token_type add".to_string())?;
        }
        ...
        embeddings = self
            .layer_norm
            .forward(&embeddings)
            .with_context(|| "DBG embeddings.layer_norm".to_string())?;
...
impl XSoftmax {
    pub fn apply(input: &Tensor, mask: &Tensor, dim: D, device: &Device) -> Result<Tensor> {
        ...
        // f16 fix: both fill tensors below were hardcoded to F32 regardless
        // of `input`'s actual dtype -- same bug pattern as the `score`
        // accumulator fix in disentangled_attention_bias above, confirmed
        // via the same debug instrumentation once that first fix let
        // execution reach this function. `where_cond`'s two value branches
        // must share a dtype; `input` (and the post-softmax `output`) run at
        // whatever dtype the model's weights are loaded at (F16 on wasm32).
...
impl DebertaV2DisentangledSelfAttention {
    ...
    pub fn forward(
        ...
        let mut attention_scores: Tensor = (|| -> Result<Tensor> {
        ...
        .with_context(|| "DBG dsa.base_attention_scores".to_string())?;
        ...
        if self.relative_attention {
            if let Some(rel_embeddings) = rel_embeddings {
                ...
                rel_att = Some(
                    self.disentangled_attention_bias(
                        query_layer,
                        key_layer,
                        relative_pos,
                        rel_embeddings,
                    ...
                    .with_context(|| "DBG dsa.disentangled_attention_bias".to_string())?,
                );
            }
        }
        ...
        if let Some(rel_att) = rel_att {
            attention_scores = attention_scores
                .broadcast_add(&rel_att)
                .with_context(|| "DBG dsa.scores+rel_att add".to_string())?;
        }
        ...
        let mut attention_probs = XSoftmax::apply(&attention_scores, attention_mask, D::Minus1, &self.device)
            .with_context(|| "DBG dsa.xsoftmax".to_string())?;
    ...
    fn disentangled_attention_bias(
        ...
        // f16 fix: this accumulator was hardcoded to F32 regardless of the
        // model's actual working dtype -- confirmed via targeted debug
        // instrumentation (see the DBG-prefixed .with_context() labels
        // throughout this function) that this is the exact site of
        // "dtype mismatch in add, lhs: F32, rhs: F16": `score` (F32) later
        // gets `broadcast_add`ed with `c2p_att`/`p2c_att` (both F16, since
        // they derive from `query_layer`/`key_layer`/`pos_key_layer`, all
        // loaded at F16 by xberg-gliner-candle on wasm32). Cast to
        // query_layer's dtype so `score` matches whatever dtype the rest of
        // this computation actually runs at.
...
impl DebertaV2Attention {
    ...
    fn forward(
        ...
        let self_output = self
            ...
            .with_context(|| "DBG attention.dsa".to_string())?;

        self.output
            .forward(&self_output, query_states.unwrap_or(hidden_states))
            .with_context(|| "DBG attention.output".to_string())
    }
}
...
impl DebertaV2Layer {
    ...
    fn forward(
        ...
        let attention_output = self
            ...
            .with_context(|| "DBG layer.attention".to_string())?;

        let intermediate_output = self
            .intermediate
            .forward(&attention_output)
            .with_context(|| "DBG layer.intermediate".to_string())?;

        let layer_output = self
            .output
            .forward(&intermediate_output, &attention_output)
            .with_context(|| "DBG layer.output".to_string())?;
    ...
    pub fn forward(
        ...
        for (i, layer_module) in self.layer.iter().enumerate() {
            ...
            output_states = layer_module
                ...
                .with_context(|| format!("DBG layer[{i}].forward"))?;
