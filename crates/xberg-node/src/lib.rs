...
impl From<JsOcrExtractionResult> for xberg::OcrExtractionResult {
    fn from(val: JsOcrExtractionResult) -> Self {
        let _ = val;
        compile_error!(
            "alef cannot generate From<JsOcrExtractionResult> for xberg::OcrExtractionResult: the core type has private fields and does not implement Default, so it cannot be constructed from a binding value. Derive Default on xberg::OcrExtractionResult (or expose a public constructor / exclude it from this backend)."
        )
    }
}
