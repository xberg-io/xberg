...
pub const EMBEDDINGMODELTYPE_PLUGIN: &str = "Plugin";
...
impl KreuzbergApi {
    ...
    pub fn batch_extract_file_sync(items: Vec<String>, config: &ExtractionConfig) -> PhpResult<Vec<ExtractionResult>> {
        let items_core: Vec<_> = items
            .into_iter()
            .map(|s| {
                serde_json::from_str::<_>(&s).map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))
            })
            .collect::<Result<Vec<_>, _>>()?;
        let config_json = serde_json::to_string(&config).map_err(|e| format!("{e}"))?;
        let config_core: kreuzberg::ExtractionConfig =
            serde_json::from_str(&config_json).map_err(|e| format!("{e}"))?;
        let result = kreuzberg::batch_extract_file_sync(items_core, &config_core)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
