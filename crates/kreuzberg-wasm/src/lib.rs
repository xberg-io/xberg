...
// alef:hash:a29debfc01ed20d8bbee734d1acb1443eb81d40a0e01ceb44c61043cc4fb63ed
...
pub fn clear_embedding_backends() -> Result<(), JsValue> {
    let result = kreuzberg::clear_embedding_backends().map_err(|e| JsValue::from_str(&e.to_string()))?;
    Ok(result)
}
...
pub fn clear_document_extractors() -> Result<(), JsValue> {
    let result = kreuzberg::clear_document_extractors().map_err(|e| JsValue::from_str(&e.to_string()))?;
    Ok(result)
}
...
pub fn clear_renderers() -> Result<(), JsValue> {
    let result = kreuzberg::clear_renderers().map_err(|e| JsValue::from_str(&e.to_string()))?;
    Ok(result)
}
