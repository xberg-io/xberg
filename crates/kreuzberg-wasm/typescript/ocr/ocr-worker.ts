...
async function initWasm(wasmGluePath: string, wasmBinary?: ArrayBufferLike): Promise<void> {
	const glue = (await import(/* @vite-ignore */ /* webpackIgnore: true */ wasmGluePath)) as OcrWasm;
...
async function bootstrap(): Promise<void> {
	...
	if (isNodeEnv) {
		const { parentPort, workerData } = await import(/* @vite-ignore */ /* webpackIgnore: true */ "node:worker_threads");
