	...
	return new Promise<string>((resolve, reject) => {
		const timer = setTimeout(() => {
			if (pendingRequests.has(id)) {
				pendingRequests.delete(id);
				// Reject remaining pending requests and forcibly terminate the worker.
				// Without termination the worker continues spinning at 100% CPU
				// (Tesseract PSM AUTO hangs on near-uniform images) which triggers
				// "Page Unresponsive" dialogs in Chrome and Firefox.
				const err = new Error(
					"OCR timed out after 20s — Tesseract hung on this image. " +
					"Try a different image or a text-based file format.",
				);
				for (const pending of pendingRequests.values()) {
					pending.reject(err);
				}
				pendingRequests.clear();
				terminateOcrWorker().catch(() => undefined);
				reject(err);
			}
		}, 20_000);

		pendingRequests.set(id, {
			resolve: (text: string) => { clearTimeout(timer); resolve(text); },
			reject: (errVal: Error) => { clearTimeout(timer); reject(errVal); },
		});
