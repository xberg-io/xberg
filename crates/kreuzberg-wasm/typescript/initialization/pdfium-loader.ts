...
async function loadPdfiumForNode(): Promise<Record<string, unknown> | null> {
	try {
		const fs = await import(/* @vite-ignore */ /* webpackIgnore: true */ "node:fs/promises");
		const path = await import(/* @vite-ignore */ /* webpackIgnore: true */ "node:path");
		const url = await import(/* @vite-ignore */ /* webpackIgnore: true */ "node:url");
		...
		for (const candidate of candidates) {
			try {
				...
				return (await import(/* @vite-ignore */ /* webpackIgnore: true */ moduleUrl)) as Record<string, unknown>;
			} catch {
