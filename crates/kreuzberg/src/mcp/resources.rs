...
//!
//! Exposes kreuzberg metadata (supported formats, model manifest, OCR languages,
...
pub const URI_FORMATS: &str = "kreuzberg://formats";

/// URI for the model manifest resource.
pub const URI_MODELS: &str = "kreuzberg://models";

/// URI for the OCR languages resource.
pub const URI_OCR_LANGUAGES: &str = "kreuzberg://languages/ocr";

/// URI for the embedding presets resource (requires `embeddings` feature).
#[cfg(feature = "embeddings")]
pub const URI_EMBEDDING_PRESETS: &str = "kreuzberg://presets/embeddings";
...
pub fn list_resources() -> ListResourcesResult {
    ...
    let mut resources = vec![
        RawResource::new(URI_FORMATS, "Supported Formats")
            .with_description("All document formats and MIME types supported by Kreuzberg")
    ...
    match uri {
        ...
        URI_MODELS => {
            ...
            let payload = serde_json::json!({
                "kreuzberg_version": env!("CARGO_PKG_VERSION"),
                "models": entries,
            });
...
mod tests {
    ...
    fn test_read_resource_models_is_valid_json() {
        ...
        if let ResourceContents::TextResourceContents { text, .. } = &result.contents[0] {
            ...
            assert!(v.get("kreuzberg_version").is_some());
    ...
    fn test_read_resource_unknown_uri_errors() {
        let result = read_resource("kreuzberg://unknown");
