...
//!    ```sh
//!    KREUZBERG_HUNYUAN_OCR_MODEL_PATH=/path/to/model \
//!        cargo test -p kreuzberg-candle-ocr --features hunyuan-ocr \
//!                   --test hunyuan_ocr_integration -- --ignored --nocapture
//!    ```

#![cfg(feature = "hunyuan-ocr")]

use kreuzberg_candle_ocr::models::hunyuan_ocr::HunyuanOCREngine;
...
/// Skips gracefully when `KREUZBERG_HUNYUAN_OCR_MODEL_PATH` is unset.
#[test]
...
fn hunyuan_ocr_extracts_text_from_sample_image() {
    let model_path = match std::env::var("KREUZBERG_HUNYUAN_OCR_MODEL_PATH") {
        ...
        _ => {
            println!(
                "KREUZBERG_HUNYUAN_OCR_MODEL_PATH is not set — skipping integration test.\n\
