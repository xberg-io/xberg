...
- **fix(ocr)**: fall back to natural input order when Y-position sort is visually inconsistent. When sorting page elements by vertical position produces jumps > 50 points backward (indicating reordering), the sort is reverted to preserve natural input order, preventing layout scrambling on OCR-rendered pages. This fixes category F (layout reordering) from the Paddle quality regression audit. Added `is_sort_visually_consistent` validation and unit test to detect sort violations.
