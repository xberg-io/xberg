...
- **(ocr): `force_ocr` renders each page batch in parallel.** The route rendered the pages of a batch one after another on one thread and only then ran OCR on the batch across the configured threads, so on a 40-page scan at `max_threads` 8 it took 17.9 s against 9.5 s for `force_ocr_pages` on the same pages, and raising `max_threads` to 16 changed nothing. The batch now renders through the same parallel per-page body the `force_ocr_pages` route already used, and the two routes share that body so their render settings cannot drift apart again. Output text and page order are unchanged. (GH#1796)
