...
- **html/chunking**: Fix content duplication when extracting deeply nested mixed lists (`ul > li > ul > li > ol`). `html-to-markdown-rs` ≤3.4.1 incorrectly included rendered nested-list Markdown in the parent list item text, causing each inner item to appear multiple times in the extracted content. Fixed by upgrading to `html-to-markdown-rs` 3.5.0 (kreuzberg-dev/html-to-markdown#385). The same malformed output also triggered an integer-underflow panic in the Markdown chunker; both issues are resolved together. (#1004)
