...
- **(pdf): a page's extracted text no longer depends on the order pages are read.** Reading a document's pages concurrently could return different text on different runs; extraction now returns the same text every time, whatever order the pages are read in. (GH#1725)
