...
- **alef.toml**: bumped `alef_version` to `0.22.1` and regenerated bindings. Picks up alef codegen fixes: WASM `Vec::<T>::from(result)` turbofish (was `Vec<T>::from(result)`, invalid Rust); Dart FRB bridge `is_mut` propagation for Named/opaque/`Vec<Named>` parameters (translate_result, apply_strategy); Dart `Vec<Named>` is_ref slice-from-raw-parts (was raw `*const T`); Dart `collect::<Vec<_>>()` turbofish in `return_transmute_expr` (E0282 inference fix); Dart `Vec<PathBuf>` cast uses `.to_string_lossy().into_owned()` (PathBuf doesn't implement Display); FFI `is_mut` Named-param conversion template + call-sites; FFI stubs for `Vec<Named>` / `Map<*,Named>` returns when Named lacks `serde::Serialize`. Remaining FFI codegen bugs (`*const` → `*mut` pointer upgrade, double-mut-borrow at call site, enum-i32 local naming) are tracked separately and will land in a follow-up alef + kreuzberg regen.
