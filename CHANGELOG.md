...
- **(ocr): a multi-word row label no longer makes table detection drop the whole table.** OCR word grouping can leave the last word of a label such as "Closing stock balance" in a column of its own, filled in one row and headed by nothing, and the column-sparsity gate then rejected the entire table, so a scanned table page came back with no table at all. Such a column is now folded back into the label column before the gate runs, when it has no header, is empty in more than three data rows of four, and holds no digit. A sparse column that carries numbers is still rejected as before, and layout-guided tables are unchanged. (GH#1797)
