...
- **#844**: Python wrapper `extract_*` functions no longer crash on every call. Picked up upstream alef 0.11.24+ codegen fixes (kreuzberg-dev/alef#44): `#[serde(skip)]` is propagated to wrapper structs (no more `unknown field 'cancel_token'`), `api.py` wrappers forward arguments by keyword (no more `extract_file` mime_type/config arg-reorder `TypeError`), async pyo3 functions emit `async def` + `await`, and trait-bridge `register_*` helpers are re-exported through `api.py` / `__init__.py` `__all__`.
- **#845**: Demo and `@kreuzberg/wasm` OCR no longer hang indefinitely on small or near-uniform images (e.g. a 320×180 gradient PNG). Root cause: Tesseract's default `PSM_AUTO` layout analysis enters an infinite loop on images with little or no text content; `runOcrInWorker` posted to the OCR Web Worker and waited for a response with no timeout, so any Tesseract spin locked the page. Fix: `runOcrInWorker` in `worker-bridge.ts` now races the worker response against a 30-second `setTimeout`; on expiry the pending request is removed and rejected with a descriptive error message. The live demo additionally wraps `extractFromFile` in the same 30-second client-side timeout and surfaces a user-readable error instead of silently freezing. The CDN version-pinning from PR #847 (fetch `package.json` first, then load `index.js` and `.wasm` from the same resolved version) is retained as a defence against future split-cache ABI mismatches.
