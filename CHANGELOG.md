...
- **#962**: PDF text extracted one character per line on glyph-spaced documents — when a PDF (e.g. Word-exported broken-image placeholder text) positions each glyph via its own `BT…ET` block with a sinusoidal y-coordinate jitter, the pdf_oxide backend emits one single-character span per glyph with near-zero bbox height; the same-line heuristic (threshold = height × 0.5) then collapses to zero and every glyph starts a new line. The extraction path now detects the fragmentation signature (≥ 5 consecutive single-character spans with bbox height < 1 pt) and rebuilds page text from span positions: spans are sorted by y-descending, grouped by chained y-proximity (4 pt tolerance), re-sorted by x-ascending within each group, and spaces are inserted at word gaps (x-gap > font_size × 0.25). The same-line heuristic is also hardened with a font_size fallback for isolated zero-height spans that fall below the detection threshold.
