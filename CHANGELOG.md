...
- **(ocr): a split multi-word row label no longer makes table detection drop the whole table.** OCR word grouping can leave the last word of a row label in a column of its own beside the labels, filled in one or two rows and headed by nothing, and the column-sparsity gate then rejected the entire table, so a scanned table page came back with no table at all. That column now joins the labels before the gate runs, but only when it fails the same sparsity rule and every filled cell holds letters and no digit. Every other sparse column, and a column beside the labels that holds a number, still rejects the table as before. Layout-guided tables are unchanged. (GH#1797)
