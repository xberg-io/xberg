...
import { Content as Snip_cli_install_scoop } from "../../../snippets/cli/install_scoop.md";
...
Manage the grammar cache used for code intelligence. This command is available when the CLI is
built with the `tree-sitter` feature.
...
| `--max-concurrent-ocr <N>` | Set concurrent Tesseract recognition sessions on their own. Applied as given, not capped by `--max-threads`. The first extraction in a process fixes it for that process. |
