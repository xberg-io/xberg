...
<!-- alef:hash:2596ee8772edca853a4887da984a96e30a05525c2ce8cfa71a3f3f6d93b34c02 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Elixir API Reference <span class="version-badge">v1.2.6</span>
...
```elixir
@spec list_ocr_backend_capabilities() :: {:ok, term()} | {:error, term()}
def list_ocr_backend_capabilities()
```

**Example:**

```elixir
{:ok, result} = list_ocr_backend_capabilities()
```
...
```elixir
@spec ocr_backend_supports_language(backend, language) :: {:ok, term()} | {:error, term()}
def ocr_backend_supports_language(backend, language)
```

**Example:**

```elixir
{:ok, result} = ocr_backend_supports_language("value", "value")
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `backend` | `String.t()` | Yes | Name of a registered OCR backend, as returned by `list_ocr_backends`. Lookup is case-insensitive and resolves the same `paddleocr` alias as backend dispatch. |
| `language` | `String.t()` | Yes | Language code to check (e.g. `"eng"`, `"deu"`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model_path` | `String.t() \| nil` | `nil` | Local DeepSeek-OCR model directory. Takes precedence over `model_id` when present. |
| `model_id` | `String.t() \| nil` | `nil` | Optional Hugging Face repository identifier. Defaults to the checksum-pinned `deepseek-ai/DeepSeek-OCR`. Ignored when `model_path` is set. |
| `hf_revision` | `String.t() \| nil` | `nil` | Optional immutable Hugging Face model revision. The default model is pinned automatically; a custom `model_id` requires this to be set explicitly. |
...
| `dtype` | `CandleDeepseekOcrDtype \| nil` | `nil` | Optional weight precision override; see `CandleDeepseekOcrDtype`. |
...
| `ocr_aggregate` | `float() \| nil` | `nil` | OCR recognition confidence, word-count-weighted across every recognized word, when OCR ran; `nil` when it did not. |
...
| `security_limits` | `SecurityLimits \| nil` | `nil` | Security limits applied when decoding raw image bytes for OCR (GH#1554). Not user-configurable via config files — injected at runtime from `ExtractionConfig.security_limits` before each `process_image` call, the same pattern `Self.acceleration` uses. `ExtractionConfig.security_limits` remains the source of truth and overwrites this field whenever it carries a value; a value set here directly is honoured only when `ExtractionConfig` carries none, so the two cannot drift while a directly-set limit is no longer silently discarded (GH#1651). A backend consulting a `backend_options` override for this call may still let that override win, but in the absence of one this field is what backends should fall back to instead of `SecurityLimits.default()`. `nil` means "use `SecurityLimits.default()`", never "disable the check". |
...
| Value | Description |
|-------|-------------|
| `pdf` | Metadata extracted from a PDF document. — Fields (flattened into the tagged object): `pdf_version`: `String.t()`, `producer`: `String.t()`, `is_encrypted`: `boolean()`, `width`: `integer()`, `height`: `integer()`, `page_count`: `integer()`, `scanned_confidence`: `float()`, `scanned_pages`: `list(integer())`, `fabricated_text_pages`: `list(integer())`, `implausible_text_pages`: `list(integer())`, `layout_gated_pages`: `list(integer())`, `layout_gate_reasons`: `list(String.t())` |
| `docx` | Metadata extracted from a DOCX Word document. — Fields (flattened into the tagged object): `core_properties`: `CoreProperties`, `app_properties`: `DocxAppProperties`, `custom_properties`: `map()` |
| `excel` | Metadata extracted from an Excel spreadsheet. — Fields (flattened into the tagged object): `sheet_count`: `integer()`, `sheet_names`: `list(String.t())` |
| `email` | Metadata extracted from an email message (EML/MSG). — Fields (flattened into the tagged object): `from_email`: `String.t()`, `from_name`: `String.t()`, `to_emails`: `list(String.t())`, `cc_emails`: `list(String.t())`, `bcc_emails`: `list(String.t())`, `message_id`: `String.t()`, `attachments`: `list(String.t())` |
| `pptx` | Metadata extracted from a PowerPoint presentation. — Fields (flattened into the tagged object): `slide_count`: `integer()`, `slide_names`: `list(String.t())`, `image_count`: `integer()`, `table_count`: `integer()` |
| `archive` | Metadata extracted from an archive (ZIP, TAR, 7Z, etc.). — Fields (flattened into the tagged object): `format`: `String.t()`, `file_count`: `integer()`, `file_list`: `list(String.t())`, `total_size`: `integer()`, `compressed_size`: `integer()` |
| `image` | Metadata extracted from a raster or vector image. — Fields (flattened into the tagged object): `width`: `integer()`, `height`: `integer()`, `format`: `String.t()`, `exif`: `map()` |
| `xml` | Metadata extracted from an XML document. — Fields (flattened into the tagged object): `element_count`: `integer()`, `unique_elements`: `list(String.t())` |
| `text` | Metadata extracted from a plain-text file. — Fields (flattened into the tagged object): `line_count`: `integer()`, `word_count`: `integer()`, `character_count`: `integer()`, `headers`: `list(String.t())`, `links`: `list(MarkdownLink)`, `code_blocks`: `list(MarkdownCodeBlock)` |
| `html` | Metadata extracted from an HTML document. — Fields (flattened into the tagged object): `title`: `String.t()`, `description`: `String.t()`, `keywords`: `list(String.t())`, `author`: `String.t()`, `canonical_url`: `String.t()`, `base_href`: `String.t()`, `language`: `String.t()`, `text_direction`: `TextDirection`, `open_graph`: `map()`, `twitter_card`: `map()`, `meta_tags`: `map()`, `headers`: `list(HeaderMetadata)`, `links`: `list(LinkMetadata)`, `images`: `list(ImageMetadataType)`, `structured_data`: `list(StructuredData)` |
| `ocr` | Metadata produced by an OCR pipeline. — Fields (flattened into the tagged object): `language`: `String.t()`, `psm`: `integer()`, `output_format`: `String.t()`, `table_count`: `integer()`, `table_rows`: `integer()`, `table_cols`: `integer()` |
| `csv` | Metadata extracted from a CSV or TSV file. — Fields (flattened into the tagged object): `row_count`: `integer()`, `column_count`: `integer()`, `delimiter`: `String.t()`, `has_header`: `boolean()`, `column_types`: `list(String.t())` |
| `bibtex` | Metadata extracted from a BibTeX bibliography file. — Fields (flattened into the tagged object): `entry_count`: `integer()`, `citation_keys`: `list(String.t())`, `authors`: `list(String.t())`, `year_range`: `YearRange`, `entry_types`: `map()` |
| `citation` | Metadata extracted from a citation file (RIS, PubMed, EndNote). — Fields (flattened into the tagged object): `citation_count`: `integer()`, `format`: `String.t()`, `authors`: `list(String.t())`, `year_range`: `YearRange`, `dois`: `list(String.t())`, `keywords`: `list(String.t())` |
| `fiction_book` | Metadata extracted from a FictionBook (FB2) e-book. — Fields (flattened into the tagged object): `genres`: `list(String.t())`, `sequences`: `list(String.t())`, `annotation`: `String.t()` |
| `dbf` | Metadata extracted from a dBASE (DBF) database file. — Fields (flattened into the tagged object): `record_count`: `integer()`, `field_count`: `integer()`, `fields`: `list(DbfFieldInfo)` |
| `jats` | Metadata extracted from a JATS (Journal Article Tag Suite) XML file. — Fields (flattened into the tagged object): `copyright`: `String.t()`, `license`: `String.t()`, `history_dates`: `map()`, `contributor_roles`: `list(ContributorRole)` |
| `epub` | Metadata extracted from an EPUB e-book. — Fields (flattened into the tagged object): `coverage`: `String.t()`, `dc_format`: `String.t()`, `relation`: `String.t()`, `source`: `String.t()`, `dc_type`: `String.t()`, `cover_image`: `String.t()` |
| `pst` | Metadata extracted from an Outlook PST archive. — Fields (flattened into the tagged object): `message_count`: `integer()` |
| `audio` | Metadata extracted from an audio or video file. — Fields (flattened into the tagged object): `duration_ms`: `integer()`, `codec`: `String.t()`, `container`: `String.t()`, `sample_rate_hz`: `integer()`, `channels`: `integer()`, `bitrate`: `integer()` |
...
| Value | Description |
|-------|-------------|
| `context` | Unchanged context line. — Fields: `text`: `String.t()` |
| `added` | Line added in the "after" version. — Fields: `text`: `String.t()` |
| `removed` | Line removed from the "before" version. — Fields: `text`: `String.t()` |
