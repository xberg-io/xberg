...
<!-- alef:hash:9e4c6c21213d7e8a6c1e51e6c0713fa0561f17f3b7fd33a8934f381c434e6eae -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## C# API Reference <span class="version-badge">v1.2.6</span>
...
```csharp
public static bool OcrBackendSupportsLanguage(string backend, string language)
```

**Example:**

```csharp
var result = XbergConverter.OcrBackendSupportsLanguage("value", "value");
```
...
| `OcrAggregate` | `float?` | `null` | OCR recognition confidence, word-count-weighted across every recognized word, when OCR ran; `null` when it did not. |
...
| `SecurityLimits` | `SecurityLimits?` | `null` | Security limits applied when decoding raw image bytes for OCR (GH#1554). Not user-configurable via config files — injected at runtime from `ExtractionConfig.security_limits` before each `process_image` call, the same pattern `Self.acceleration` uses. `ExtractionConfig.security_limits` remains the source of truth and overwrites this field whenever it carries a value; a value set here directly is honoured only when `ExtractionConfig` carries none, so the two cannot drift while a directly-set limit is no longer silently discarded (GH#1651). A backend consulting a `backend_options` override for this call may still let that override win, but in the absence of one this field is what backends should fall back to instead of `SecurityLimits.default()`. `null` means "use `SecurityLimits.default()`", never "disable the check". |
...
| `MaxContentSize` | `ulong` | `104857600` | Maximum string growth and decoded image allocation per operation (100 MB). Per-page passes such as layout detection charge each batch against this limit, not the whole document; only `max_pages` bounds the rasters retained across a document (GH#1721). |
...
| Value | Description |
|-------|-------------|
| `Pdf` | Metadata extracted from a PDF document. — Fields (flattened into the tagged object): `PdfVersion`: `string`, `Producer`: `string`, `IsEncrypted`: `bool`, `Width`: `long`, `Height`: `long`, `PageCount`: `uint`, `ScannedConfidence`: `float`, `ScannedPages`: `List<uint>`, `FabricatedTextPages`: `List<uint>`, `ImplausibleTextPages`: `List<uint>`, `LayoutGatedPages`: `List<uint>`, `LayoutGateReasons`: `List<string>` |
| `Docx` | Metadata extracted from a DOCX Word document. — Fields (flattened into the tagged object): `CoreProperties`: `CoreProperties`, `AppProperties`: `DocxAppProperties`, `CustomProperties`: `Dictionary<string, object>` |
| `Excel` | Metadata extracted from an Excel spreadsheet. — Fields (flattened into the tagged object): `SheetCount`: `uint`, `SheetNames`: `List<string>` |
...
| `Pptx` | Metadata extracted from a PowerPoint presentation. — Fields (flattened into the tagged object): `SlideCount`: `uint`, `SlideNames`: `List<string>`, `ImageCount`: `uint`, `TableCount`: `uint` |
| `Archive` | Metadata extracted from an archive (ZIP, TAR, 7Z, etc.). — Fields (flattened into the tagged object): `Format`: `string`, `FileCount`: `uint`, `FileList`: `List<string>`, `TotalSize`: `ulong`, `CompressedSize`: `ulong` |
| `Image` | Metadata extracted from a raster or vector image. — Fields (flattened into the tagged object): `Width`: `uint`, `Height`: `uint`, `Format`: `string`, `Exif`: `Dictionary<string, string>` |
| `Xml` | Metadata extracted from an XML document. — Fields (flattened into the tagged object): `ElementCount`: `uint`, `UniqueElements`: `List<string>` |
| `Text` | Metadata extracted from a plain-text file. — Fields (flattened into the tagged object): `LineCount`: `uint`, `WordCount`: `uint`, `CharacterCount`: `uint`, `Headers`: `List<string>`, `Links`: `List<MarkdownLink>`, `CodeBlocks`: `List<MarkdownCodeBlock>` |
| `Html` | Metadata extracted from an HTML document. — Fields (flattened into the tagged object): `Title`: `string`, `Description`: `string`, `Keywords`: `List<string>`, `Author`: `string`, `CanonicalUrl`: `string`, `BaseHref`: `string`, `Language`: `string`, `TextDirection`: `TextDirection`, `OpenGraph`: `Dictionary<string, string>`, `TwitterCard`: `Dictionary<string, string>`, `MetaTags`: `Dictionary<string, string>`, `Headers`: `List<HeaderMetadata>`, `Links`: `List<LinkMetadata>`, `Images`: `List<ImageMetadataType>`, `StructuredData`: `List<StructuredData>` |
| `Ocr` | Metadata produced by an OCR pipeline. — Fields (flattened into the tagged object): `Language`: `string`, `Psm`: `int`, `OutputFormat`: `string`, `TableCount`: `uint`, `TableRows`: `uint`, `TableCols`: `uint` |
| `Csv` | Metadata extracted from a CSV or TSV file. — Fields (flattened into the tagged object): `RowCount`: `uint`, `ColumnCount`: `uint`, `Delimiter`: `string`, `HasHeader`: `bool`, `ColumnTypes`: `List<string>` |
| `Bibtex` | Metadata extracted from a BibTeX bibliography file. — Fields (flattened into the tagged object): `EntryCount`: `ulong`, `CitationKeys`: `List<string>`, `Authors`: `List<string>`, `YearRange`: `YearRange`, `EntryTypes`: `Dictionary<string, ulong>` |
| `Citation` | Metadata extracted from a citation file (RIS, PubMed, EndNote). — Fields (flattened into the tagged object): `CitationCount`: `ulong`, `Format`: `string`, `Authors`: `List<string>`, `YearRange`: `YearRange`, `Dois`: `List<string>`, `Keywords`: `List<string>` |
| `FictionBook` | Metadata extracted from a FictionBook (FB2) e-book. — Fields (flattened into the tagged object): `Genres`: `List<string>`, `Sequences`: `List<string>`, `Annotation`: `string` |
| `Dbf` | Metadata extracted from a dBASE (DBF) database file. — Fields (flattened into the tagged object): `RecordCount`: `ulong`, `FieldCount`: `ulong`, `Fields`: `List<DbfFieldInfo>` |
| `Jats` | Metadata extracted from a JATS (Journal Article Tag Suite) XML file. — Fields (flattened into the tagged object): `Copyright`: `string`, `License`: `string`, `HistoryDates`: `Dictionary<string, string>`, `ContributorRoles`: `List<ContributorRole>` |
...
| `Pst` | Metadata extracted from an Outlook PST archive. — Fields (flattened into the tagged object): `MessageCount`: `ulong` |
| `Audio` | Metadata extracted from an audio or video file. — Fields (flattened into the tagged object): `DurationMs`: `ulong`, `Codec`: `string`, `Container`: `string`, `SampleRateHz`: `uint`, `Channels`: `ushort`, `Bitrate`: `uint` |
