...
<!-- alef:hash:4576af7ec88805c717ecd2180901a7b6f6e451e1bb8a736afd52426b49529f73 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Kotlin (Android) API Reference <span class="version-badge">v1.2.6</span>
...
```kotlin
val result = listOcrBackendCapabilities()
```
...
```kotlin
@Throws(XbergError::class)
fun ocrBackendSupportsLanguage(backend: String, language: String): Boolean
```

**Example:**

```kotlin
val result = ocrBackendSupportsLanguage("value", "value")
```
...
| Value | Description |
|-------|-------------|
| `Pdf` | Metadata extracted from a PDF document. — Fields (flattened into the tagged object): `pdfVersion`: `String`, `producer`: `String`, `isEncrypted`: `Boolean`, `width`: `Long`, `height`: `Long`, `pageCount`: `Int`, `scannedConfidence`: `Float`, `scannedPages`: `List<Int>`, `fabricatedTextPages`: `List<Int>`, `implausibleTextPages`: `List<Int>`, `layoutGatedPages`: `List<Int>`, `layoutGateReasons`: `List<String>` |
| `Docx` | Metadata extracted from a DOCX Word document. — Fields (flattened into the tagged object): `coreProperties`: `CoreProperties`, `appProperties`: `DocxAppProperties`, `customProperties`: `Map<String, Any>` |
