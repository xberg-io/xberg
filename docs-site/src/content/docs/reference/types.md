...
<!-- alef:hash:03ddd8ae21ea83b82b5ecb9233a693c5a532462a1760bc18fee43849f725c492 -->
...
| `links` | `Vec<MarkdownLink>` | `None` | Markdown links (Markdown files only). |
| `code_blocks` | `Vec<MarkdownCodeBlock>` | `None` | Code blocks (Markdown files only). |
...
See [Configuration Reference](./configuration/) for detailed defaults and language-specific representations.
...
| `providers` | `Vec<LlmProviderConfig>` | `vec![]` | Custom provider configurations, in addition to liter-llm's built-in providers. Mirrors liter-llm's `LlmConfig::providers`, for OpenAI-compatible gateways and self-hosted model servers that are not in the built-in provider catalog. |
...
| `backend_config` | `HashMap<String, String>` | `HashMap::new()` | Backend-specific configuration key/value pairs. |
...
| `model_limits` | `HashMap<String, f64>` | `HashMap::new()` | Per-model spend limits in USD, keyed by model name. |
...
| `languages` | `Vec<String>` | `None` | Languages to pre-download on init (e.g., `["python", "rust"]`). Consumed only by the CLI's `tree-sitter download --from-config` and `cache warm` commands as a pre-download hint. Extraction itself does not read this field: a given source file always processes with a single, already auto-detected language, so there is nothing for a language allowlist to gate at extraction time. |
| `groups` | `Vec<String>` | `None` | Language groups to pre-download (e.g., `["web", "systems", "scripting"]`). Consumed only by the CLI's `tree-sitter download --from-config` and `cache warm` commands, for the same reason as `languages` above. |
...
| `detected_language_confidences` | `Vec<LanguageConfidence>` | `vec![]` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `None` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
...
| `links` | `Vec<MarkdownLink>` | `vec![]` | Markdown links (for Markdown files). |
| `code_blocks` | `Vec<MarkdownCodeBlock>` | `vec![]` | Code blocks (for Markdown files). |
...
| `quad_points` | `Vec<BoundingBox>` | `/* serde(default) */` | Per-line bounding boxes derived from the annotation's `/QuadPoints` entry. Present for text markup annotations (Highlight, Underline, StrikeOut, Squiggly), one box per marked line/run of text. |
...
| Variant | Wire value | Description |
|---------|------------|-------------|
| `Context` | `context` | Unchanged context line. — Fields: `text`: `String` |
| `Added` | `added` | Line added in the "after" version. — Fields: `text`: `String` |
| `Removed` | `removed` | Line removed from the "before" version. — Fields: `text`: `String` |
...
| `Formula` | `formula` | Mathematical formula (LaTeX source in `text`) |
...
| Variant | Wire value | Description |
|---------|------------|-------------|
| `Pdf` | `pdf` | Metadata extracted from a PDF document. — Fields (flattened into the tagged object): `pdf_version`: `String`, `producer`: `String`, `is_encrypted`: `bool`, `width`: `i64`, `height`: `i64`, `page_count`: `u32`, `scanned_confidence`: `f32`, `scanned_pages`: `Vec<u32>`, `fabricated_text_pages`: `Vec<u32>`, `implausible_text_pages`: `Vec<u32>`, `layout_gated_pages`: `Vec<u32>`, `layout_gate_reasons`: `Vec<String>` |
| `Docx` | `docx` | Metadata extracted from a DOCX Word document. — Fields (flattened into the tagged object): `core_properties`: `CoreProperties`, `app_properties`: `DocxAppProperties`, `custom_properties`: `HashMap<String, serde_json::Value>` |
| `Excel` | `excel` | Metadata extracted from an Excel spreadsheet. — Fields (flattened into the tagged object): `sheet_count`: `u32`, `sheet_names`: `Vec<String>` |
| `Email` | `email` | Metadata extracted from an email message (EML/MSG). — Fields (flattened into the tagged object): `from_email`: `String`, `from_name`: `String`, `to_emails`: `Vec<String>`, `cc_emails`: `Vec<String>`, `bcc_emails`: `Vec<String>`, `message_id`: `String`, `attachments`: `Vec<String>` |
| `Pptx` | `pptx` | Metadata extracted from a PowerPoint presentation. — Fields (flattened into the tagged object): `slide_count`: `u32`, `slide_names`: `Vec<String>`, `image_count`: `u32`, `table_count`: `u32` |
| `Archive` | `archive` | Metadata extracted from an archive (ZIP, TAR, 7Z, etc.). — Fields (flattened into the tagged object): `format`: `String`, `file_count`: `u32`, `file_list`: `Vec<String>`, `total_size`: `u64`, `compressed_size`: `u64` |
| `Image` | `image` | Metadata extracted from a raster or vector image. — Fields (flattened into the tagged object): `width`: `u32`, `height`: `u32`, `format`: `String`, `exif`: `HashMap<String, String>` |
| `Xml` | `xml` | Metadata extracted from an XML document. — Fields (flattened into the tagged object): `element_count`: `u32`, `unique_elements`: `Vec<String>` |
| `Text` | `text` | Metadata extracted from a plain-text file. — Fields (flattened into the tagged object): `line_count`: `u32`, `word_count`: `u32`, `character_count`: `u32`, `headers`: `Vec<String>`, `links`: `Vec<MarkdownLink>`, `code_blocks`: `Vec<MarkdownCodeBlock>` |
| `Html` | `html` | Metadata extracted from an HTML document. — Fields (flattened into the tagged object): `title`: `String`, `description`: `String`, `keywords`: `Vec<String>`, `author`: `String`, `canonical_url`: `String`, `base_href`: `String`, `language`: `String`, `text_direction`: `TextDirection`, `open_graph`: `HashMap<String, String>`, `twitter_card`: `HashMap<String, String>`, `meta_tags`: `HashMap<String, String>`, `headers`: `Vec<HeaderMetadata>`, `links`: `Vec<LinkMetadata>`, `images`: `Vec<ImageMetadataType>`, `structured_data`: `Vec<StructuredData>` |
| `Ocr` | `ocr` | Metadata produced by an OCR pipeline. — Fields (flattened into the tagged object): `language`: `String`, `psm`: `i32`, `output_format`: `String`, `table_count`: `u32`, `table_rows`: `u32`, `table_cols`: `u32` |
| `Csv` | `csv` | Metadata extracted from a CSV or TSV file. — Fields (flattened into the tagged object): `row_count`: `u32`, `column_count`: `u32`, `delimiter`: `String`, `has_header`: `bool`, `column_types`: `Vec<String>` |
| `Bibtex` | `bibtex` | Metadata extracted from a BibTeX bibliography file. — Fields (flattened into the tagged object): `entry_count`: `usize`, `citation_keys`: `Vec<String>`, `authors`: `Vec<String>`, `year_range`: `YearRange`, `entry_types`: `HashMap<String, usize>` |
| `Citation` | `citation` | Metadata extracted from a citation file (RIS, PubMed, EndNote). — Fields (flattened into the tagged object): `citation_count`: `usize`, `format`: `String`, `authors`: `Vec<String>`, `year_range`: `YearRange`, `dois`: `Vec<String>`, `keywords`: `Vec<String>` |
| `FictionBook` | `fiction_book` | Metadata extracted from a FictionBook (FB2) e-book. — Fields (flattened into the tagged object): `genres`: `Vec<String>`, `sequences`: `Vec<String>`, `annotation`: `String` |
| `Dbf` | `dbf` | Metadata extracted from a dBASE (DBF) database file. — Fields (flattened into the tagged object): `record_count`: `usize`, `field_count`: `usize`, `fields`: `Vec<DbfFieldInfo>` |
| `Jats` | `jats` | Metadata extracted from a JATS (Journal Article Tag Suite) XML file. — Fields (flattened into the tagged object): `copyright`: `String`, `license`: `String`, `history_dates`: `HashMap<String, String>`, `contributor_roles`: `Vec<ContributorRole>` |
| `Epub` | `epub` | Metadata extracted from an EPUB e-book. — Fields (flattened into the tagged object): `coverage`: `String`, `dc_format`: `String`, `relation`: `String`, `source`: `String`, `dc_type`: `String`, `cover_image`: `String` |
| `Pst` | `pst` | Metadata extracted from an Outlook PST archive. — Fields (flattened into the tagged object): `message_count`: `usize` |
| `Audio` | `audio` | Metadata extracted from an audio or video file. — Fields (flattened into the tagged object): `duration_ms`: `u64`, `codec`: `String`, `container`: `String`, `sample_rate_hz`: `u32`, `channels`: `u16`, `bitrate`: `u32` |
...
| `MetadataBlock` | `metadata_block` | Structured metadata block (email headers, YAML frontmatter, etc.). — Fields: `entries`: `Vec<KeyValueAttribute>` |
...
| `Squiggly` | `squiggly` | Squiggly (wavy) underline text markup |
| `Ink` | `ink` | Freehand drawing (ink) annotation |
| `Square` | `square` | Rectangle/box shape annotation |
| `Circle` | `circle` | Ellipse/oval shape annotation |
| `Polygon` | `polygon` | Closed polygon shape annotation |
| `PolyLine` | `poly_line` | Open polyline shape annotation |
| `Line` | `line` | Line annotation |
| `Caret` | `caret` | Caret (text-insertion marker) annotation |
| `FileAttachment` | `file_attachment` | Embedded file attachment annotation |
| `Sound` | `sound` | Embedded sound annotation |
| `Movie` | `movie` | Embedded movie annotation |
...
| Variant | Wire value | Description |
|---------|------------|-------------|
| `Pass` | `pass` | The backend or setting will work as configured. |
| `Warn` | `warn` | The check ran and found something actionable, but nothing is broken (e.g. stray cache files, stale model revisions). Never fails the report. |
| `Fail` | `fail` | The configured setup will not work (or will silently degrade) on this host. |
| `Skip` | `skip` | The check cannot run locally (e.g. model not cached, feature not compiled in); first real use decides, possibly after a download. |
