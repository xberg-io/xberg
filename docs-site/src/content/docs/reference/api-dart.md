...
<!-- alef:hash:40b1f11d8c6ca99e801f450584ea2409e4ec815589a12a941620dcc478f3c211 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Dart API Reference <span class="version-badge">v1.2.6</span>
...
```dart
final result = listOcrBackendCapabilities();
```
...
```dart
Future<bool> ocrBackendSupportsLanguage(String backend, String language)
```

**Example:**

```dart
final result = ocrBackendSupportsLanguage("value", "value");
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `modelPath` | `String?` | `null` | Local DeepSeek-OCR model directory. Takes precedence over `model_id` when present. |
| `modelId` | `String?` | `null` | Optional Hugging Face repository identifier. Defaults to the checksum-pinned `deepseek-ai/DeepSeek-OCR`. Ignored when `model_path` is set. |
| `hfRevision` | `String?` | `null` | Optional immutable Hugging Face model revision. The default model is pinned automatically; a custom `model_id` requires this to be set explicitly. |
...
| `dtype` | `CandleDeepseekOcrDtype?` | `null` | Optional weight precision override; see `CandleDeepseekOcrDtype`. |
...
| `ocrAggregate` | `double?` | `null` | OCR recognition confidence, word-count-weighted across every recognized word, when OCR ran; `null` when it did not. |
...
| `maxContentSize` | `PlatformInt64` | `104857600` | Maximum string growth and decoded image allocation per operation (100 MB). Per-page passes such as layout detection charge each batch against this limit, not the whole document; only `max_pages` bounds the rasters retained across a document (GH#1721). |
...
| Value | Description |
|-------|-------------|
| `Pdf` | Metadata extracted from a PDF document. — Fields (flattened into the tagged object): `pdfVersion`: `String`, `producer`: `String`, `isEncrypted`: `bool`, `width`: `int`, `height`: `int`, `pageCount`: `int`, `scannedConfidence`: `double`, `scannedPages`: `List<int>`, `fabricatedTextPages`: `List<int>`, `implausibleTextPages`: `List<int>`, `layoutGatedPages`: `List<int>`, `layoutGateReasons`: `List<String>` |
| `Docx` | Metadata extracted from a DOCX Word document. — Fields (flattened into the tagged object): `coreProperties`: `CoreProperties`, `appProperties`: `DocxAppProperties`, `customProperties`: `Map<String, String>` |
...
| `Archive` | Metadata extracted from an archive (ZIP, TAR, 7Z, etc.). — Fields (flattened into the tagged object): `format`: `String`, `fileCount`: `int`, `fileList`: `List<String>`, `totalSize`: `int`, `compressedSize`: `int` |
...
| `Csv` | Metadata extracted from a CSV or TSV file. — Fields (flattened into the tagged object): `rowCount`: `int`, `columnCount`: `int`, `delimiter`: `String`, `hasHeader`: `bool`, `columnTypes`: `List<String>` |
| `Bibtex` | Metadata extracted from a BibTeX bibliography file. — Fields (flattened into the tagged object): `entryCount`: `PlatformInt64`, `citationKeys`: `List<String>`, `authors`: `List<String>`, `yearRange`: `YearRange`, `entryTypes`: `Map<String, PlatformInt64>` |
| `Citation` | Metadata extracted from a citation file (RIS, PubMed, EndNote). — Fields (flattened into the tagged object): `citationCount`: `PlatformInt64`, `format`: `String`, `authors`: `List<String>`, `yearRange`: `YearRange`, `dois`: `List<String>`, `keywords`: `List<String>` |
...
| `Dbf` | Metadata extracted from a dBASE (DBF) database file. — Fields (flattened into the tagged object): `recordCount`: `PlatformInt64`, `fieldCount`: `PlatformInt64`, `fields`: `List<DbfFieldInfo>` |
...
| `Pst` | Metadata extracted from an Outlook PST archive. — Fields (flattened into the tagged object): `messageCount`: `PlatformInt64` |
| `Audio` | Metadata extracted from an audio or video file. — Fields (flattened into the tagged object): `durationMs`: `int`, `codec`: `String`, `container`: `String`, `sampleRateHz`: `int`, `channels`: `int`, `bitrate`: `int` |
