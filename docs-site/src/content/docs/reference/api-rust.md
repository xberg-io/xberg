...
<!-- alef:hash:895f7457c521144f2556efdfed5143d73bd68b32d03511ffb19750370a56d070 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Rust API Reference <span class="version-badge">v1.2.6</span>
...
```rust
pub fn list_ocr_backend_capabilities() -> Result<Vec<OcrBackendCapabilities>, XbergError>
```
...
**Returns:** `Vec<OcrBackendCapabilities>`
...
```rust
pub fn ocr_backend_supports_language(backend: &str, language: &str) -> Result<bool, XbergError>
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `backend` | `&str` | Yes | Name of a registered OCR backend, as returned by `list_ocr_backends`. Lookup is case-insensitive and resolves the same `paddleocr` alias as backend dispatch. |
| `language` | `&str` | Yes | Language code to check (e.g. `"eng"`, `"deu"`). |
...
```rust
pub async fn get_or_init_backend(model_name: Option<&str>) -> Result<String, XbergError>
```

**Example:**

```rust
let result = get_or_init_backend("value").await?;
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `model_name` | `Option<&str>` | No | The model name |
...
- `Embedding` for an unknown preset, or for an `Llm` or
  `Plugin` model, which keep no local engine.

**Signature:**

```rust
pub fn evict_model(model: &EmbeddingModelType) -> Result<usize, XbergError>
```

**Example:**

```rust
let result = evict_model(&EmbeddingModelType::default())?;
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `model` | `&EmbeddingModelType` | Yes | The embedding model type |
...
```rust
pub fn set_engine_cache_limit(max_resident: Option<String>)
```

**Example:**

```rust
set_engine_cache_limit("value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `max_resident` | `Option<String>` | No | The max resident |
...
- `Reranking` for an unknown preset, or for an `Llm` or
  `Plugin` model, which keep no local engine.

**Signature:**

```rust
pub fn evict_model(model: &RerankerModelType) -> Result<usize, XbergError>
```

**Example:**

```rust
let result = evict_model(&RerankerModelType::default())?;
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `model` | `&RerankerModelType` | Yes | The reranker model type |
...
- `Embedding` for an unknown preset or a `Plugin` model.

**Signature:**

```rust
pub fn evict_model(model: &SparseEmbeddingModelType) -> Result<usize, XbergError>
```

**Example:**

```rust
let result = evict_model(&SparseEmbeddingModelType::default())?;
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `model` | `&SparseEmbeddingModelType` | Yes | The sparse embedding model type |
...
```rust
pub fn evict_model(model: &LateInteractionModelType) -> Result<usize, XbergError>
```

**Example:**

```rust
let result = evict_model(&LateInteractionModelType::default())?;
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `model` | `&LateInteractionModelType` | Yes | The late interaction model type |
...
```rust
let result = clear_engine_caches();
```
...
| `max_concurrent_ocr` | `Option<usize>` | `Default::default()` | Maximum number of Tesseract recognition sessions that run at once. When `None`, recognition follows `max_threads`, reduced to the number of sessions the host's free memory holds. Each session keeps its own page image and recognition working set resident, so a host with many cores and little memory needs this lower than the thread budget. Set it to `4` to keep the fixed limit that releases up to 1.2.6 applied. A value set here is applied as given: neither the thread budget nor the memory reading reduces it. Both of those bound the automatic limit, and a caller who names a number has already decided what the host can carry. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model_path` | `Option<String>` | `Default::default()` | Local DeepSeek-OCR model directory. Takes precedence over `model_id` when present. |
| `model_id` | `Option<String>` | `Default::default()` | Optional Hugging Face repository identifier. Defaults to the checksum-pinned `deepseek-ai/DeepSeek-OCR`. Ignored when `model_path` is set. |
| `hf_revision` | `Option<String>` | `Default::default()` | Optional immutable Hugging Face model revision. The default model is pinned automatically; a custom `model_id` requires this to be set explicitly. |
...
| `dtype` | `Option<CandleDeepseekOcrDtype>` | `Default::default()` | Optional weight precision override; see `CandleDeepseekOcrDtype`. |
...
| `ocr_aggregate` | `Option<f32>` | `None` | OCR recognition confidence, word-count-weighted across every recognized word, when OCR ran; `None` when it did not. |
...
```rust
let result = instance.runs_ocr_on_embedded_images();
```
...
```rust
let result = instance.wants_own_bytes_in_result();
```
...
| `max_response_bytes` | `Option<usize>` | `Default::default()` | Maximum size, in bytes, of a single HTTP response body read from the LLM provider. Bounds response bodies on every non-streaming call (chat completions, embeddings, model listings, …) and the error body read on a failed request; a successful streaming response keeps its own existing frame bounds and is unaffected. `None` (the default) means unbounded, matching liter-llm's own default. Mirrors liter-llm's `client::ClientConfigBuilder::max_response_bytes`, which is native-only (`native-http`, non-`wasm32`) — see `llm::client::build_client_config`. `Some(0)` is rejected by `LlmConfig::validate` rather than reaching liter-llm, which would refuse it at client-build time with the same complaint. |
...
| `supported_languages` | `Vec<String>` | — | The languages this backend declares support for, via `OcrBackend::supported_languages`. An empty list means the backend does not enumerate its languages — it is *not* a statement that the backend supports no languages. `OcrBackend::supported_languages` is a defaulted trait method that returns `vec![]`, and not every backend overrides it: the VLM backend (`llm::vlm_ocr::VlmOcrBackend`) accepts every language via `supports_language` while inheriting the empty default here. Use `ocr_backend_supports_language` to decide whether one specific language is usable — never infer "unsupported" from an empty list. The order of this list is preserved exactly as the backend reported it and is *not* re-sorted. Tesseract's order comes from enumerating installed tessdata files; PaddleOCR's comes from its own `SUPPORTED_LANGUAGES` constant. Re-sorting would disagree with the precedence each backend's own `supports_language` implementation uses internally. |
...
| `security_limits` | `Option<SecurityLimits>` | `None` | Security limits applied when decoding raw image bytes for OCR (GH#1554). Not user-configurable via config files — injected at runtime from `ExtractionConfig::security_limits` before each `process_image` call, the same pattern `Self::acceleration` uses. `ExtractionConfig::security_limits` remains the source of truth and overwrites this field whenever it carries a value; a value set here directly is honoured only when `ExtractionConfig` carries none, so the two cannot drift while a directly-set limit is no longer silently discarded (GH#1651). A backend consulting a `backend_options` override for this call may still let that override win, but in the absence of one this field is what backends should fall back to instead of `SecurityLimits::default()`. `None` means "use `SecurityLimits::default()`", never "disable the check". |
...
| `fabricated_text_pages` | `Option<Vec<u32>>` | `vec![]` | Pages whose text was dominated by fabricated character mappings (1-indexed): `MappingProvenance::Fallback`, a font whose glyph-to-Unicode mapping resolved to a value the extractor chose rather than read from the file (issue #1254). This is a fact about how the text was derived, independent of `scanned_pages`'s raster-based scan detection, and independent of whether the resulting text happens to look structurally like prose (issue #1667: a broken mapping that lands on ordinary letters and punctuation passes every character-shape check but is still fabricated). `None` when `OcrQualityThresholds::enable_provenance_ocr_routing` is `false`; empty when no page qualifies. |
| `implausible_text_pages` | `Option<Vec<u32>>` | `vec![]` | Pages whose native text layer reads as no real detectable language (1-indexed): a `/ToUnicode` CMap (or other mapping tier) that resolves every glyph to *a* character, consistently the WRONG one (e.g. a ROT-shifted mapping), so the page is structurally indistinguishable from real prose to `fabricated_text_pages`'s provenance check and to every character-shape heuristic (issue #1696; issue #1667's `quality_score: 1.0` with no warning on such a page is the same underlying gap). This is a content-plausibility fact, independent of `fabricated_text_pages` and of `scanned_pages`. `None` when `OcrQualityThresholds::enable_plausibility_ocr_routing` is `false`; empty when no page qualifies. |
...
| `max_content_size` | `usize` | `104857600` | Maximum string growth and decoded image allocation per operation (100 MB). Per-page passes such as layout detection charge each batch against this limit, not the whole document; only `max_pages` bounds the rasters retained across a document (GH#1721). |
...
| Value | Description |
|-------|-------------|
| `Pdf` | Metadata extracted from a PDF document. — Fields (flattened into the tagged object): `pdf_version`: `String`, `producer`: `String`, `is_encrypted`: `bool`, `width`: `i64`, `height`: `i64`, `page_count`: `u32`, `scanned_confidence`: `f32`, `scanned_pages`: `Vec<u32>`, `fabricated_text_pages`: `Vec<u32>`, `implausible_text_pages`: `Vec<u32>`, `layout_gated_pages`: `Vec<u32>`, `layout_gate_reasons`: `Vec<String>` |
| `Docx` | Metadata extracted from a DOCX Word document. — Fields (flattened into the tagged object): `core_properties`: `CoreProperties`, `app_properties`: `DocxAppProperties`, `custom_properties`: `HashMap<String, serde_json::Value>` |
| `Excel` | Metadata extracted from an Excel spreadsheet. — Fields (flattened into the tagged object): `sheet_count`: `u32`, `sheet_names`: `Vec<String>` |
| `Email` | Metadata extracted from an email message (EML/MSG). — Fields (flattened into the tagged object): `from_email`: `String`, `from_name`: `String`, `to_emails`: `Vec<String>`, `cc_emails`: `Vec<String>`, `bcc_emails`: `Vec<String>`, `message_id`: `String`, `attachments`: `Vec<String>` |
| `Pptx` | Metadata extracted from a PowerPoint presentation. — Fields (flattened into the tagged object): `slide_count`: `u32`, `slide_names`: `Vec<String>`, `image_count`: `u32`, `table_count`: `u32` |
| `Archive` | Metadata extracted from an archive (ZIP, TAR, 7Z, etc.). — Fields (flattened into the tagged object): `format`: `String`, `file_count`: `u32`, `file_list`: `Vec<String>`, `total_size`: `u64`, `compressed_size`: `u64` |
| `Image` | Metadata extracted from a raster or vector image. — Fields (flattened into the tagged object): `width`: `u32`, `height`: `u32`, `format`: `String`, `exif`: `HashMap<String, String>` |
| `Xml` | Metadata extracted from an XML document. — Fields (flattened into the tagged object): `element_count`: `u32`, `unique_elements`: `Vec<String>` |
| `Text` | Metadata extracted from a plain-text file. — Fields (flattened into the tagged object): `line_count`: `u32`, `word_count`: `u32`, `character_count`: `u32`, `headers`: `Vec<String>`, `links`: `Vec<MarkdownLink>`, `code_blocks`: `Vec<MarkdownCodeBlock>` |
| `Html` | Metadata extracted from an HTML document. — Fields (flattened into the tagged object): `title`: `String`, `description`: `String`, `keywords`: `Vec<String>`, `author`: `String`, `canonical_url`: `String`, `base_href`: `String`, `language`: `String`, `text_direction`: `TextDirection`, `open_graph`: `HashMap<String, String>`, `twitter_card`: `HashMap<String, String>`, `meta_tags`: `HashMap<String, String>`, `headers`: `Vec<HeaderMetadata>`, `links`: `Vec<LinkMetadata>`, `images`: `Vec<ImageMetadataType>`, `structured_data`: `Vec<StructuredData>` |
| `Ocr` | Metadata produced by an OCR pipeline. — Fields (flattened into the tagged object): `language`: `String`, `psm`: `i32`, `output_format`: `String`, `table_count`: `u32`, `table_rows`: `u32`, `table_cols`: `u32` |
| `Csv` | Metadata extracted from a CSV or TSV file. — Fields (flattened into the tagged object): `row_count`: `u32`, `column_count`: `u32`, `delimiter`: `String`, `has_header`: `bool`, `column_types`: `Vec<String>` |
| `Bibtex` | Metadata extracted from a BibTeX bibliography file. — Fields (flattened into the tagged object): `entry_count`: `usize`, `citation_keys`: `Vec<String>`, `authors`: `Vec<String>`, `year_range`: `YearRange`, `entry_types`: `HashMap<String, usize>` |
| `Citation` | Metadata extracted from a citation file (RIS, PubMed, EndNote). — Fields (flattened into the tagged object): `citation_count`: `usize`, `format`: `String`, `authors`: `Vec<String>`, `year_range`: `YearRange`, `dois`: `Vec<String>`, `keywords`: `Vec<String>` |
| `FictionBook` | Metadata extracted from a FictionBook (FB2) e-book. — Fields (flattened into the tagged object): `genres`: `Vec<String>`, `sequences`: `Vec<String>`, `annotation`: `String` |
| `Dbf` | Metadata extracted from a dBASE (DBF) database file. — Fields (flattened into the tagged object): `record_count`: `usize`, `field_count`: `usize`, `fields`: `Vec<DbfFieldInfo>` |
| `Jats` | Metadata extracted from a JATS (Journal Article Tag Suite) XML file. — Fields (flattened into the tagged object): `copyright`: `String`, `license`: `String`, `history_dates`: `HashMap<String, String>`, `contributor_roles`: `Vec<ContributorRole>` |
| `Epub` | Metadata extracted from an EPUB e-book. — Fields (flattened into the tagged object): `coverage`: `String`, `dc_format`: `String`, `relation`: `String`, `source`: `String`, `dc_type`: `String`, `cover_image`: `String` |
| `Pst` | Metadata extracted from an Outlook PST archive. — Fields (flattened into the tagged object): `message_count`: `usize` |
| `Audio` | Metadata extracted from an audio or video file. — Fields (flattened into the tagged object): `duration_ms`: `u64`, `codec`: `String`, `container`: `String`, `sample_rate_hz`: `u32`, `channels`: `u16`, `bitrate`: `u32` |
| `Code` | Code (tree-sitter analyzable source). Carries the structural chunks (function, class, and module boundaries) produced by the tree-sitter extractor, consumed by the chunking pipeline to emit structure-aware `Chunk`s instead of falling back to text-based splitting. Wraps `CodeMetadata` (a named struct) rather than `Vec<CodeChunkInfo>` directly: `FormatMetadata` is internally tagged (`#[serde(tag = "format_type")]`), and serde cannot serialize a tagged newtype variant that wraps a sequence — the tag has no map to live in. Wrapping a struct gives serde a map to hold the tag, and keeps this variant shape consistent with every sibling (`Variant(XMetadata)`) so the derived OpenAPI discriminator can reference a named component schema. — Fields (flattened into the tagged object): `chunks`: `Vec<CodeChunkInfo>`, `data`: `CodeDataNode` |
