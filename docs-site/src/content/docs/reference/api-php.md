...
<!-- alef:hash:f921b7463bef25074c1e7d618c52668e0d2847332343d478e08e83934b55b614 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## PHP API Reference <span class="version-badge">v1.2.6</span>
...
```php
public static function listOcrBackendCapabilities(): array<OcrBackendCapabilities>
```

**Example:**

```php
$result = listOcrBackendCapabilities();
```
...
```php
$result = ocrBackendSupportsLanguage("value", "value");
```
...
| `minReliableLanguageChunkRatio` | `float` | `0.1` | Minimum fraction of a page's language-detection chunks that whatlang classifies as reliable (`Info::is_reliable()`) before the page is trusted as legible (issue #1696). Below this AND below the mean-confidence guard together, the page's text layer is treated as implausible and routed to OCR. Only used when `enable_plausibility_ocr_routing` is `true`. |
...
| `maxContentSize` | `int` | `104857600` | Maximum string growth and decoded image allocation per operation (100 MB). Per-page passes such as layout detection charge each batch against this limit, not the whole document; only `max_pages` bounds the rasters retained across a document (GH#1721). |
...
| Value | Description |
|-------|-------------|
| `Pdf` | Metadata extracted from a PDF document. — Fields (flattened into the tagged object): `pdfVersion`: `string`, `producer`: `string`, `isEncrypted`: `bool`, `width`: `int`, `height`: `int`, `pageCount`: `int`, `scannedConfidence`: `float`, `scannedPages`: `array<int>`, `fabricatedTextPages`: `array<int>`, `implausibleTextPages`: `array<int>`, `layoutGatedPages`: `array<int>`, `layoutGateReasons`: `array<string>` |
| `Docx` | Metadata extracted from a DOCX Word document. — Fields (flattened into the tagged object): `coreProperties`: `CoreProperties`, `appProperties`: `DocxAppProperties`, `customProperties`: `array<string, mixed>` |
| `Excel` | Metadata extracted from an Excel spreadsheet. — Fields (flattened into the tagged object): `sheetCount`: `int`, `sheetNames`: `array<string>` |
...
| `Pptx` | Metadata extracted from a PowerPoint presentation. — Fields (flattened into the tagged object): `slideCount`: `int`, `slideNames`: `array<string>`, `imageCount`: `int`, `tableCount`: `int` |
| `Archive` | Metadata extracted from an archive (ZIP, TAR, 7Z, etc.). — Fields (flattened into the tagged object): `format`: `string`, `fileCount`: `int`, `fileList`: `array<string>`, `totalSize`: `int`, `compressedSize`: `int` |
| `Image` | Metadata extracted from a raster or vector image. — Fields (flattened into the tagged object): `width`: `int`, `height`: `int`, `format`: `string`, `exif`: `array<string, string>` |
| `Xml` | Metadata extracted from an XML document. — Fields (flattened into the tagged object): `elementCount`: `int`, `uniqueElements`: `array<string>` |
| `Text` | Metadata extracted from a plain-text file. — Fields (flattened into the tagged object): `lineCount`: `int`, `wordCount`: `int`, `characterCount`: `int`, `headers`: `array<string>`, `links`: `array<MarkdownLink>`, `codeBlocks`: `array<MarkdownCodeBlock>` |
| `Html` | Metadata extracted from an HTML document. — Fields (flattened into the tagged object): `title`: `string`, `description`: `string`, `keywords`: `array<string>`, `author`: `string`, `canonicalUrl`: `string`, `baseHref`: `string`, `language`: `string`, `textDirection`: `TextDirection`, `openGraph`: `array<string, string>`, `twitterCard`: `array<string, string>`, `metaTags`: `array<string, string>`, `headers`: `array<HeaderMetadata>`, `links`: `array<LinkMetadata>`, `images`: `array<ImageMetadataType>`, `structuredData`: `array<StructuredData>` |
...
| `Csv` | Metadata extracted from a CSV or TSV file. — Fields (flattened into the tagged object): `rowCount`: `int`, `columnCount`: `int`, `delimiter`: `string`, `hasHeader`: `bool`, `columnTypes`: `array<string>` |
| `Bibtex` | Metadata extracted from a BibTeX bibliography file. — Fields (flattened into the tagged object): `entryCount`: `int`, `citationKeys`: `array<string>`, `authors`: `array<string>`, `yearRange`: `YearRange`, `entryTypes`: `array<string, int>` |
| `Citation` | Metadata extracted from a citation file (RIS, PubMed, EndNote). — Fields (flattened into the tagged object): `citationCount`: `int`, `format`: `string`, `authors`: `array<string>`, `yearRange`: `YearRange`, `dois`: `array<string>`, `keywords`: `array<string>` |
...
| `Dbf` | Metadata extracted from a dBASE (DBF) database file. — Fields (flattened into the tagged object): `recordCount`: `int`, `fieldCount`: `int`, `fields`: `array<DbfFieldInfo>` |
| `Jats` | Metadata extracted from a JATS (Journal Article Tag Suite) XML file. — Fields (flattened into the tagged object): `copyright`: `string`, `license`: `string`, `historyDates`: `array<string, string>`, `contributorRoles`: `array<ContributorRole>` |
...
| `Pst` | Metadata extracted from an Outlook PST archive. — Fields (flattened into the tagged object): `messageCount`: `int` |
