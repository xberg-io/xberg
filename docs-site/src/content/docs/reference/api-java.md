...
<!-- alef:hash:6be51ed6e7f01c2a376ba002412248e764431e1beff89f0d93230b3affba5f55 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Java API Reference <span class="version-badge">v1.2.6</span>
...
```java
public static List<OcrBackendCapabilities> listOcrBackendCapabilities() throws XbergRsException
```

**Example:**

```java
var result = listOcrBackendCapabilities();
```

**Returns:** `List<OcrBackendCapabilities>`

**Errors:** Throws `XbergRsException`.

---

#### ocrBackendSupportsLanguage()
...
```java
public static boolean ocrBackendSupportsLanguage(String backend, String language) throws XbergRsException
```

**Example:**

```java
var result = ocrBackendSupportsLanguage("value", "value");
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `backend` | `String` | Yes | Name of a registered OCR backend, as returned by `list_ocr_backends`. Lookup is case-insensitive and resolves the same `paddleocr` alias as backend dispatch. |
| `language` | `String` | Yes | Language code to check (e.g. `"eng"`, `"deu"`). |
...
| `ocrAggregate` | `@Nullable Float` | `null` | OCR recognition confidence, word-count-weighted across every recognized word, when OCR ran; `null` when it did not. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mimeDetectionPolicy` | `MimeDetectionPolicy` | `MimeDetectionPolicy.PREFER_CONTENT` | Controls whether MIME inference prefers content, a supported extension, or content alone. |
...
| `securityLimits` | `@Nullable SecurityLimits` | `null` | Security limits applied when decoding raw image bytes for OCR (GH#1554). Not user-configurable via config files — injected at runtime from `ExtractionConfig.security_limits` before each `process_image` call, the same pattern `Self.acceleration` uses. `ExtractionConfig.security_limits` remains the source of truth and overwrites this field whenever it carries a value; a value set here directly is honoured only when `ExtractionConfig` carries none, so the two cannot drift while a directly-set limit is no longer silently discarded (GH#1651). A backend consulting a `backend_options` override for this call may still let that override win, but in the absence of one this field is what backends should fall back to instead of `SecurityLimits.default()`. `null` means "use `SecurityLimits.default()`", never "disable the check". |
...
| `maxContentSize` | `long` | `104857600` | Maximum string growth and decoded image allocation per operation (100 MB). Per-page passes such as layout detection charge each batch against this limit, not the whole document; only `max_pages` bounds the rasters retained across a document (GH#1721). |
...
| Value | Description |
|-------|-------------|
| `PDF` | Metadata extracted from a PDF document. — Fields (flattened into the tagged object): `pdfVersion`: `String`, `producer`: `String`, `isEncrypted`: `boolean`, `width`: `long`, `height`: `long`, `pageCount`: `int`, `scannedConfidence`: `float`, `scannedPages`: `List<Integer>`, `fabricatedTextPages`: `List<Integer>`, `implausibleTextPages`: `List<Integer>`, `layoutGatedPages`: `List<Integer>`, `layoutGateReasons`: `List<String>` |
| `DOCX` | Metadata extracted from a DOCX Word document. — Fields (flattened into the tagged object): `coreProperties`: `CoreProperties`, `appProperties`: `DocxAppProperties`, `customProperties`: `Map<String, Object>` |
| `EXCEL` | Metadata extracted from an Excel spreadsheet. — Fields (flattened into the tagged object): `sheetCount`: `int`, `sheetNames`: `List<String>` |
| `EMAIL` | Metadata extracted from an email message (EML/MSG). — Fields (flattened into the tagged object): `fromEmail`: `String`, `fromName`: `String`, `toEmails`: `List<String>`, `ccEmails`: `List<String>`, `bccEmails`: `List<String>`, `messageId`: `String`, `attachments`: `List<String>` |
| `PPTX` | Metadata extracted from a PowerPoint presentation. — Fields (flattened into the tagged object): `slideCount`: `int`, `slideNames`: `List<String>`, `imageCount`: `int`, `tableCount`: `int` |
| `ARCHIVE` | Metadata extracted from an archive (ZIP, TAR, 7Z, etc.). — Fields (flattened into the tagged object): `format`: `String`, `fileCount`: `int`, `fileList`: `List<String>`, `totalSize`: `long`, `compressedSize`: `long` |
| `IMAGE` | Metadata extracted from a raster or vector image. — Fields (flattened into the tagged object): `width`: `int`, `height`: `int`, `format`: `String`, `exif`: `Map<String, String>` |
| `XML` | Metadata extracted from an XML document. — Fields (flattened into the tagged object): `elementCount`: `int`, `uniqueElements`: `List<String>` |
| `TEXT` | Metadata extracted from a plain-text file. — Fields (flattened into the tagged object): `lineCount`: `int`, `wordCount`: `int`, `characterCount`: `int`, `headers`: `List<String>`, `links`: `List<MarkdownLink>`, `codeBlocks`: `List<MarkdownCodeBlock>` |
| `HTML` | Metadata extracted from an HTML document. — Fields (flattened into the tagged object): `title`: `String`, `description`: `String`, `keywords`: `List<String>`, `author`: `String`, `canonicalUrl`: `String`, `baseHref`: `String`, `language`: `String`, `textDirection`: `TextDirection`, `openGraph`: `Map<String, String>`, `twitterCard`: `Map<String, String>`, `metaTags`: `Map<String, String>`, `headers`: `List<HeaderMetadata>`, `links`: `List<LinkMetadata>`, `images`: `List<ImageMetadataType>`, `structuredData`: `List<StructuredData>` |
| `OCR` | Metadata produced by an OCR pipeline. — Fields (flattened into the tagged object): `language`: `String`, `psm`: `int`, `outputFormat`: `String`, `tableCount`: `int`, `tableRows`: `int`, `tableCols`: `int` |
| `CSV` | Metadata extracted from a CSV or TSV file. — Fields (flattened into the tagged object): `rowCount`: `int`, `columnCount`: `int`, `delimiter`: `String`, `hasHeader`: `boolean`, `columnTypes`: `List<String>` |
| `BIBTEX` | Metadata extracted from a BibTeX bibliography file. — Fields (flattened into the tagged object): `entryCount`: `long`, `citationKeys`: `List<String>`, `authors`: `List<String>`, `yearRange`: `YearRange`, `entryTypes`: `Map<String, Long>` |
| `CITATION` | Metadata extracted from a citation file (RIS, PubMed, EndNote). — Fields (flattened into the tagged object): `citationCount`: `long`, `format`: `String`, `authors`: `List<String>`, `yearRange`: `YearRange`, `dois`: `List<String>`, `keywords`: `List<String>` |
| `FICTION_BOOK` | Metadata extracted from a FictionBook (FB2) e-book. — Fields (flattened into the tagged object): `genres`: `List<String>`, `sequences`: `List<String>`, `annotation`: `String` |
| `DBF` | Metadata extracted from a dBASE (DBF) database file. — Fields (flattened into the tagged object): `recordCount`: `long`, `fieldCount`: `long`, `fields`: `List<DbfFieldInfo>` |
| `JATS` | Metadata extracted from a JATS (Journal Article Tag Suite) XML file. — Fields (flattened into the tagged object): `copyright`: `String`, `license`: `String`, `historyDates`: `Map<String, String>`, `contributorRoles`: `List<ContributorRole>` |
| `EPUB` | Metadata extracted from an EPUB e-book. — Fields (flattened into the tagged object): `coverage`: `String`, `dcFormat`: `String`, `relation`: `String`, `source`: `String`, `dcType`: `String`, `coverImage`: `String` |
| `PST` | Metadata extracted from an Outlook PST archive. — Fields (flattened into the tagged object): `messageCount`: `long` |
| `AUDIO` | Metadata extracted from an audio or video file. — Fields (flattened into the tagged object): `durationMs`: `long`, `codec`: `String`, `container`: `String`, `sampleRateHz`: `int`, `channels`: `short`, `bitrate`: `int` |
| `CODE` | Code (tree-sitter analyzable source). Carries the structural chunks (function, class, and module boundaries) produced by the tree-sitter extractor, consumed by the chunking pipeline to emit structure-aware `Chunk`s instead of falling back to text-based splitting. Wraps `CodeMetadata` (a named struct) rather than `List<CodeChunkInfo>` directly: `FormatMetadata` is internally tagged (`#[serde(tag = "format_type")]`), and serde cannot serialize a tagged newtype variant that wraps a sequence — the tag has no map to live in. Wrapping a struct gives serde a map to hold the tag, and keeps this variant shape consistent with every sibling (`Variant(XMetadata)`) so the derived OpenAPI discriminator can reference a named component schema. — Fields (flattened into the tagged object): `chunks`: `List<CodeChunkInfo>`, `data`: `CodeDataNode` |
...
| Value | Description |
|-------|-------------|
| `CONTEXT` | Unchanged context line. — Fields: `text`: `String` |
| `ADDED` | Line added in the "after" version. — Fields: `text`: `String` |
| `REMOVED` | Line removed from the "before" version. — Fields: `text`: `String` |
