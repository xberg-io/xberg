...
pub fn generate_parity(manifest: &ParityManifest, output_root: &Utf8Path) -> Result<()> {
    ...
    writeln!(buffer)?;
    writeln!(buffer, "# rubocop:disable Metrics/BlockLength, Layout/LineLength")?;
    writeln!(buffer, "require_relative 'spec_helper'")?;
    writeln!(buffer)?;

    writeln!(buffer, "RSpec.describe 'ExtractionResult field parity' do")?;
    ...
    for (type_name, type_def) in &manifest.types {
        ...
        writeln!(buffer)?;
        writeln!(buffer, "RSpec.describe '{type_name} field parity' do")?;
        let all_fields: Vec<&String> = fields.keys().collect();
        writeln!(buffer, "  %w[{}].each do |field|", all_fields.iter().join(" "))?;
        writeln!(buffer, "    it \"has field '#{{field}}'\" do")?;
        writeln!(
            buffer,
            "      expect(Kreuzberg::{type_name}.method_defined?(field.to_sym) || Kreuzberg::{type_name}.instance_methods.include?(field.to_sym)).to be(true)"
        )?;
        writeln!(buffer, "    end")?;
        writeln!(buffer, "  end")?;
        writeln!(buffer, "end")?;
    ...
    for (type_name, type_def) in &manifest.types {
        ...
        writeln!(buffer)?;
        writeln!(buffer, "RSpec.describe '{type_name} enum parity' do")?;
        for value in values {
    ...
    writeln!(buffer, "# rubocop:enable Metrics/BlockLength, Layout/LineLength")?;
