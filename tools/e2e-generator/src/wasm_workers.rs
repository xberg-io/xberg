...
pub fn generate_parity(manifest: &ParityManifest, output_root: &Utf8Path) -> Result<()> {
    ...
    writeln!(buf)?;
    writeln!(buf, "import type {{ ExtractionResult }} from \"@kreuzberg/wasm\";")?;
    writeln!(buf, "import {{ extractBytes }} from \"@kreuzberg/wasm\";")?;
    ...
    if let Some(fields) = parity::fields_for_type_and_lang(manifest, "ExtractionResult", lang) {
        ...
        writeln!(buf, "describe(\"parity\", () => {{")?;
        writeln!(buf, "\tit(\"ExtractionResult has all expected fields\", async () => {{")?;
        writeln!(
            buf,
    ...
    if let Some(fields) = parity::fields_for_type_and_lang(manifest, "ExtractionConfig", lang) {
        ...
        writeln!(buf)?;
        writeln!(buf, "\tit(\"ExtractionConfig has all expected fields\", () => {{")?;
    ...
    let path = tests_dir.join("parity.spec.ts");
