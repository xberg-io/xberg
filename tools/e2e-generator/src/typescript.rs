...
export const assertions = {
    ...
    assertLlmUsage(
        ...
        const usage = (result as unknown as PlainRecord).llmUsage
            ?? (result as unknown as PlainRecord).llm_usage;
        if (isEmpty === true) {
            if (usage != null) {
...
fn render_assertions(assertions: &Assertions) -> String {
    ...
    if let Some(pages) = assertions.pages.as_ref() {
        ...
        let layout_classes = pages
            ...
            .unwrap_or_else(|| "null".into());
        buffer.push_str(&format!(
            "    chunkAssertions.assertPages(result, {min}, {exact}, {has_layout}, {layout_classes});\n"
        ));
    }
