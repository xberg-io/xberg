use crate::fixtures::{
    Assertions, EmbedAssertions, EmbedSpec, ExtractionMethod, Fixture, GenerationMode, InputType, RenderAssertions,
};
...
fn render_embed_test_go(fixture: &Fixture) -> Result<String> {
    ...
    let count_lit = assertions
        .count
        .map(|c| format!("kreuzberg.Int({c})"))
        .unwrap_or_else(|| "nil".to_string());
    let dims_lit = assertions
        .dimensions
        .map(|d| format!("kreuzberg.Int({d})"))
        .unwrap_or_else(|| "nil".to_string());
    writeln!(
        code,
        "\tassertEmbedResult(t, result, {count_lit}, {dims_lit}, {}, {}, {}, {})",
