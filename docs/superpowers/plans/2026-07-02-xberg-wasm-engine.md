...
### Task 1: `ner-candle-wasm` feature + wasm build validation (sub-project A)

De-risk Candle-on-wasm first: this task proves `xberg-gliner-candle` compiles to `wasm32` without `tokio-runtime`, and adds the gated feature. If Candle or `tokenizers` cannot compile to wasm, this task's failure is the signal to drop in-binary NER to a later cycle (the injected ORT-Web path in Task 6 still gives NER) — record the outcome either way.

**Files:**
- Modify: `crates/xberg/Cargo.toml` (feature `ner-candle-wasm`; add to `wasm-target`) — via `alef.toml` if this manifest is Alef-managed; verify first.
- Modify: `alef.toml` (if `xberg`/`xberg-wasm` features are generated from it)
- Modify: `crates/xberg-gliner-candle/Cargo.toml` (make any non-wasm dep optional/target-gated)
- Test: build command (no unit test — this is a compile-validation task)

**Interfaces:**
- Produces: cargo feature `ner-candle-wasm` on crate `xberg` (Candle GLiNER2 NER without `tokio-runtime`), included in `wasm-target`.

- [ ] **Step 1: Check ownership of the manifests**

Run: `grep -n "alef" crates/xberg/Cargo.toml crates/xberg-wasm/Cargo.toml`
Expected: confirm whether each is auto-generated (header `auto-generated by alef`). `xberg-wasm/Cargo.toml` IS generated (has the alef header). Edit `alef.toml` for generated manifests, direct-edit otherwise.

- [ ] **Step 2: Add the `ner-candle-wasm` feature to core**

In `crates/xberg/Cargo.toml` (or its `alef.toml` source), add next to `ner-candle`:

```toml
# GLiNER2 Candle NER for wasm: no tokio-runtime, synchronous inference.
ner-candle-wasm = ["ner", "dep:xberg-gliner-candle"]
```

Add `ner-candle-wasm` to the `wasm-target` aggregate list.

- [ ] **Step 3: Target-gate ORT out of `xberg-gliner-candle` for wasm**

In `crates/xberg-gliner-candle/Cargo.toml`, make the `xberg-gliner` dependency non-wasm only (its ORT symbols cannot link on wasm):
...
Move the `xberg-gliner` line out of `[dependencies]`. Feature-gate any code path in `src/` that references `xberg_gliner` behind `#[cfg(not(target_arch = "wasm32"))]`.

- [ ] **Step 4: Regenerate Alef output if manifests are generated**

Run: `task alef:generate`
Expected: `crates/xberg-wasm/Cargo.toml` now lists `ner-candle-wasm` under `[features]` and `wasm-target`.

- [ ] **Step 5: Attempt the wasm build (the validation)**

Run: `cargo build -p xberg-wasm --target wasm32-unknown-unknown --no-default-features --features wasm-target 2>&1 | tail -40`
Expected: SUCCESS. If it fails, capture the first blocking error (likely `tokenizers` or a `candle` backend dep pulling native/`std::fs`). Record it in the commit body and open a follow-up; do NOT fabricate a fix — either target-gate the offending dep or defer in-binary NER.

- [ ] **Step 6: Commit**

```bash
prek run --all-files
git add alef.toml crates/xberg/Cargo.toml crates/xberg-wasm/Cargo.toml crates/xberg-gliner-candle/
git commit -m "feat(wasm): add ner-candle-wasm feature and wasm build gate"
```
