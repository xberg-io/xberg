...
curl -LO https://github.com/kreuzberg-dev/kreuzberg/releases/download/v4.5.2/go-ffi-linux-x86_64.tar.gz
...
- `string`: Version string (e.g., "4.5.2")
...
type PaddleOcrConfig struct {
	...
	Language             string   // Language code
	ModelTier            string   // (v4.5.2) Model tier: "mobile" (default, ~21MB total, fast) or "server" (~172MB, best with GPU)
	Padding              *int     // (v4.5.2) Padding in pixels (0-100) around image before detection. Default: 10
