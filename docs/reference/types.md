...
| `entities` | `Option<Vec<Entity>>` | `None` | Extracted named entities when NER post-processor ran. Populated by NER backends (ONNX or LLM) when `ner` config is enabled. `None` when NER was disabled or did not run. |
| `summary` | `Option<DocumentSummary>` | `None` | Document summary when summarisation ran. Populated by TextRank (extractive, local) or LLM (abstractive, remote). `None` when summarisation was disabled. |
| `translation` | `Option<Translation>` | `None` | Translated content when translation ran. Populated by the liter-llm translation provider when `translation` config is enabled. `None` when translation was disabled. |
| `page_classifications` | `Option<Vec<PageClassification>>` | `None` | Per-page classification labels when page classification ran. Populated when `page_classification` config is enabled. `None` when page classification was disabled. |
| `redaction_report` | `Option<RedactionReport>` | `None` | Redaction findings and replacements applied to the document. Populated by the redaction post-processor when enabled. `None` when redaction was disabled or did not run. |
...
#### Entity <span class="version-badge">v5.0.0-rc.3</span>

Named entity extracted from text via NER.

| Field | Type | Description |
|-------|------|-------------|
| `category` | `EntityCategory` | Detected entity category |
| `text` | `String` | The mention text |
| `start` | `u32` | UTF-8 byte-offset start in `ExtractionResult.content` |
| `end` | `u32` | UTF-8 byte-offset end (exclusive) |
| `confidence` | `Option<f32>` | Confidence in `[0.0, 1.0]`; `None` when backend doesn't expose |

**EntityCategory** — enum: `Person`, `Organization`, `Location`, `Date`, `Time`, `Money`, `Percent`, `Email`, `Phone`, `Url`, `Custom(String)`.

---

#### DocumentSummary <span class="version-badge">v5.0.0-rc.3</span>

Document summary generated by extractive or abstractive summarisation.

| Field | Type | Description |
|-------|------|-------------|
| `text` | `String` | Prose summary |
| `strategy` | `SummaryStrategy` | Which backend produced the summary |
| `token_count` | `Option<u32>` | When known |

**SummaryStrategy** — enum: `Extractive` (default, TextRank pure-Rust), `Abstractive` (liter-llm).

---

#### Translation <span class="version-badge">v5.0.0-rc.3</span>

Translated document content.

| Field | Type | Description |
|-------|------|-------------|
| `target_lang` | `String` | BCP-47 tag (e.g. `"de"`, `"fr-CA"`) |
| `source_lang` | `Option<String>` | BCP-47 tag; `None` when auto-detected |
| `content` | `String` | Translated plain text |
| `formatted_content` | `Option<String>` | Translated Markdown/HTML when `TranslationConfig.preserve_markup = true` |

---

#### PageClassification <span class="version-badge">v5.0.0-rc.3</span>

Per-page classification labels.

| Field | Type | Description |
|-------|------|-------------|
| `page_number` | `u32` | 1-indexed page number |
| `labels` | `Vec<ClassificationLabel>` | Single-label mode yields exactly one; multi-label may yield any subset |

**ClassificationLabel**

| Field | Type | Description |
|-------|------|-------------|
| `label` | `String` | The configured label string |
| `confidence` | `Option<f32>` | When backend reports |

---

#### RedactionReport <span class="version-badge">v5.0.0-rc.3</span>

Redaction findings and replacement tokens applied to the document.

| Field | Type | Description |
|-------|------|-------------|
| `findings` | `Vec<RedactionFinding>` | Individual redaction events in source order |
| `total_redacted` | `u32` | Total redactions applied across the document |

**RedactionFinding**

| Field | Type | Description |
|-------|------|-------------|
| `start` | `u32` | Byte-offset start in the ORIGINAL pre-redaction content |
| `end` | `u32` | Byte-offset end (exclusive) in the original content |
| `category` | `PiiCategory` | Category that fired the redaction |
| `strategy` | `RedactionStrategy` | Strategy applied to this finding |
| `replacement_token` | `String` | String that replaced the original mention (empty for `Drop`) |

**RedactionStrategy** — enum: `Mask` (default, `[REDACTED]`), `Hash` (SHA-256 truncated to 16 hex chars), `TokenReplace` (`[PERSON_1]`, `[PERSON_2]`, …), `Drop` (empty string).

**PiiCategory** — enum with 13 variants: `Email`, `Phone`, `Ssn`, `CreditCard`, `PostalCode`, `IpAddress`, `Iban`, `SwiftBic`, `DateOfBirth`, `Person` (NER), `Organization` (NER), `Location` (NER), `Custom(String)`.

---

#### QrCode <span class="version-badge">v5.0.0-rc.3</span>

Decoded QR code from an image.

| Field | Type | Description |
|-------|------|-------------|
| `payload` | `String` | Decoded text |
| `confidence` | `Option<f32>` | `Some(1.0)` for `rqrr` (successful decode is high-confidence) |
| `bbox` | `Option<QrBoundingBox>` | Pixel-space bounding box when available |
...
| `width` | `u32` | Rectangle width in pixels |
| `height` | `u32` | Rectangle height in pixels |
...
| `caption` | `Option<String>` | `None` | VLM-generated caption describing the image content. Populated when `CaptioningConfig` is enabled and the image meets the `min_image_area` threshold. `None` when captioning is disabled or the image is too small. |
| `qr_codes` | `Option<Vec<QrCode>>` | `None` | Detected and decoded QR codes within this image. Populated when `ExtractionConfig.qr_codes = Some(true)`. Empty array when no QR codes are found. `None` when QR detection is disabled. |
