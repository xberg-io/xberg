...
// Select EasyOCR by name via the `backend` field. The Gleam binding does not
// expose a typed EasyOCR config struct — backend-specific tuning happens at
// the Rust core / plugin layer. EasyOCR uses ISO 639-1 codes ("en", "de", ...).
//
// Requires the EasyOCR plugin to be registered in the Rust core at runtime.
...
pub fn main() {
  case kreuzberg.extract_file_sync("scanned.pdf", option.None, config()) {
    Ok(result) -> io.println("EasyOCR text: " <> result.content)
