...
// Switch the OCR backend to PaddleOCR by name. The Gleam binding exposes
// `paddle_ocr_config` as a JSON-serialised string; pass `option.None` to use
// PaddleOCR defaults, or a JSON config string for advanced tuning.
//
// The PaddleOCR backend requires the Rust core to be built with the
// `paddle-ocr` feature and the ONNX Runtime available at runtime.
...
pub fn main() {
  case kreuzberg.extract_file_sync("scanned.pdf", option.None, config()) {
    Ok(result) -> io.println("PaddleOCR text: " <> result.content)
