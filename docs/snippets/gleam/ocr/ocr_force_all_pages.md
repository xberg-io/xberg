...
// Setting `force_ocr: True` makes the pipeline OCR every page, even if the
// document already contains an extractable text layer.
...
pub fn main() {
  case kreuzberg.extract_file_sync("hybrid.pdf", option.None, config()) {
