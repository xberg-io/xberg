...
pub fn main() {
  case kreuzberg.extract_file_sync("multilingual.pdf", option.None, config()) {
    Ok(result) ->
      case result.detected_languages {
        option.Some(langs) -> {
          io.println(
            "Detected " <> int.to_string(list.length(langs)) <> " language(s):",
          )
          list.each(langs, fn(lang) { io.println("  - " <> lang) })
        }
        option.None -> io.println("No languages detected")
      }
